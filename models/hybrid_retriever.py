...
def _rerank_via_svc(question: str, candidates: list, top_k: int = 6) -> list:
    ...
    Either failure mode here (embed svc unreachable, or reachable but its
    CrossEncoder never loaded — see services/embed_svc/reranker.py's own
    RRF fallback) used to be silent beyond a server-log warning: the caller
    got usable-looking results back either way, with no way to tell "the
    configured reranker actually scored these" apart from "this is a
    lower-quality RRF fusion, not a real rerank". Recorded via
    core.retrieval_status so agents/orchestrator.py can tell the user.
    """
        ...
        try:
            ...
            if data.get("degraded"):
                set_retrieval_warning(
                    "The reranker model isn't loaded on the embed service "
                    "(check RERANKER_MODEL / RERANKER_VARIANT and the "
                    "embed-svc container logs for a load error) — answers "
                    "are using a lower-quality fallback ranking."
                )
            return data["results"]
        except Exception as e:
            last_exc = e
    ...
    set_retrieval_warning(
        "Reranking is unavailable right now (" +
        (describe_embed_svc_error(last_exc) if last_exc else "no embed service configured") +
        ") — set EMBED_SVC_URL / EMBED_SVC_URLS and confirm the embed-svc "
        "container is running. Answers are using a lower-quality fallback "
