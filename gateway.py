...
# Used by the rag_mode history filters, which must treat NULL as "off" (rows
# written by the video/doc routes carry no rag_mode). See the history loader.
...
# Mirrors AgentStudio's SecurityHeadersMiddleware (AgentStudio/backend/app/main.py
...
from auth.google_auth import google_auth_router
...
# AgentStudio/backend is added to sys.path so its `app.*` packages resolve without
...
try:
    import sys as _sys
    _abs_backend_path = os.path.join(os.path.dirname(__file__), "AgentStudio", "backend")
...
# Google sign-in. The routes self-gate on ENABLE_GOOGLE_LOGIN (404 when off),
# so mounting them unconditionally keeps the flag a pure runtime switch — no
# restart-time import branch, and /auth/ui-config stays the single source of
# truth for whether the button renders.
app.include_router(google_auth_router,      prefix="/ainxt/v1/api")
...
if _ABS_ROUTERS_LOADED:
    ...
    #
    # ``app.owner_tag`` imports nothing but hashlib, and AgentStudio/backend is
    ...
    def _abs_serve_generated_file(relative_path: str, user_id: str = ""):
        ...
        Mirrors the logic in AgentStudio/backend/app/main.py::download_generated_file
        ...
        _base = _pathlib.Path(
            ...
            or os.path.join(os.path.dirname(__file__), "AgentStudio", "tmp")
        ).resolve()
...
async def startup():
    ...
    # ------------------------------------------------------------
    # IN-PROCESS RQ WORKER — safety-net consumer for queues that have no
    # dedicated worker container in this deployment. Without a consumer,
    # everything enqueued via core.job_queue sits forever (e.g. codebase
    # indexing never leaves status='running').
    #
    # docker-compose.yml already runs dedicated worker containers for
    # every queue EXCEPT exec_queue (Cowork's run_code sandbox —
    # workers/exec_worker.py exists but has no docker-compose service or
    # start_workers.py --exec flag yet). Consuming ALL_QUEUES here used to
    # make this thread race the dedicated doc-worker container for
    # doc_queue: RQ's default Worker forks a work-horse per job
    # (fork_work_horse), and os.fork() from a background thread of an
    # already multi-threaded gunicorn process is unsafe — the forked child
    # can inherit a lock held by a thread that doesn't exist in it and
    # hang immediately post-fork, before running any job code. The job
    # then sits with a single stale heartbeat until RQ's registry cleanup
    # (run by the healthy doc-worker pool itself) sweeps it into
    # FailedJobRegistry with "AbandonedJobError" — the intermittent doc
    # generation failures reported 2026-09-03. Scoping this thread to just
    # exec_queue removes the race for every queue that already has a real
    # dedicated pool, while keeping the one queue that still needs it.
    #
    # gunicorn runs WORKERS (default 2) copies of *this* process — each
    # one starts its own daemon-thread consumer below; RQ dequeues
    # atomically, so concurrent consumers are safe, not duplicated work.
    # Runs in a plain OS thread (not asyncio) because worker.work() is a
    # blocking loop — never do this on the event loop.
    # Gated by IN_PROCESS_WORKER (default true) so a deployment that wants
    # a separate, independently-scalable worker process instead can set
    # this to false and run `python workers/start_workers.py --exec` on
    # its own (once that flag exists).
    # ------------------------------------------------------------
    if os.getenv("IN_PROCESS_WORKER", "true").lower() == "true":
        try:
            import threading as _threading
            from core.job_queue import Q_EXEC as _Q_EXEC
            _ALL_QUEUES = [_Q_EXEC]
            from workers.start_workers import start_worker as _start_rq_worker

            # RQ's Worker.work() unconditionally calls signal.signal(SIGINT/
            # SIGTERM, ...) to install its own graceful-shutdown handlers,
            # and separately uses SIGALRM (via UnixSignalDeathPenalty) to
            # enforce each job's execution timeout. signal.signal() only
            # works on the main thread of the main interpreter — called from
            # this background thread, the first raised "ValueError: signal
            # only works in main thread" right after the "Listening on ..."
            # log line and before the poll loop ever ran (jobs sat in the
            # queue forever with a "started" worker that never actually
            # consumed); the SIGALRM one would then crash every job the
            # moment it started executing. Both neutralised/swapped here,
            # scoped to this in-process embedding only — `python
            # workers/start_workers.py` run standalone (main thread) keeps
            # real signal handling and the OS-level per-job timeout, since
            # that path isn't touched. The daemon thread already dies with
            # the gunicorn worker process, so RQ's own SIGINT/SIGTERM
            # handling isn't needed here; TimerDeathPenalty (threading.Timer
            # based, not signal-based) still enforces job_execution_timeout.
            from rq.worker import Worker as _RQWorker
            from rq.timeouts import TimerDeathPenalty as _TimerDeathPenalty
            _RQWorker._install_signal_handlers = lambda self: None
            _RQWorker.death_penalty_class = _TimerDeathPenalty

            _threading.Thread(
                target=_start_rq_worker,
                args=(_ALL_QUEUES,),
                daemon=True,
                name="in-process-rq-worker",
            ).start()
            logger.info(f"[Worker] in-process RQ consumer started for queues={_ALL_QUEUES}")
        except Exception as _wexc:
            logger.warning(f"[Worker] in-process RQ consumer failed to start (non-fatal): {_wexc}")

        # Cron scheduler thread — only ever started today by `python
        # workers/start_workers.py --scheduler`, a standalone mode nothing
        # in this in-process setup runs. Without it, EVERY interval job
        # registered in start_workers.py's interval_jobs list silently never
        # fires: kb_stale_recovery, codewiki_stale_recovery,
        # index_stale_recovery (recover_stale_index_locks — the fix for the
        # per-repo Redis lock that outlives a crashed indexing job), plus
        # thread_purge/ad_sync/governance_sla/hitl_watchdog/etc. Started as
        # its own daemon thread here, same pattern as the RQ worker above.
        try:
            from workers.start_workers import _cron_scheduler_thread
            ...
            _threading.Thread(
                ...
                daemon=True,
                name="in-process-cron-scheduler",
            ).start()
            logger.info("[Worker] in-process cron scheduler thread started")
        except Exception as _cexc:
            logger.warning(f"[Worker] in-process cron scheduler failed to start (non-fatal): {_cexc}")
...
# (0.85) that services/doc_router.py uses to decide whether it can trust this
# hint, so the two never drift apart — the gateway must not act on a hint that
# doc_router would insist on re-checking. See the _use_cil_doc_intent block.
try:
    from services.doc_router import _INTENT_TRUST_THRESHOLD as _DOC_INTENT_TRUST_FLOOR
except Exception:  # noqa: BLE001
    _DOC_INTENT_TRUST_FLOOR = float(
        os.getenv("DOC_INTENT_TRUST_THRESHOLD", "0.85") or "0.85"
    )

_AINXT_SESS_KEY_PREFIX = "ainxt:sess:"
_AINXT_SESS_TTL        = _AINXT_SESS_TTL_CFG or 86400  # fallback 24h if not set in .env
# ainxt-api (the RUST CLI-agent backend) is enabled ONLY when the flag is on
# AND it actually has an address. AINXT_API_URL is unset in this deployment, so
# the flag alone defaulted this to True and every /ask turn built the request
# against an empty base URL:
#   f"{_AINXT_API_URL}/sessions" -> "/sessions"
# which httpx rejects with "Request URL is missing an 'http://' or 'https://'
# protocol". That produced a guaranteed-failing round trip plus an ERROR log on
# EVERY chat turn before falling back to model_router.
#
# It also had a quieter and worse effect: three doc-generation branches below
# are gated on `not _AINXT_API_ENABLED` (STEP 5.6-pre edit-followup, and the
# two STEP 5.6 doc paths), on the assumption that when ainxt-api is enabled the
# CLI agent owns document handling. With the flag stuck True but the backend
# unreachable, NOBODY owned them — the gateway skipped its own doc paths and
# handed off to a service that could never answer.
#
# Requiring a non-empty URL makes the flag mean what it says. Set
# AINXT_API_ENABLED=false to disable explicitly even when a URL is configured.
_AINXT_API_ENABLED     = (
    os.getenv("AINXT_API_ENABLED", "true").lower().strip() in ("1", "true", "yes")
    and bool((_AINXT_API_URL or "").strip())
)
...
async def ask_ai(q: Question, request: Request, authorization: Optional[str] = _Header(default=None)):
    ...
    if (not q.ephemeral
        ...
        try:
            ...
            #   • CIL is CONFIDENT ENOUGH **given that a prior document exists**
            #     in this chat. Committing a turn to doc generation is
            #     irreversible from the user's point of view: we enqueue a job
            #     and return {route:"doc"} instead of streaming an answer.
            #
            #     The failure this guards: after "generate a doc on weather
            #     report", the follow-up "what does it say about wind and
            #     precipitation" was classified doc_intent='extract' at
            #     confidence 0.60, so the gateway enqueued a SECOND document
            #     instead of answering. The authoritative classifier disagreed
            #     emphatically — the worker's resolve_doc_plan() returned
            #     intent='none' at 0.95, reasoning "The user is asking a question
            #     ABOUT a previously generated document" — but the job was
            #     already running by then.
            #
            #     The floor is applied ONLY when _doc_mem is non-empty, i.e. this
            #     chat already produced a document. That is exactly the window
            #     where "make me a doc" and "tell me about the doc you made" are
            #     confusable. Measured on real traffic, genuine first-time
            #     "generate" turns score ~0.80 — below the 0.85 threshold — so
            #     applying the floor unconditionally would add a redundant
            #     classifier call to ordinary doc generation for no benefit.
            #     Scoping it to the ambiguity window keeps the fast path intact
            #     for the common case and pays the extra call only where the
            #     0.60-confidence misroute can actually happen.
            #
            #     Threshold source: services/doc_router.py only trusts this very
            #     same CIL hint at >= DOC_INTENT_TRUST_THRESHOLD before skipping
            #     its own classify() call, so the gateway should not commit on a
            #     hint doc_router would insist on re-checking.
            _cil_doc_prior_docs = bool((_doc_mem or "").strip())
            _cil_doc_conf_ok = (
                _rc is not None
                and _rc.conv_state is not None
                and (
                    _rc.conv_state.doc_intent == "none"       # negative: cheap + safe to trust
                    or not _cil_doc_prior_docs                # no prior doc → no ambiguity
                    or float(_rc.conv_state.doc_confidence or 0.0) >= _DOC_INTENT_TRUST_FLOOR
                )
            )
            _use_cil_doc_intent = (
                _PIPELINE_V2
                ...
                and _cil_doc_conf_ok
            )
            if (_PIPELINE_V2 and _rc is not None and _rc.conv_state is not None
                    and not _skip_intent_model and _cil_has_doc_signal
                    and _rc.conv_state.doc_reason == "model"
                    and not _cil_doc_conf_ok):
                logger.info(
                    "[doc_intent] CIL doc-intent below trust floor — deferring to "
                    f"authoritative classifier | request_id={request_id!r} "
                    f"cil_intent={_rc.conv_state.doc_intent!r} "
                    f"cil_conf={float(_rc.conv_state.doc_confidence or 0.0):.2f} "
                    f"floor={_DOC_INTENT_TRUST_FLOOR} prior_docs=True"
                )
                    ...
                    if not _probe:
                        ...
                        try:
                            _has_chat_ctx = _pdb.query(_ProbeCM.id).filter(
                                ...
                                # NULL counts as "off" — video/doc-route turns are
                                # written without rag_mode. See the history loader.
                                _sa_or(_ProbeCM.rag_mode == "off",
                                       _ProbeCM.rag_mode.is_(None)),
                            ).first() is not None
                        finally:
                            _pdb.close()
                except Exception:  # noqa: BLE001
                            ...
                            try:
                                _rows = (
                                    ...
                                    .filter(
                                        ...
                                        # NULL counts as "off" — see the history loader.
                                        _sa_or(_DocCM.rag_mode == "off",
                                               _DocCM.rag_mode.is_(None)),
                                        or_(
    ...
    #   duration_secs: 4–8 seconds                     (default: 8)
    # CIL infers both from the user's phrasing; the routing block clamps them.
    # The 4–8 s window is a PRODUCT policy (cost + LRO timeout), narrower than
    # what Veo itself accepts — the single source of truth is
    # cil/intent.py::_VID_MIN_DURATION/_VID_MAX_DURATION, imported below rather
    # than hardcoded so the clamp can never drift from the CIL prompt schema.
    #
    ...
    if (not q.ephemeral
        ...
        try:
            ...
            from cil.intent import (
                _VID_MIN_DURATION as _VID_MIN,
                _VID_MAX_DURATION as _VID_MAX,
                _VID_DEFAULT_DURATION as _VID_DEF,
            )
            _vid_duration = int(_rc.conv_state.vid_duration_secs or _VID_DEF)
            # Clamp to the 4–8 s product window (mirrors /chat/video-generate's
            # authoritative server-side clamp).
            _vid_duration = max(_VID_MIN, min(_VID_MAX, _vid_duration))
            ...
            return _JR_vid(status_code=200, content={
                ...
                "original_question": q.question or "",
                # Forwarded so /chat/video-generate can persist them on the chat
                # turn. Without this the video turn lands with attachment_ids=[]
                # and nothing links the uploaded image to the conversation, so
                # follow-ups ("what car is it?") get no image context.
                "attachment_ids": _att_ids_vid,
                "rag_mode":       (_rc.rag_mode or "off"),
            })
        ...
        try:
            ...
            try:
                ...
                for a in attachments:
                    # Content source, in order of richness. Keying solely off
                    # parsed_text dropped the attachment entirely whenever that
                    # column was empty but a description existed — the exact case
                    # for images whose Vision pass wrote image_description while
                    # parsed_text stayed NULL. Falling back means an attachment
                    # the user just uploaded can never silently contribute
                    # nothing to the prompt.
                    _src_text = (
                        a.parsed_text
                        or getattr(a, "image_description", None)
                        or getattr(a, "image_caption", None)
                        or ""
                    )
                    if not _src_text:
                        continue
                    _truncated = _attach_cap > 0 and len(_src_text) > _attach_cap
                    _ptext = _src_text if _attach_cap <= 0 else _src_text[:_attach_cap]
                    ...
                    _warn = (
                        f"\n[NOTE: this file is {len(_src_text):,} characters; only the first "
                        f"{_attach_cap:,} are shown here ({len(_src_text) - _attach_cap:,} characters were "
                        f"NOT included]. Do not re-request this file — tell the user the data is partial.\n"
                    ) if _truncated else ""
                    # Label images as VISION OUTPUT, not as file text. For an
                    # image attachment parsed_text IS the Gemini-vision
                    # description (chat_router mirrors it into
                    # image_description/image_caption), so a bare "[File: x.jpg]"
                    # header invited two failures: the model treating a machine
                    # description as if the user had typed it, and the model
                    # saying it "cannot see images" while a full description sat
                    # in its context. Mirrors the wording the follow-up
                    # injection already uses ("[Image previously shared: ...]").
                    _is_img = (a.kind or "") == "image"
                    if _is_img:
                        _hdr = (
                            f"[Image attached by the user: {a.file_name} — the text below is an "
                            f"automated vision description of that image, not the user's own words. "
                            f"Treat it as your view of the image and answer from it; do not tell the "
                            f"user you cannot see the image.]"
                        )
                    else:
                        _hdr = (
                            f"[File attached by the user: {a.file_name} — the text below is the "
                            f"extracted contents of that file, not the user's own words.]"
                        )
                    blocks.append(f"{_hdr}{_warn}\n{_ptext}")
                    logger.info(
                        f"[DOCTRACE] L2 attach-inject | corr={request_id} "
                        f"attachment_id={a.id} file={a.file_name!r} "
                        f"kind={a.kind!r} as_image={_is_img} "
                        f"stored_chars={len(_src_text)} injected_chars={len(_ptext)} "
                        f"cap={_attach_cap or 'none'} "
                        f"truncated={_truncated}"
                    )
                if blocks:
                    # "User's message:" (was "User question:") — the delimiter is
                    # the only thing separating attachment content from what the
                    # user actually typed, and not every prompt is a question.
                    original = (
                        "\n\n".join(blocks)
                        + "\n\nUser's message (this, and only this, is what the user typed): "
                        + original
                    )
                    logger.info(
        ...
        if not _messages and not q.cli_mode:
            ...
            if not _messages:
                ...
                try:
                    ...
                    if _rag_mode == "off":
                        # NULL counts as "off". rag_mode is only set by the
                        # streaming chat publishes; the video route
                        # (/chat/video-generate) and the doc fast path omit it, so
                        # their rows land with rag_mode IS NULL. A bare
                        # `rag_mode = 'off'` is FALSE for NULL in SQL, which made
                        # those turns INVISIBLE to Generic history reads — "generate
                        # a video of the car in this image" then "what car is it?"
                        # loaded zero history, so the model had neither the image
                        # nor the conversation. Treat unset as Generic (it is: these
                        # routes never touch the KB).
                        _pg_q = _pg_q.filter(
                            _sa_or(_CM.rag_mode == "off", _CM.rag_mode.is_(None))
                        )
                    _pg_hist = (
                        _pg_q
                ...
                if _pg_hist:
                    ...
                    if _raw_tokens < _TRIGGER_TOKENS:
                        ...
                        try:
                            ...
                            if _rag_mode == "off":
                                # NULL counts as "off" — see the note above.
                                _pg_q2 = _pg_q2.filter(
                                    _sa_or(_CM.rag_mode == "off", _CM.rag_mode.is_(None))
                                )
                            _recent = (
                                _pg_q2
    ...
    # ── Persistent ATTACHMENT context (multi-turn image + document memory) ───
    # SCOPE: this block runs ONLY inside the /ask handler (ask_ai). It replays a
    # COMPACT description for every image AND document shared earlier in THIS
    # chat, so follow-up turns keep context of the asset without re-sending
    # image bytes or the whole parsed document. WEBCHAT/Buddy ONLY: those uploads
    # go through routers/chat_router.py, which persists image_description/
    # image_caption (images) and parsed_text (documents) on the ChatAttachment
    # row at upload time. CLI images are untouched (CLI sends base64 images
    # inline via q.images and never gets a ChatAttachment row), so this block is
    # a no-op for CLI traffic — it just won't find any rows to replay.
    #
    # Why it is needed: the web frontend clears attachment_ids after the upload
    # turn, so STEP 0 only injects attachment content for the CURRENT request —
    # an upload influenced turn 1 and later turns lost it entirely. Here we look
    # up ChatAttachment rows referenced by past chat_messages.attachment_ids and
    # replay a description for each.
    #
    # DOCUMENTS were previously excluded (the query hardcoded kind=="image"),
    # so "summarise the PDF I uploaded" one turn later had no document context
    # at all and the model answered from nothing. Documents are now replayed
    # too, with a SEPARATE (smaller) per-asset budget: an image caption is
    # already a summary, whereas parsed_text is the raw document body, so
    # replaying 600 chars of a 200-page PDF is near-useless while replaying the
    # whole thing would blow the context window on every turn. _DOC_CTX_CHARS is
    # an excerpt sized to re-establish WHAT the document is; when the user
    # actually needs its contents they re-reference it and STEP 0 injects the
    # full parsed_text.
    #
    # Guardrails: last _IMG_CTX_MAX images and _DOC_CTX_MAX documents; per-asset
    # char caps; redacted via _hist_redact; fully best-effort (never breaks the
    # turn). Does NOT re-send bytes and does NOT touch other endpoints.
    # Shared injection budget for BOTH history blocks below (attachments and
    # generated documents). Initialised out here, not inside either try, so a
    # failure in the first block can never leave the second one referencing an
    # undefined name — that NameError would be swallowed by its own except and
    # silently disable generated-doc context.
    try:
        _hist_ctx_remaining = int(
            os.getenv("ASK_HIST_CTX_TOTAL_CHARS", "200000") or "200000"
        )
    except Exception:  # noqa: BLE001
        _hist_ctx_remaining = 200000
    _hist_ctx_seen: set = set()

    try:
        if _messages:   # same-chat history turns present (web/Buddy AND cli)
            # POLICY: the CONTENT of every attachment in this chat reaches the
            # model, in FULL. The CIL only needs to know an attachment EXISTS;
            # the model needs enough to answer FROM it. Truncating here is what
            # produced "I don't have any prior context about..." on follow-ups.
            #
            # Images have no per-item cap: image_description is already a Vision
            # summary and measures 43-3034 chars in practice, so sending it whole
            # is nearly free. The previous 600-char cap threw away most of a
            # 3000-char description for no benefit.
            #
            # Documents are the only material cost, so they share ONE total
            # budget consumed newest-first instead of a per-document cap — one
            # relevant document arriving COMPLETE beats five arriving uselessly
            # truncated. Generated docs (below) draw from the same budget so the
            # two blocks cannot combine into an unbounded prompt.
            _IMG_CTX_MAX   = 5     # most-recent N images to remember
            _DOC_CTX_MAX   = 5     # most-recent N documents to remember
            ...
            try:
                ...
                # ── Fallback: attachments owned by this chat but NOT referenced
                # by any chat_messages.attachment_ids ────────────────────────
                # chat_messages.attachment_ids is only populated by routes that
                # bother to forward it, and several do not:
                #   • the video route (gateway {route:"video"} → /chat/video-generate)
                #     publishes its chat-history record with no attachment_ids at
                #     all, so "generate a video of the car in this image" followed
                #     by "what car is it?" had zero image context.
                #   • generated docs/images are mirrored into chat_attachments
                #     (kind="generated") with a fresh id that is never written
                #     back onto any message row.
                # Since chat_attachments.chat_id is now reliably populated (the
                # FK/eager-create fix in db/chat_rows.py), scanning by chat_id
                # catches every asset in the chat regardless of which route
                # persisted the turn — so this works for present and future
                # routes without each one having to remember to plumb ids
                # through. The existing per-kind caps and char budgets below
                # still bound how much of it reaches the prompt.
                try:
                    for _crow in (
                        _img_db.query(_ImgCA.id)
                        .filter(_ImgCA.chat_id == _chat_id)
                        .order_by(_ImgCA.created_at.desc())
                        .limit(50)
                        .all()
                    ):
                        if _crow[0] and _crow[0] not in _att_ids_seen:
                            _att_ids_seen.append(_crow[0])
                except Exception as _att_scan_err:  # noqa: BLE001
                    logger.debug(f"[L2-att] chat_id attachment scan skipped: {_att_scan_err}")

                # Attachments referenced by THIS turn are already injected in
                # full by STEP 0 — replaying an excerpt of them here would be a
                # strictly worse duplicate, so exclude them.
                _cur_att_ids = set(getattr(q, "attachment_ids", None) or [])
                if _att_ids_seen:
                    # Resolve every attachment row (images AND documents); the
                    # kind is used below to pick the right description field.
                    _att_rows = (
                        _img_db.query(_ImgCA)
                        .filter(_ImgCA.id.in_(_att_ids_seen))
                        .all()
                    )
                    # Preserve newest-first ordering from _att_ids_seen.
                    _att_by_id = {r.id: r for r in _att_rows}
                    _ordered_all = [
                        _att_by_id[_aid] for _aid in _att_ids_seen
                        if _aid in _att_by_id and _aid not in _cur_att_ids
                    ]
                    # Cap images and documents independently so a chat full of
                    # screenshots can't starve the documents (or vice versa).
                    _img_ordered = [r for r in _ordered_all if (r.kind or "") == "image"][:_IMG_CTX_MAX]
                    _doc_ordered = [r for r in _ordered_all if (r.kind or "") != "image"][:_DOC_CTX_MAX]

                    _img_blocks = []
                    for _r in _img_ordered:
                        # Prefer the FULL Vision description. image_caption is a
                        # ~600-char truncation of that same text, so it is a
                        # fallback for rows where no description was stored, not
                        # a first choice.
                        _cap = (
                            _r.image_description
                            or _r.parsed_text
                            or _r.image_caption
                            or ""
                        ).strip()
                        if not _cap:
                            continue
                        _cap = _hist_redact(_cap)
                        # Neutral on origin: generated images are ALSO stored with
                        # kind="image" (chat_router's image-generate mirror), so
                        # kind cannot tell an upload from something the assistant
                        # produced. Saying "shared earlier" would be wrong half the
                        # time, so state the fact both cases share instead.
                        _img_blocks.append(
                            f"[Image earlier in this conversation: {_r.file_name}] {_cap}"
                        )
                        logger.info(
                    ...
                    _doc_blocks = []
                    for _r in _doc_ordered:
                        _dtext = (_r.parsed_text or "").strip()
                        if not _dtext:
                            continue
                        _dfull = len(_dtext)
                        if _hist_ctx_remaining <= 0:
                            logger.info(
                                f"[DOCTRACE] L2-doc budget exhausted — skipped | "
                                f"corr={request_id} attachment_id={_r.id} "
                                f"file={_r.file_name!r} stored_chars={_dfull}"
                            )
                            continue
                        _draw = _dtext[:_hist_ctx_remaining]
                        _hist_ctx_remaining -= len(_draw)
                        _hist_ctx_seen.add(_dtext[:200])
                        _dexc = _hist_redact(_draw)
                        # State any truncation as a fact (same rationale as
                        # STEP 0's truncation note): a model handed a silently
                        # cut file tends to conclude the read failed and
                        # re-request it.
                        _dnote = (
                            f" [truncated — {_dfull:,} chars total, {len(_draw):,} shown]"
                            if len(_draw) < _dfull else ""
                        )
                        # kind="generated" IS reliable for documents (set by
                        # services/doc_context.py's mirror), so attribute these
                        # correctly — telling the model the user "shared" a report
                        # the assistant itself wrote invites it to describe a
                        # nonexistent upload.
                        _dorigin = (
                            "generated earlier in this conversation"
                            if (_r.kind or "") == "generated"
                            else "shared earlier by the user"
                        )
                        _doc_blocks.append(
                            f"[Document {_dorigin}: {_r.file_name}]{_dnote}\n{_dexc}"
                        )
                        logger.info(
                            f"[DOCTRACE] L2-doc history-doc-inject | corr={request_id} "
                            f"attachment_id={_r.id} file={_r.file_name!r} "
                            f"stored_chars={_dfull} injected_chars={len(_draw)} "
                            f"budget_left={_hist_ctx_remaining}"
                        )

                    if _img_blocks:
                        _img_ctx = (
                            "[Image(s) are already part of this conversation. The "
                            "descriptions below are automated vision output for them, "
                            "not the user's own words. Answer from them; do not say "
                            "you cannot see the image.]\n" + "\n".join(_img_blocks)
                        )
                        _messages.append({"role": "user", "content": _img_ctx})
                        add_trace(request_id, f"image_context injected ({len(_img_blocks)} images)")
                    if _doc_blocks:
                        _doc_ctx_msg = (
                            "[Document(s) are already part of this conversation. The "
                            "excerpts below are their extracted contents, not the "
                            "user's own words:]\n" + "\n\n".join(_doc_blocks)
                        )
                        _messages.append({"role": "user", "content": _doc_ctx_msg})
                        add_trace(request_id, f"doc_context injected ({len(_doc_blocks)} documents)")
            finally:
                _img_db.close()
    except Exception as _img_ctx_err:
        logger.debug(f"attachment context inject skipped: {_img_ctx_err}")

    # ── Prior GENERATED-DOCUMENT content (answer follow-ups about a doc) ─────
    # Why this exists: after "generate a doc on weather report", asking "what
    # does it say about wind and precipitation" produced "I don't have any prior
    # context about..." — the generated document's TEXT was nowhere in the
    # prompt. Nothing above supplies it:
    #   • chat history holds only the "[DOCJOB:<id>:...]" marker, not content;
    #   • the L2 attachment block above finds attachments via
    #     ChatMessage.attachment_ids, and services/doc_context.py::
    #     mirror_generated_doc_as_attachment writes its kind="generated"
    #     ChatAttachment row WITHOUT linking that id onto any message, so the
    #     row is unreachable from there;
    #   • list_docs_for_chat is already called earlier in this request, but only
    #     for the classifier's TITLE listing (doc_memory_summary) — titles, not
    #     bodies.
    # So we inject the actual content_md of the most recent generated doc(s),
    # which is what makes a grounded answer possible instead of a refusal.
    #
    # Budget is deliberately larger per-item than the attachment excerpt above
    # (a title-only mention is useless when the question is about a detail
    # BURIED in the document) but capped to very few documents. Tune with
    # ASK_GENDOC_CTX_MAX / ASK_GENDOC_CTX_CHARS; set MAX=0 to disable.
    try:
        _GEN_CTX_MAX = int(os.getenv("ASK_GENDOC_CTX_MAX", "3") or "3")
    except Exception:  # noqa: BLE001
        _GEN_CTX_MAX = 3
    try:
        if _messages and _chat_id and _GEN_CTX_MAX > 0:
            from services.doc_context import list_docs_for_chat as _ldfc_ctx
            _gen_mem = _ldfc_ctx(_chat_id, _user_id, limit=_GEN_CTX_MAX,
                                 include_source=True)
            _gen_blocks = []
            for _d in (_gen_mem.docs or [])[:_GEN_CTX_MAX]:
                _md = (getattr(_d, "content_md", "") or "").strip()
                if not _md:
                    continue
                # Already injected verbatim by the attachment block above (a
                # generated doc mirrored as a kind="generated" ChatAttachment
                # that IS linked to a message shows up in both places).
                if _md[:200] in _hist_ctx_seen:
                    logger.info(
                        f"[DOCTRACE] L2-gendoc already injected above — skipping | "
                        f"corr={request_id} artifact_id={_d.artifact_id}"
                    )
                    continue
                _md_full = len(_md)
                if _hist_ctx_remaining <= 0:
                    logger.info(
                        f"[DOCTRACE] L2-gendoc budget exhausted — skipped | "
                        f"corr={request_id} artifact_id={_d.artifact_id} "
                        f"title={_d.title!r} stored_chars={_md_full}"
                    )
                    continue
                _md_raw = _md[:_hist_ctx_remaining]
                _hist_ctx_remaining -= len(_md_raw)
                _hist_ctx_seen.add(_md[:200])
                _md_exc = _hist_redact(_md_raw)
                _md_note = (
                    f" [truncated — {_md_full:,} chars total, {len(_md_raw):,} shown]"
                    if len(_md_raw) < _md_full else ""
                )
                _gen_blocks.append(
                    f"[Document generated earlier in this conversation: "
                    f"\"{_d.title}\" ({_d.format}, v{_d.version})]{_md_note}\n{_md_exc}"
                )
                logger.info(
                    f"[DOCTRACE] L2-gendoc history-gendoc-inject | corr={request_id} "
                    f"artifact_id={_d.artifact_id} title={_d.title!r} "
                    f"stored_chars={_md_full} injected_chars={len(_md_raw)} "
                    f"budget_left={_hist_ctx_remaining}"
                )
            if _gen_blocks:
                _messages.append({"role": "user", "content": (
                    "[Documents generated earlier in this conversation. Use their "
                    "content to answer questions about them — do NOT say you lack "
                    "context, and do NOT create a new document unless explicitly "
                    "asked:]\n" + "\n\n".join(_gen_blocks)
                )})
                add_trace(request_id, f"gendoc_context injected ({len(_gen_blocks)} docs)")
    except Exception as _gen_ctx_err:  # noqa: BLE001 — best-effort only
        logger.debug(f"generated-doc context inject skipped: {_gen_ctx_err}")
    ...
    if not _is_cli_early:
        try:
            # Import HERE, not borrowed from STEP 5.2's try-block above. Both
            # names used by this step (get_semantic_memory,
            # format_memory_for_prompt) were bound only by 5.2's import, so when
            # `store.semantic_cache` fails to import at MODULE level — e.g. the
            # "EMBED_SVC_URL must be set." raised in this deployment, where
            # embed-svc is not running — none of the three names ever got bound.
            # 5.2 swallowed the real error, then this step died on
            #   UnboundLocalError: cannot access local variable
            #   'get_semantic_memory' where it is not associated with a value
            # which masked the actual cause (missing EMBED_SVC_URL) behind a
            # confusing Python-internals message. A local import lets this step
            # fail with the REAL reason and degrade independently of 5.2.
            from store.semantic_cache import (
                get_semantic_memory,
                format_memory_for_prompt,
            )
...
def get_all_models(request: Request):
    """
    Return all available models grouped by provider — sourced ENTIRELY from
    core.llm_provider_registry (the admin "LLM Providers" screen's data;
    db/models.py's llm_providers/llm_models tables). Used by Chat UI + CLI to
    show a comprehensive model selector. Auto = routing logic. Specific
    selection = bypass routing.

    Previously this endpoint was a hand-maintained literal list keyed off
    ~15 core.model_registry env vars (CLAUDE_PRIMARY_MODEL, OPENAI_CODING_MODEL,
    ...) with hardcoded CLI/IDE-only and web-only channel gating for
    Opus 4.8 / Veo. Those env vars are role-specific overrides that
    install.sh's "LLM Providers" flow never sets (it only ever writes the raw
    API key) — so for any deployment configured purely through the admin
    screen, every one of those constants was blank, and the picker showed
    empty/broken entries. Switching this endpoint to read only the registry
    fixes that at the source, and matches every model source (chat picker,
    model governance, endpoint allowlists) to the same admin-managed data.

    Channel restriction is now per-model: a model's `capabilities.channels`
    list (if an admin sets one via the admin screen) limits which
    client_source values ("platform" | "cli" | "ide-vscode" |
    "ide-jetbrains" | "api") see it; a model with no `channels` entry (the
    default — nothing is seeded with one today) is visible on every channel.
    Per-user access is still governed separately by the model governance
    tables (dept_model_permissions / user_model_permissions), exactly as
    before — this endpoint's result is filtered again client-side against
    /model-governance/my-models.
    """
    _cs = getattr(request.state, "client_source", "platform")

    # modelId = the full concrete model ID used by governance (dept_model_permissions.model_id).
    # id      = also the full model ID now (no more short aliases like "claude"/"gpt" —
    #           those only existed because the built-in blocks needed a stable UI value
    #           independent of the env-var-configured model; the registry's model_id IS
    #           the stable value).
    providers = [
        {
            "provider": "Auto",
    ...
    try:
        ...
        by_provider: dict = {}
        for m in get_enabled_models(channel=_cs):
            caps = m["capabilities"] or {}
            # Ollama defaults to "free" (self-hosted); every other family
            # defaults to "paid" — only used when capabilities.billing_tier
            # wasn't explicitly stored (e.g. an Ollama model synced before
            # this default existed).
            _default_tier = "free" if m["family"] == "ollama" else "paid"
            entry = {
                "id": m["model_id"],
                "modelId": m["model_id"],
                "label": f"{m['display_name']} ({m['model_id']})",
                "hint": m["model_id"],
                "tier": caps.get("billing_tier", _default_tier),
            }
        ...
        logger.warning(f"[all-models] registry read failed: {exc}")
...
@_v1.get("/health", tags=["observability"])
def health():
    checks = {}
...
def circuit_breaker_health(_caller: dict = Depends(require_role("admin"))):
    """Return the current state of all registered circuit breakers.

    Admin-only: consumed exclusively by the Monitoring dashboard (Observe
    section), which is restricted to admins.
    """
...
_abs_dist = _os.path.join(_os.path.dirname(__file__), "AgentStudio", "frontend", "dist")
...
# ── Office add-in task pane (/office-addin/...)
# Optional: mounted only once the bundle has been built (`cd office-addin &&
# npm install && npm run build`). Office manifests cannot use relative paths, so
# the generated manifest points every SourceLocation and IconUrl at
# {AINXT_ADDIN_BASE_URL}/office-addin/... — this mount is what answers those
# requests. Registered BEFORE the SPA catch-all below, which would otherwise
# swallow the path and hand the Office client index.html instead of the pane.
_addin_dist = _os.path.join(_os.path.dirname(__file__), "office-addin", "dist")
if _os.path.isdir(_addin_dist):
    from fastapi.staticfiles import StaticFiles as _SF_addin
    app.mount(
        "/office-addin",
        _SF_addin(directory=_addin_dist, html=False),
        name="office-addin",
    )
    logger.info(f"Serving Office add-in task pane from {_addin_dist}")
