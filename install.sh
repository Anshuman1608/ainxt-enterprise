...
# A git checkout, curl download, tarball extraction or Windows filesystem does
# not reliably preserve the execute bit. Individual steps below used to chmod
# their own script right before running it (kb-setup.sh, sdlc-setup.sh,
# docker/doc-sandbox/build.sh) — but doctor.sh had no such call, so
# verify_install() would find it non-executable and silently skip the
# post-install checks instead of running them, on the exact machines that
# actually need to see whether the install worked. Fix it once, here, for
# every script in the checkout, so a user running any of them by hand never
# hits "Permission denied" either.
chmod_all_scripts() {
  step "Making scripts executable"
  local f
  # Prune node_modules/.git — on a repo that has previously run in --local
  # mode (ai-ui/node_modules exists on the host), those trees can contain
  # thousands of unrelated *.sh files and would make this needlessly slow on
  # every re-run without excluding them.
  while IFS= read -r -d '' f; do
    chmod 755 "$f"
  done < <(find . \( -name node_modules -o -name .git \) -prune -o -name '*.sh' -type f -print0 2>/dev/null)
  ok "chmod 755 applied to all *.sh scripts in $(pwd)"
}

# ── 3. Model provider ───────────────────────────────────────────────────────
PROVIDER=""            # first/primary choice — decides the default chat provider
PROVIDER_KEYS=()       # collected "VAR=value" pairs
# Tracks whether Ollama was picked AT ALL — primary OR as a secondary
# provider via "Add another provider?". Deliberately separate from
# $PROVIDER: $PROVIDER only ever holds the FIRST/primary choice (see its own
# comment), so gating Ollama's model pull/registration on
# `[[ "$PROVIDER" == "ollama" ]]` meant picking, say, Anthropic first and
# then adding Ollama as a second provider left this flag false — Ollama's
# model was never pulled and db/bootstrap_llm_providers.py's auto-discovery
# never ran for it, so its provider row (created separately, elsewhere)
# stayed with ZERO models until an admin manually clicked "Sync" in the LLM
# Providers screen. Cloud providers don't have this problem because
# seed_provider_db() already loops over every configured provider, primary
# or not — confirmed live: a fresh install with Anthropic primary + Ollama
# added second left ollama-env with 0 registered models.
OLLAMA_CONFIGURED="0"
choose_provider() {
...
start_stack() {
  ...
  # Ollama needs no API key, unlike every other family here — so unlike
  # them, its availability shouldn't depend on whether the user happened to
  # pick "Ollama" in the provider menu above. OLLAMA_URL/LOCAL_LLM_BASE_URL
  # are ALREADY wired to a host-installed Ollama whenever one is reachable
  # (use_container_service_names(), earlier), regardless of $PROVIDER — so
  # the gateway will happily route to it, but until now its models stayed
  # invisible in the picker unless the admin opened LLM Providers and clicked
  # "Sync" by hand. Confirmed live: a chat answered via llama3.2:3b through
  # the host Ollama even with ONLY Anthropic configured as a provider — the
  # model was being used but not listed. Registering + auto-discovering
  # whenever a host Ollama is reachable (any models it already has, pulled in
  # a prior session or by hand, get picked up too) fixes that; a BUNDLED
  # Ollama with nothing pulled and not explicitly chosen is left alone below,
  # since there'd be nothing real to discover and forcing a ~2GB download on
  # an Anthropic-only install would be unwanted.
  if host_ollama_usable; then
...
verify_install() {
  ...
  # checkout/curl download/Windows filesystem — chmod_all_scripts() already
  # did this for every script up front, but re-asserting it here means this
  # check still runs even if that step was skipped or something downstream
  # reset the bit.
  chmod 755 ./doctor.sh 2>/dev/null || true
  step "Checking the install"
  # Invoked via `bash` rather than ./doctor.sh so it runs the same way
  # regardless of whether the execute bit actually took — some Windows
  # filesystems mounted into WSL2 don't honor chmod reliably (same reasoning
  # kb-setup.sh/sdlc-setup.sh already use for their own invocation below).
...
EOF
  ...
  if [[ "$INDEX_WORKER_ENABLED" == "1" ]]; then
    say ""
    say "    Codebase indexing ${GRN}on${R}  ${DIM}(index-worker — submit a repo via the Codebase/Index Router)${R}"
  fi
  if [[ "$WITH_CODEWIKI" == "1" ]]; then
    say ""
  ...
  say ""
  # setup_index_worker() offers to start this above; only point at the manual
  # command here if it was declined (or this is a --local install, where it's
  # never offered). start-index-worker.sh brings up embed-svc first
  # (index-worker needs it reachable) if it isn't already, reusing whatever
  # it's already configured with rather than asking again.
  if [[ "$INDEX_WORKER_ENABLED" != "1" ]]; then
...
INDEX_WORKER_ENABLED="0"
setup_index_worker() {
  step "Codebase indexing / semantic code search (optional)"
  say "  Enables AST-based chunking + embedding of a codebase for semantic code"
  say "  search and SDLC pipeline context (the index-worker container)."
  say "  ${DIM}Reuses the Knowledge Base's embedding service (embed-svc) — starts it"
  say "  too if it isn't already running, on whatever model/port it's already${R}"
  say "  ${DIM}configured with, so it doesn't matter whether you set up the${R}"
  say "  ${DIM}Knowledge Base above or not.${R}"

  if [[ "$MODE" == "local" ]]; then
    say "  ${DIM}Codebase indexing is Docker-only — skipping in local mode.${R}"
    return 0
  fi

  if [[ "$ASSUME_YES" == "yes" || -z "$TTY" ]]; then
    say "  ${DIM}non-interactive — leaving codebase indexing disabled${R}"
    say "  ${DIM}enable later with: ./start-index-worker.sh${R}"
    return 0
  fi

  local idx_yn=""
  printf '\n  Set up codebase indexing now? [y/N]: ' > "$TTY"
  read -r idx_yn < "$TTY" || idx_yn="n"
  case "$idx_yn" in
    y|Y|yes|YES) ;;
    *) say "  ${DIM}Skipping — enable later with: ./start-index-worker.sh${R}"; return 0 ;;
  esac

  chmod 755 ./start-index-worker.sh 2>/dev/null || true
  # start-index-worker.sh handles its own errors/warnings and never dies on a
  # failed embed-svc bring-up in a way that would abort this installer — it
  # always returns so this installer's own final summary can still print.
  if bash ./start-index-worker.sh; then
    INDEX_WORKER_ENABLED="1"
  else
    warn "Codebase indexing setup did not finish — re-run ./start-index-worker.sh any time."
  fi
}
...
if [[ "$MODE" == "local" ]]; then
  ...
  start_kafka_consumer_local
  start_ui_local
  # Same reasoning as the Docker-mode branch of start_stack(): Ollama needs
  # no API key, so register + auto-discover its models whenever a host
  # Ollama is reachable at all, regardless of whether it was the chosen
  # provider — it's already wired for inference via OLLAMA_URL either way.
  # export_env_for_host() points the native gateway process at this same
  # host Ollama (127.0.0.1:${OLLAMA_PORT}); pulling into the bundled
  # ainxt-ollama container instead would leave a model missing from the one
  # the app actually calls. Only pull the bundled default model (~2GB) when
  # Ollama was actually chosen AND no host Ollama exists to use instead.
  if host_ollama_usable; then
    ok "host Ollama is already available; no bundled Ollama pull is required"
    seed_ollama_model_db ./.venv/bin/python
  elif [[ "$OLLAMA_CONFIGURED" == "1" ]]; then
    model="$(default_ollama_model)"
    step "Downloading the local model ($model, ~2GB)"
    docker exec ainxt-ollama ollama pull "$model" >/dev/null 2>&1 \
