...
def prod_env(monkeypatch, tmp_path):
    ...
    # validate_prod_config() also hard-requires a real (non-placeholder,
    # >=32 char) AUDIT_SIGNING_KEY and a FERNET_KEY — set fixed test-only
    # values here so every test using this fixture reflects the CURRENT
    # hard-requirement set, not the pre-AUDIT_SIGNING_KEY-era config.
    monkeypatch.setenv(
        "AUDIT_SIGNING_KEY",
        "test-only-audit-signing-key-not-a-real-secret-0123456789",
    )
    monkeypatch.setenv("FERNET_KEY", "VdSBK4aT-GBKR8tC-tPl99J1Sx4RG4FYbVqNkKfG8Hs=")
