...
def test_request_and_approve_budget_increase_end_to_end(hod_fixture, db_session, monkeypatch):
    ...
    This test's fixture models a real department/HOD org chart
    (department_hod_mapping rows), which is only consulted for routing when
    HOD_APPROVAL_ENABLED=true — core/config.py defaults it to false (flat
    "user raises, admin approves" mode for fresh/OSS installs, see the
    kill-switch comment there). resolve_budget_approvers_for_request()
    re-imports the flag from core.config on every call, so patching the
    module attribute here is enough to opt this test into HOD-hierarchy mode
    without needing an importlib.reload dance.
    """
    monkeypatch.setattr("core.config.HOD_APPROVAL_ENABLED", True)

    from store.budget_store import (
        request_budget_increase, resolve_budget_approvers_for_request,
    ...
    # Budget-increase requests use the DEDICATED resolver, not the one shared
    # by Products/KB/Codebase/Cowork — see resolve_budget_approvers_for_request()'s
    # docstring in store/budget_store.py and its real caller,
    # routers/budget_router.py's request_increase().
    approvers = resolve_budget_approvers_for_request(requester_email)
