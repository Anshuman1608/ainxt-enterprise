...
class _FakeUser:
    # Class-level defaults: _upsert_google_user's `User.sso_provider == "google"`
    # filter expression accesses these as CLASS attributes (evaluated before
    # being passed into _FakeQuery.filter(), which ignores its args anyway) —
    # without these, that access raises AttributeError on the class itself.
    # Kept alongside the instance-level assignments below, which give each
    # fake instance its own real per-object value.
    sso_provider = None
    sso_subject = None
    email = None
