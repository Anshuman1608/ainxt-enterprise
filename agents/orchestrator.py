...
Return JSON array only:"""
    ...
    def run(
        ...
        try:
            ...
            #
            # _retrieval_infra_warning (from core.retrieval_status) is set by
            # models/hybrid_search.py / models/hybrid_retriever.py when the
            # embed service or reranker was actually unreachable/misconfigured
            # rather than "genuinely nothing relevant exists" — when present,
            # it replaces the generic message below with the specific reason
            # and what to do about it, instead of leaving the user to guess.
            # ====================================================
            from core.retrieval_status import get_and_clear_retrieval_warning
            _retrieval_infra_warning = get_and_clear_retrieval_warning()

            _no_context_warning = ""
            if not state.context and state.repo_filter:
                if _retrieval_infra_warning:
                    _no_context_warning = (
                        f"> ⚠️ **Retrieval unavailable** for repo `{state.repo_filter}`: "
                        f"{_retrieval_infra_warning} This answer is based on AI training "
                        f"data only — NOT your actual codebase. Verify against the real "
                        f"code before acting on it.\n\n"
                    )
                else:
                    _no_context_warning = (
                ...
                logger.warning(
                    ...
                    f"(infra_warning={bool(_retrieval_infra_warning)}). "
                    f"Answer will be from model priors — flagging to user."
                )
            elif _retrieval_infra_warning:
                # Context WAS retrieved (so the answer is still grounded), but
                # something degraded along the way — e.g. the reranker fell
                # back to plain RRF fusion. Worth a lighter, separate note
                # rather than silence, regardless of whether this is a
                # codebase-scoped question or a general KB one.
                _no_context_warning = f"> ⚠️ {_retrieval_infra_warning}\n\n"
                logger.warning(f"RETRIEVAL DEGRADED (context still present) — {_retrieval_infra_warning}")
