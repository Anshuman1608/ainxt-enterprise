...
class CodingStateMachine:
    ...
    def _ensure_run_workspace(self, repo_slug: str, resume_in_place: bool = False) -> str:
        ...
        if _row and _row.git_url:
            ...
            # Build the clone URL from gitlab_repo (the real namespace/project,
            # e.g. "npci-oss/NPCI-AiNxt"), NOT self.repo which here can be the
            # normalized index slug (e.g. "npci_ainxt") → gitlab.com/npci_ainxt.git
            # (404). gitlab_repo defaults to repo when unset, so this is safe.
            _clone_url = _auth_url(self.gitlab_repo or self.repo, _gl_url, _gl_token)
            logger.info(
