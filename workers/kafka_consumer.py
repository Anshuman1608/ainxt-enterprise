...
def _handle_chat_history(records: list) -> None:
    ...
    db = SessionLocal()
    # SessionLocal is created with autoflush=False, so a Chat added earlier in
    # this same batch is NOT visible to the per-record query below. Two turns of
    # one conversation arriving in a single batch therefore each added a Chat
    # with the same primary key, and the resulting UniqueViolation on
    # chats_pkey rolled the whole batch back — every ChatMessage in it included.
    # That is silent: the handler logs and returns, the offset still advances.
    # Track what this batch has already created so the second turn updates the
    # pending row instead of inserting a duplicate.
    _batch_chats: dict = {}
    try:
        for rec in records:
            ...
            if "question" in rec and "answer" in rec:
                ...
                chat = _batch_chats.get(chat_id) or db.query(Chat).filter(Chat.id == chat_id).first()
                if not chat:
