...
from core.config import (
    ...
    EMBED_PROVIDER as _EMBED_PROVIDER,
...
def _next_embed_url() -> str:
    if not _EMBED_SVC_POOL:
        # itertools.cycle(range(0)) raises StopIteration on the first next()
        # call, which — inside a generator (pool.map's internal chunking) —
        # PEP 479 turns into an opaque "RuntimeError: generator raised
        # StopIteration" with no indication of the actual cause. Fail fast
        # with an actionable message instead.
        raise RuntimeError(
            "No embed service configured — set EMBED_SVC_URL (or EMBED_SVC_URLS) "
            "and start an embed service before indexing a repository."
        )
...
def index_repo_job(payload: dict) -> dict:
    ...
    try:
        ...
        # 0 indexed chunks is a legitimate outcome (e.g. a repo containing
        # only a README, with no file extension the chunker recognizes as
        # code/docs) -- not a crash, so this path never raised and status is
        # genuinely 'done'. But routers/index_router.py's list_repos()
        # derives display_status='failed' from vec_count==0 regardless
        # (there is nothing to retrieve from), and the UI's fallback text
        # for that is the generic "Check server logs." -- which sent
        # operators looking for a stack trace that doesn't exist. Recording
        # the real reason here means it surfaces as this repo's actual
        # error_msg instead.
        _zero_chunk_note = (
            "0 chunks indexed — no supported source files were found in this "
            "repo/branch (only recognized code/doc file extensions are "
            "chunked). This can be expected for a repo containing only a "
            "README or unsupported file types, not necessarily an indexing "
            "error — check the repo/branch if source code was expected."
        ) if result.get("indexed", 0) == 0 else None
        _update_index_request(payload.get("request_id"), "done", error_msg=_zero_chunk_note)
...
def _call_embed_svc(texts: list[str]) -> list[list[float]]:
    """
    Call embed svc with round-robin across the _EMBED_SVC_POOL, a per-provider
    circuit breaker (core/circuit_breaker — "embed_svc" is already tuned there:
    failure_threshold=8, recovery_timeout=15s), and bounded exponential backoff
    retry. Only retries on 5xx / network errors — 4xx (bad request) raises
    immediately since retrying won't fix a malformed payload, and (matching
    connectors/engine.py._execute_with_retry's documented rationale) a 4xx
    reflects the request, not upstream health, so it must not trip the breaker.

    No direct-Ollama fallback: embed svc is the only path in production.

    Previously this retried forever with backoff up to 300s between attempts
    ("blocks... rather than failing the wave") whenever embed svc was down or
    misconfigured — indistinguishable from a truly stuck worker: no error, no
    end, just index_requests sitting at status='running' until RQ's own
    job_execution_timeout (2100s / 35min) eventually killed it. Bounded here to
    match the rest of the codebase's convention for calling flaky external
    dependencies (workers/doc_worker.py's _CODE_MAX_ATTEMPTS + time budget;
    connectors/engine.py's max_attempts=3) — a persistently failing/absent
    embed svc now fails the job in well under a minute with a clear message,
    instead of retrying for up to 35 minutes before anyone finds out.
    """
    from core.circuit_breaker import get_breaker

    breaker = get_breaker("embed_svc")
    if breaker.is_open:
        raise RuntimeError(
            "embed_svc circuit breaker is OPEN — too many recent failures; "
            "fast-failing instead of retrying against a likely-down/"
            "misconfigured embed service. It will automatically retry after "
            "the recovery window."
        )

    max_attempts = 5   # 10s,20s,40s,80s,160s backoff — bounded, not indefinite
    ...
    for attempt in range(max_attempts):
        ...
        try:
            resp = httpx.post(
                f"{url}/embed",
                json={"texts": texts, "provider": _EMBED_PROVIDER},
                timeout=120.0,
            )
            ...
            if 400 <= exc.response.status_code < 500:
                ...
                # Re-raised as a plain RuntimeError (not the bare httpx exception)
                # so the clear reason embed_svc actually returned survives into
                # index_repo_job's `except Exception as exc: ... error_msg=str(exc)`
                # — str(httpx.HTTPStatusError) never includes the response body,
                # only the generic "Client error '4xx ...' for url ...".
                raise RuntimeError(describe_embed_svc_error(exc)) from exc  # not a breaker signal — see docstring
            # 5xx — embed svc itself failed (Ollama error propagated up)
            logger.warning(
                f"index_worker [←embed_svc 5xx]: {url} → HTTP {exc.response.status_code} "
                f"body={body!r} | attempt={attempt + 1}/{max_attempts}"
            )
            ...
            logger.warning(
                ...
                f"| texts={len(texts)} attempt={attempt + 1}/{max_attempts}"
            )
            breaker.record_failure(exc)
            last_exc = exc
        except Exception as exc:
        ...
        logger.warning(
            f"index_worker [embed_svc retry]: attempt {attempt + 1}/{max_attempts} "
            f"— retry in {wait}s | last_error={last_exc}"
        )
        time.sleep(wait)

    # describe_embed_svc_error() pulls the actual FastAPI 'detail' message
    # embed_svc wrote (e.g. "Nomic embedder not ready — set NOMIC_EMBED_URL
    # in .env and restart the embed service") out of the response body —
    # str(last_exc) alone never includes it, so without this the operator
    # only ever saw "giving up after 5 attempts — last_error=Server error
    # '503 Service Unavailable' for url ...", with no indication of what to
    # actually fix. This message is what ends up in index_requests.error_msg,
    # visible in the Codebase tab.
    raise RuntimeError(
        f"embed_svc: giving up after {max_attempts} attempts — "
        f"{describe_embed_svc_error(last_exc)}"
    ) from last_exc
...
_STALE_LOCK_MIN_AGE_MINUTES = 10   # cheap first-pass filter; matches the 10-min cron tick


def recover_stale_index_locks() -> dict:
    """Find index_requests rows stuck in 'running' whose RQ job no longer
    exists at all, and reset them to 'failed' (+ release the per-repo Redis
    lock) so the repo can be re-submitted instead of silently skipping every
    future attempt.

    Called every 10 minutes by the cron scheduler in start_workers.py — same
    slot as workers/kb_cleanup_worker.py's recover_stale_indexing_docs() and
    workers/codewiki_worker.py's recover_orphaned_codewiki_jobs(), and this
    function mirrors the latter's technique exactly (cross-check against
    RQ's own job registry rather than a fixed time threshold): indexing
    legitimately runs anywhere from seconds to 10+ hours (see LOCK_TTL's
    comment above), so elapsed time alone can never distinguish "crashed"
    from "still going" — only RQ genuinely not having the job anymore can.

    The normal path already releases this lock cleanly (index_repo_job's
    `finally: ... _R.delete(lock_key)`) on success, on any caught exception,
    and on RQ's own job-timeout. This function only ever fires for the one
    case no in-process cleanup can reach: the worker process itself being
    killed outright (container recreate, OOM, crash) mid-run.
    ...
    try:
        ...
        cutoff = datetime.now(timezone.utc) - timedelta(minutes=_STALE_LOCK_MIN_AGE_MINUTES)

        db = SessionLocal()
        try:
            candidates = db.execute(_sql("""
                SELECT id, repo_name, job_id, updated_at
                  FROM index_requests
        ...
        if not candidates:
            logger.debug("index_worker[stale_lock_recovery]: no running jobs old enough to check")
            return {"recovered": 0, "checked": 0}

        recovered = 0
        for request_id, repo_name, job_id, updated_at in candidates:
            status_info = get_job_status(str(job_id))
            # Fail closed: "unknown"/anything-but-the-exact-"Job not found"
            # case (RQ/Redis unreachable, or a genuine ambiguity) means do
            # nothing this cycle rather than risk mass-failing in-flight
            # jobs during a transient Redis blip — same reasoning as
            # codewiki_worker.py's recover_orphaned_codewiki_jobs().
            if status_info.get("status") != "unknown":
                continue
            if status_info.get("error") != "Job not found":
                logger.warning(
                    f"index_worker[stale_lock_recovery]: could not determine RQ "
                    f"status for job {job_id} (request={request_id}, repo={repo_name}) "
                    f"— skipping this cycle: {status_info.get('error')}"
                )
                continue
            ...
            error_msg = (
                ...
                f"{updated_at.isoformat()}). Please re-submit."
            )
            db = SessionLocal()
            try:
                db.execute(_sql("""
                    UPDATE index_requests
                    SET status = 'failed', error_msg = :err, updated_at = NOW()
                    WHERE id = :id
                """), {"id": request_id, "err": error_msg})
                db.commit()
            finally:
                db.close()

            # Release the lock only if it's still this exact request's —
            # a different, newer job may have legitimately acquired it since.
            lock_key = f"index:lock:{repo_name}"
            try:
                held = _R.get(lock_key)
                if held:
                    import json as _json
                    held_info = _json.loads(held)
                    if held_info.get("request_id") == str(request_id):
                        _R.delete(lock_key)
            except Exception as _lock_exc:
                logger.warning(
                    f"index_worker[stale_lock_recovery]: could not release lock "
                    f"for '{repo_name}': {_lock_exc}"
                )

            logger.warning(
                f"index_worker[stale_lock_recovery]: recovered orphaned index "
                f"request request_id={request_id} repo={repo_name} job_id={job_id} "
        ...
        if recovered:
            logger.info(
                f"index_worker[stale_lock_recovery]: recovered {recovered} orphaned "
                f"index request(s) (running -> failed) out of {len(candidates)} checked"
            )
        return {"recovered": recovered, "checked": len(candidates)}

    except Exception as exc:
        logger.error(f"index_worker[stale_lock_recovery]: failed: {exc}")
        return {"recovered": 0, "checked": 0}
