...
def _load_skill_context(fmt: str) -> str:
    ...
    Files loaded per format (docx/pptx/xlsx alike):
      SKILL.md + brand file (DOC_BRAND_FILE) + SKELETON.<js|py>
    The per-format API references (docx/DOCXJS.md, pptx/PPTXGENJS.md) document the
    RAW libraries, which the wrapper path forbids, so they are deliberately NOT
    injected — they are maintainer reference for the wrappers themselves.
    ...
    # Nothing else is injected: DOCXJS.md / PPTXGENJS.md cover the raw libraries
    # the wrapper path forbids, so adding them would only invite hand-rolled code.
