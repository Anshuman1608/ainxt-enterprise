...
class JiraAdapter(AdapterBase):
    ...
    def execute(
        ...
        # Per-user Jira site URL captured at connect time (pat_connect stores it
        # in metadata.base_url as "<site>/rest/api/3"). jira_tools builds paths
        # that already start with /rest/api/3, so hand it the bare site root.
        # Empty → jira_tools falls back to the JIRA_URL env var (single-site).
        site_base = ((context.metadata or {}).get("base_url", "") or "").strip().rstrip("/")
        for _suf in ("/rest/api/3", "/rest/api/2", "/rest/api"):
            if site_base.endswith(_suf):
                site_base = site_base[: -len(_suf)].rstrip("/")
                break

        set_credentials(email, token, site_base)
        try:
