        ...
        try:
            ...
            if connector_name == "gitlab":
                ...
                if not _email_part:
                    ...
                    pat = raw
                    email = _email_part
                # Per-user Jira site URL from the profile vault ("jira_url" entry,
                # set in Profile → API Token Vault) wins; falls back to the
                # gateway-wide JIRA_URL env. normalize_jira_site() reduces
                # whatever was pasted (bare host, site root, API URL, or a full
                # browser URL) to scheme://host. Same helper the router uses.
                from tools.jira_tools import normalize_jira_site
                _jira_site = normalize_jira_site(
                    get_decrypted_token(user_id, "jira_url") or _os.getenv("JIRA_URL", "")
                )
                base_url = (_jira_site + "/rest/api/3") if _jira_site else ""
                metadata = {
                    "auth_type":  "pat",
                    "pat_header": "Authorization",
                    "pat_scheme": "Basic",
