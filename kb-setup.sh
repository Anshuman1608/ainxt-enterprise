...
# A port already bound by our OWN embed-svc container is not a conflict to
# route around — it's the service this script is about to (re)configure and
# `docker compose up` will just recreate it on the same port. Without this
# check, running kb-setup.sh AFTER embed-svc was already brought up by
# ./start-index-worker.sh (which starts it non-interactively on whatever
# port .env already had) saw that port as "in use", moved to the next free
# one, and every subsequent restart drifted to a new port depending on
# whatever order things were started in — the exact same reasoning
# install.sh's choose_ports() would need for GATEWAY_PORT on a re-run.
if docker ps --format '{{.Names}}' 2>/dev/null | grep -q '^ainxt-embed-svc$'; then
  ok "embed-svc is already running — reusing its port ($EMBED_SVC_PORT)"
elif port_in_use "$EMBED_SVC_PORT"; then
...
# ── Detect an existing configuration ─────────────────────────────────────────
# EMBED_PROVIDER/EMBED_SVC_URL are NOT a usable signal on their own — both
# ship uncommented in .env.example, so they read as "set" on a totally fresh
# install that has never touched the Knowledge Base at all. The only
# trustworthy sign that embed-svc has genuinely been brought up before
# (through kb-setup.sh's own questionnaire, OR non-interactively via
# ./start-index-worker.sh -> ./start-embed-service.sh, which now always
# persists EMBED_PROVIDER/USE_DOCLING_PARSER/EMBED_SVC_PORT once it starts
# the container at all) is that the ainxt-embed-svc container actually
# exists. Re-running the full questionnaire on top of a real prior setup
# used to blindly re-ask everything (and, via the port-collision bug fixed
# above, drift the port to a new value each time) regardless of what was
# already downloaded and working. Showing what's there and defaulting to
# "keep it" makes setup order-independent: whichever of kb-setup.sh /
# start-index-worker.sh runs first, the other one just confirms the same
# config and reuses the same port.
existing_provider="$(_env_get EMBED_PROVIDER)"
SKIP_QUESTIONNAIRE=0
if docker ps -a --format '{{.Names}}' 2>/dev/null | grep -q '^ainxt-embed-svc$'; then
  existing_rr="$(_env_get RERANKER_VARIANT)"; existing_rr="${existing_rr:-bge_large (code default)}"
  existing_docling="$(_env_get USE_DOCLING_PARSER)"; existing_docling="${existing_docling:-1}"
  case "$existing_provider" in
    ollama) em="$(_env_get OLLAMA_EMBED_MODEL)"; existing_embed_desc="${em:-nomic-embed-text} (Ollama, local)" ;;
    openai) em="$(_env_get OPENAI_EMBED_MODEL)"; existing_embed_desc="${em:-text-embedding-3-small} (OpenAI)" ;;
    nomic)  existing_embed_desc="custom endpoint ($(_env_get NOMIC_EMBED_URL))" ;;
    *)      existing_embed_desc="$existing_provider" ;;
  esac

  step "Knowledge Base — existing configuration found"
  if docker ps --format '{{.Names}}' 2>/dev/null | grep -q '^ainxt-embed-svc$'; then
    say "  ${DIM}embed-svc is currently running on port ${EMBED_SVC_PORT} with:${R}"
  else
    say "  ${DIM}embed-svc is not currently running, but was set up before — its models are still downloaded:${R}"
  fi
  say "    embedding model   ${B}${existing_embed_desc}${R}"
  say "    reranker model    ${B}${existing_rr}${R}"
  say "    Docling parsing   ${B}$([[ "$existing_docling" == "1" ]] && echo "on" || echo "off")${R}"
  say "    port              ${B}${EMBED_SVC_PORT}${R}"
  say ""
  keep_yn=""
  printf '  Keep this configuration? [Y/n]: ' > "$TTY"
  read -r keep_yn < "$TTY" || keep_yn="y"
  case "$keep_yn" in
    n|N|no|NO) say "  ${DIM}Reconfiguring — answer the questions below (Enter keeps the current value).${R}" ;;
    *)         ok "keeping the existing configuration — no re-download, same port"; SKIP_QUESTIONNAIRE=1 ;;
  esac
fi
...
# Defaults below fall back to the existing configuration (if any) rather
# than always to option 1/Y, so hitting Enter during a reconfigure preserves
# whatever isn't being actively changed instead of silently resetting it.
kb_embed_provider="ollama"
case "$existing_provider" in
  openai) _emb_default=2 ;;
  nomic)  _emb_default=3 ;;
  *)      _emb_default=1 ;;
esac
...
emb_choice=""
printf '  Enter 1-3 [%s]: ' "$_emb_default" > "$TTY"
read -r emb_choice < "$TTY" || emb_choice="$_emb_default"
emb_choice="${emb_choice:-$_emb_default}"
case "$emb_choice" in
  2)
...
case "$(_env_get RERANKER_VARIANT)" in
  bge_base)  _rr_default=2 ;;
  tinybert)  _rr_default=3 ;;
  jina_tiny|jina_v2|qwen_06b|gte_modernbert|bge_v2_m3) _rr_default=4 ;;
  *)         _rr_default=1 ;;
esac
rr_choice=""
printf '  Enter 1-4 [%s]: ' "$_rr_default" > "$TTY"
read -r rr_choice < "$TTY" || rr_choice="$_rr_default"
rr_choice="${rr_choice:-$_rr_default}"
...
_docling_existing="$(_env_get USE_DOCLING_PARSER)"
if [[ "$_docling_existing" == "0" ]]; then
  printf '  Use Docling for parsing? [y/N]: ' > "$TTY"
  read -r doc_yn < "$TTY" || doc_yn="n"
  doc_yn="${doc_yn:-n}"
else
  printf '  Use Docling for parsing? [Y/n]: ' > "$TTY"
...
fi   # SKIP_QUESTIONNAIRE

# Unconditional even on the "keep existing configuration" path: these three
# just point the app AT embed-svc and turn parsing on — not a model choice —
# and need to be set even when embed-svc's container was created earlier by
# ./start-index-worker.sh, which never sets them (codebase indexing alone
# doesn't need the KB's document-parsing wiring). Idempotent set_env, so
# re-asserting them here on the normal questionnaire path too is harmless.
...
until curl -fsS -m 5 "http://localhost:${GATEWAY_PORT:-8000}/ainxt/v1/api/health" 2>/dev/null \
  ...
  # A silent 120s wait here (this loop's own limit) reads as a hang, not
  # progress — say so periodically rather than leaving the terminal looking
  # stuck right before the next prompts (SDLC / codebase indexing) that
  # install.sh asks after this script returns.
  (( waited % 30 == 0 )) && say "  ${DIM}still waiting… ${waited}s${R}"
done
