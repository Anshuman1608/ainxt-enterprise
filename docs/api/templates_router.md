...
Both routers rely on [auth.dependencies](../auth/auth.md) for authentication and authorization. Admin endpoints additionally require the `admin` role.
...
|------------|---------------------|----------------------|
| `auth.dependencies.get_current_user` | Authenticates all public endpoints via JWT, API key, or cookie. | [auth.md](../auth/auth.md) |
| `auth.dependencies.require_admin` | Enforces admin role on `/admin/templates/*` endpoints. | [auth.md](../auth/auth.md) |
| `core.config.EMBED_SVC_URL` | URL of the embedding microservice used by `_embed_chunks`. | [core_config.md](../core/core_config.md) |
| `core.logger.logger` | Structured logging for operation initialization and indexing events. | core_logger.md |
| `db.database.VectorSessionLocal` | SQLAlchemy session factory for the pgvector database. | database.md |
| `db.models.DocumentEmbedding` | ORM model for vector chunks written by `_write_pgvector`. | db_models.md |
...
- **The [sdlc_router](../sdlc/sdlc_router.md)** and [governance_router](../sdlc/governance_router.md)** consume operation metadata and approval states for lifecycle governance.
...
The embedding service URL is read from `EMBED_SVC_URL` in [core/config.py](../core/core_config.md) (default: `http://localhost:8001`).
...
- [auth.md](../auth/auth.md) — authentication and authorization dependencies.
- [embedding_service.md](../knowledge/embedding_service.md) — microservice used for vector generation.
- database.md and db_models.md — pgvector session and `DocumentEmbedding` schema.
- [sdlc_router.md](../sdlc/sdlc_router.md) — consumes operation metadata for SDLC governance.
- [governance_router.md](../sdlc/governance_router.md) — entity approval and governance workflows.
