...
The module is primarily consumed through the companion [TaskTrackerMCPServer](../mcp/mcp_servers.md) and is registered into the platform's MCP registry. It supports use cases including action-item extraction from meeting notes (UC-57), onboarding checklists (UC-65), and inbox triage delegation (UC-86).
...
- **`TaskTrackerMCPServer`**: Wraps the three tool functions as MCP tools with JSON schemas, descriptions, and audit flags. See [mcp_servers](../mcp/mcp_servers.md) for details on MCP server construction.
- **MCP Registry**: Discovers and registers available MCP servers so that agents and workflows can invoke `task_tracker` tools by name. See [mcp_system](../mcp/mcp_system.md).
...
- **[mcp_servers](../mcp/mcp_servers.md)** — `TaskTrackerMCPServer` imports and exposes `create_task`, `list_tasks`, and `update_task` as MCP tools.
- **[mcp_system](../mcp/mcp_system.md)** — The platform's MCP registry and bridge infrastructure make the task tracker tools discoverable to agents and workflows.
- **[shared_integrations](../skills/shared_integrations.md)** — Sibling tool modules (e.g., [jira_tools](../skills/shared_integrations.md), [calendar_tools](../skills/shared_integrations.md)) follow the same pattern and may be composed with task tracker tools in workflows.
...
`task_tracker_tools` is one of many tool modules in the [shared_integrations](../skills/shared_integrations.md) layer. It is designed to be:

- **Composable**: Agents can chain `create_task` with [email_tools](../skills/shared_integrations.md) to send notifications, or with [calendar_tools](../skills/shared_integrations.md) to schedule follow-ups.
...
- **Governable**: `create_task` and `update_task` are flagged for PCI audit in the MCP server, aligning with platform governance requirements. See [mcp_servers](../mcp/mcp_servers.md) and [core_governance](../core/shared_core.md) for audit handling.

In the abstudio_backend, task tracker tools may be surfaced through the catalog API and referenced by agents or workflows built in the studio. Refer to api_catalog and agent_factory_pipeline for how tools are discovered and bound to agents.
...
- [mcp_servers](../mcp/mcp_servers.md) — Companion MCP server that exposes these tools.
- [mcp_system](../mcp/mcp_system.md) — MCP registry, bridge, and client infrastructure.
- [shared_integrations](../skills/shared_integrations.md) — Sibling tool modules and integration patterns.
- abstudio_backend — Backend APIs for catalog, agents, and workflows.
