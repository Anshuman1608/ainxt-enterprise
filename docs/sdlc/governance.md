...
| `PENDING_APPROVAL` | Submitted for manager/HOD approval (or admin-only in flat mode — see [`governance_router`](governance_router.md)'s `HOD_APPROVAL_ENABLED` note) | No |
...
|------------|-------------|
| [auth](../auth/auth.md) | RBAC permission checks (`is_admin`, `can_manage_suppression`, `require_admin`, `get_current_user`) |
...
| [core_infrastructure](../core/core_infrastructure.md) | Redis KV, Postgres, logger, audit signer |
| database | `GovernanceEvent`, `GovernanceSuppression` models |
...
| [core_workflow_repo](../workflows/core_workflow_repo.md) | Artifact resolution (agents, skills, workflows) for approval previews |
