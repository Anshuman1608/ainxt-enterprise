...
The workers do **not** contain business logic themselves — they are thin orchestration wrappers that delegate to the [shared_core_sdlc_pipeline](../sdlc/shared_core_sdlc_pipeline.md) agents and the store_layer persistence layer.
...
| `store.sdlc_store` | store_layer | `get_run`, `update_run_state`, `add_run_event`, `list_runs`, `SDLCCancelled` |
| `store.sdlc_artifacts` | store_layer | `_store_artifact`, `_load_latest_artifact`, `compute_input_hash` for `GOVERNANCE_REPORT` persistence |
...
| `db.database` | database | `SessionLocal` for governance product-ID resolution |
...
- store_layer — Run state persistence, stage artifacts, governance findings/approvers
...
- database — SQLAlchemy session management and SDLC run models
