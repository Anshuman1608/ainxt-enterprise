...
For details on the approval workflow API, see [api_governance.md](../api/api_governance.md). For the artifact-usability check, see [core_governance_client.md](core_governance_client.md).
...
- **[api_execution](../api/api_execution.md)** — `run_workflow_stream`, `run_workflow`, and `resume_workflow_stream_endpoint` create a `RunUsageTracker` and observe engine events.
- **[api_agents](../api/api_agents.md)** / **[api_agent_chat](../api/api_agent_chat.md)** — agent chat routes use the tracker and tool policy checks.
- **[agent_factory_pipeline](../agents/agent_factory_pipeline.md)** — `AgentRunner` invokes `check_tool_access` before dispatching tools.
- **[core_llm_handler](../llm/core_llm_handler.md)** — model names observed in usage payloads flow into `estimate_model_cost`.
- **[core_workflow_repo](../workflows/core_workflow_repo.md)** — workflow definitions supply `node_data` such as `allowed_tools` and `blocked_tools`.

The module itself delegates to platform services rather than reimplementing them:

- **[core.request_audit](../core/core_infrastructure.md)** for audit persistence.
- **store.budget_store** for budget checks and usage increments.
- **[core.model_registry](../core/core_infrastructure.md)** for per-model pricing.
- **[gateway_local_llm](../llm/local_llm_gateway.md)** for local model discovery.
...
- [api_execution.md](../api/api_execution.md) — workflow execution endpoints that consume this module.
- [api_governance.md](../api/api_governance.md) — HTTP API for artifact approval.
- [core_governance_client.md](core_governance_client.md) — artifact usability checks.
- [core_config.md](../core/core_config.md) — environment configuration patterns.
- [core_llm_handler.md](../llm/core_llm_handler.md) — model usage and fallback handling.
- [core_workflow_repo.md](../workflows/core_workflow_repo.md) — workflow and node persistence.
- [agent_factory_pipeline.md](../agents/agent_factory_pipeline.md) — agent runtime that uses tool policy.
