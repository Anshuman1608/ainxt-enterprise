...
See the store_layer module documentation for details on the SDLC store layer.
...
- **store_layer** — The SDLC store layer including `list_run_repos`, `upsert_run_repo`, and run context management.
- **database** — The `SDLCRunRepo` ORM model and `sdlc_run_repos` table schema.
...
- **[shared_integrations](../skills/shared_integrations.md)** — The broader tools collection this module belongs to (alongside `gitlab_tools`, `jira_tools`, `kb_search_tools`, etc.).
