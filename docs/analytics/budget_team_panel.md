...
> **HOD-approval-hierarchy mode**: this panel is unaffected by `HOD_APPROVAL_ENABLED`. It's rendered for reporting managers in both flat/admin-only mode (the default) and HOD-hierarchy mode, and scopes to the caller's reporting subtree regardless of the flag — HOD-department scoping only applies when the caller is also an HOD, which never happens in flat mode (Tier-1 suppression of `is_hod`). See [`budget_manager`](../llm/budget_manager.md) §1.1 for the flag's effect on the HOD-only tabs that live alongside this panel.
...
Clicking a member's **Details** button expands an inline panel that reuses [`BudgetManager`](../llm/budget_manager.md)`::UserBudgetDetail`, giving the manager the same stat cards, increase-request audit, and utilisation breakdown that the member sees on their own **My Budget** page.
...
Used indirectly via [`BudgetManager`](../llm/budget_manager.md)`::UserBudgetDetail` when expanding a member row.
...
|--------|-----------------|---------|
| [`config`](../ui/ai_ui_frontend_app_core.md) | `authFetch`, `API_BASE` | Authenticated HTTP requests |
| [`BudgetManager`](../llm/budget_manager.md) | `UserBudgetDetail` | Per-member budget drill-down |
...
|--------|----------|---------|
| [`budget_router`](../core/shared_api_routers.md) | `GET /budget/team` | Team roster + aggregates |
| [`budget_router`](../core/shared_api_routers.md) | `GET /budget/team/utilization` | Team utilisation breakdown |
| [`budget_router`](../core/shared_api_routers.md) | `GET /budget/users/{id}/usage` | Per-member usage details |
...
1. **Read-only by design.** The panel does not allow managers to edit allocations or approve increases. Those actions live in [`BudgetManager`](../llm/budget_manager.md) (admin/HOD flows) and the per-member **My Budget** page.
...
- [`budget_manager.md`](../llm/budget_manager.md) — Admin/HOD budget management and `UserBudgetDetail`.
- [`budget_utilization_view.md`](budget_utilization_view.md) — `UtilizationPage`, donut charts, and breakdown legends.
- [`shared_api_routers.md`](../core/shared_api_routers.md) — Backend `budget_router` endpoints (`/budget/team`, `/budget/team/utilization`).
- [`ai_ui_frontend_app_core.md`](../ui/ai_ui_frontend_app_core.md) — Application shell, auth, and `authFetch` configuration.
