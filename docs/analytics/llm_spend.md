...
- Fetch daily cost and token-usage data from each provider's admin/billing APIs (routed through [`llm_proxy`](../llm/llm_proxy.md)).
...
All times are env-overridable (see `llm_spend_orchestration`).
...
- [`llm_proxy`](../llm/llm_proxy.md) — Proxies authenticated billing API calls to OpenAI, Anthropic, and GCP.
- [`shared_core_database`](../core/shared_core.md) — Defines `llm_spend_daily`, `llm_spend_fetch_runs`, and related models in `db/models.py`.
- [`shared_api_routers_llm_spend_report_router`](../core/shared_api_routers.md) — Admin endpoints (`admin_fetch`, `admin_email_daily`, etc.) that call into the orchestrator.
- [`services_smtp_service`](../core/shared_core.md) — Sends the rendered HTML/text emails.
