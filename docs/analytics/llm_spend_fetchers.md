...
4. **Egress safety**
   - All external calls are routed through [`llm_proxy`](../llm/llm_proxy_main.md) on `web02`; `app02` carries no provider admin credentials.
...
- Exposes `window_is_settled(target_date)`, which the `orchestrator` uses in the overnight Gemini settle loop.
...
- **Triggered by**: `services.llm_spend.orchestrator` (`run_fetch_window`, `run_daily_fetch`, `run_evening_fetch`, `backfill_if_empty`).
- **Scheduled by**: `services.llm_spend.gateway_bootstrap` via APScheduler jobs registered in [`gateway.py`](../core/gateway.md) startup.
- **Consumed by**: `services.llm_spend.report_builder` and the [`llm_spend_report_router`](llm_spend_report_router.md) for admin dashboards and email digests.
- **Depends on**: [`llm_proxy`](../llm/llm_proxy_main.md) for egress, `core.model_registry` for registry defaults, `db.database` for persistence, and `ainxt.model_usages` for recently observed models.
