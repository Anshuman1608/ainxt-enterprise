...
- **`PostgresAgentChatStore`** — Production backend that stores rows in the `agent_chat_threads` table. It reuses the platform-wide `SHARED_POOL` from app.core.db_pool and creates its schema on startup.
...
| ---------- | ---------- | ------- |
| `ChatMessage`, `ThreadSummary`, `summarise_thread` | checkpoint_workflow_store | Shared data types and summary heuristics so the agent chat sidebar stays consistent with the workflow chat sidebar. |
| `core.logger` | [shared_core](../core/shared_core.md) | Structured logging for load / flush diagnostics. |
| `app.core.config.postgres_enabled` | [core_config](../core/core_config.md) | Determines whether PostgreSQL is available. |
| `app.core.db_pool.SHARED_POOL` | core_db_pool | Shared connection pool used by the PostgreSQL backend. |
...
- **No pending-interrupt or node-output support** — Unlike checkpoint_workflow_store, the agent chat store only persists messages. Agent-runner conversations do not currently require HITL snapshots, per-node outputs, loop lessons, or durable run-step replay.
...
- **Serializer parity** — `_serialise_message` mirrors the serialization logic in checkpoint_workflow_store so that `generated_files` and `usage` survive reloads and remain bit-identical when absent.
...
- Change the message schema → update `_serialise_message` / `_deserialise_message` and keep them in sync with checkpoint_workflow_store.
