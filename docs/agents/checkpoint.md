...
|------------|---------|----------------|---------------|
| Workflow Checkpoint Store | `store.py`, `postgres_store.py` | Abstract interface, file-backed implementation, and PostgreSQL implementation for workflow chat / run state. | checkpoint_workflow_store.md |
| Agent Chat Store | `agent_store.py` | Abstract interface and dual backends for per-user, per-agent chat history. | [checkpoint_agent_chat_store.md](../agents/checkpoint_agent_chat_store.md) |
