...
These helpers mirror the patterns used in [Chat.jsx](../chat/chat.md) and the backend [chat_worker](../workers/workers.md).
...
| [workers/doc_worker_agent.py](../workers/workers.md) | Document generation worker |
| [workers/chat_worker.py](../workers/workers.md) | Office-mode chat execution |
| [connectors/engine.py](../skills/shared_integrations.md) | Connector runtime |
...
- **Document retention**: Generated files have a TTL and are cleaned by [purge_worker](../workers/workers.md). The `DocDownloadButton` handles `expired` and `410 Gone` states gracefully.
