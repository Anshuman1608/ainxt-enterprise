...
> **Note:** Module references in brackets (e.g. `[core_infrastructure]`, `[database]`) point to sibling documentation files. See [core_infrastructure](../core/core_infrastructure.md), database, [agent_system](../agents/agent_system.md), store_layer, [mcp_system](../mcp/mcp_system.md), [document_knowledge_workers](../workers/document_knowledge_workers.md), and [shared_integrations](../skills/shared_integrations.md) for details on those subsystems.
...
5. **Database (`GeneratedDocument`)**: The persistent audit record. Each row stores `job_id`, `user_id`, `chat_id`, `format`, `title`, `filename`, `file_path`, `content_md` (permanent audit trail), `artifact_id` (version grouping), and `version`. See database.
