...
The consumer delegates all processing to `services.coach_ingestor.ingest()`. The pipeline is documented in detail in the [Coach System](../coach/coach_system.md) module; a summary is provided here for context:
...
For details on the rule predicates and evaluation logic, see [Coach System](../coach/coach_system.md).
...
|---|---|---|
| `core_infrastructure` | [Core Infrastructure](../core/core_infrastructure.md) | `core.config` (Kafka/Redis/Coach settings), `core.logger`, `core.kv.get_kv`, `core.ckms.load_at_boot` |
| `coach_system` | [Coach System](../coach/coach_system.md) | `services.coach_ingestor.ingest()`, `agents.coach_evaluator` rule engine, `agents.compliance_engine` redaction |
| `database` | Database | `db.models.CoachEvent`, `db.database.SessionLocal` |
| `kv_store` | KV Store | Redis DB allocation (`RDB_QUEUE` = DB 5) and `get_kv()` abstraction |
| `ckms` | [CKMS](../auth/ckms.md) | Decrypts protected env vars at boot before config import |
| `worker_orchestration` | [Worker Orchestration](../workers/worker_orchestration.md) | `start_workers.py` launches this consumer as a subprocess when `--coach` is passed and `ENABLE_COACH=true` / `COACH_DIRECT_INGEST=false` |

---

## Configuration

All configuration is environment-variable driven. See [Core Infrastructure](../core/core_infrastructure.md) for the central config module.
...
This consumer is launched by the [Worker Orchestration](../workers/worker_orchestration.md) module (`workers/start_workers.py`). When started with `--coach`:
