...
| `db/models.py` | `ScheduledWorkflow` ORM model for event-driven workflows | database |
...
- **database**: Provides the `ScheduledWorkflow` model queried by `trigger_workflow_by_event` and the `SDLCRun` model managed by `sdlc_store`.
