...
3. **Running the shim in the sandbox subprocess** via [ToolDispatcher](../agents/agent_factory_pipeline.md).
...
- Reads environment variables injected by [ToolDispatcher](../agents/agent_factory_pipeline.md):
...
| Sandbox isolation | Tool code runs under `python -I` with no `PYTHONPATH` and a 1 MB stdout cap. See [ToolDispatcher](../agents/agent_factory_pipeline.md) for details. |
...
[ToolDispatcher](../agents/agent_factory_pipeline.md) retries transient errors (timeouts, 5xx, connection issues) up to `TOOL_MAX_ATTEMPTS` with exponential backoff, but does not retry deterministic failures such as `401`, `403`, `404`, or `422`.

---

## Integration with the Tool Catalog

`M365_TOOLS` is intended to be seeded into the `tools_catalog` table. The seeding logic lives in [tools_canonical_seed](../skills/tools_canonical_seed.md):
...
- Once seeded, [NativeEngine](../agents/engine_native_engine.md) wraps each row as a `_CatalogTool` and dispatches it through [ToolDispatcher](../agents/agent_factory_pipeline.md).

---

## Dependencies

- [agent_factory_pipeline](../agents/agent_factory_pipeline.md) — `ToolDispatcher` runs the shim in a sandbox and injects `AINXT_USER_ID`.
- [tools_canonical_seed](../skills/tools_canonical_seed.md) — seeds `M365_TOOLS` into `tools_catalog`.
- [engine_native_engine](../agents/engine_native_engine.md) — `_CatalogTool` loads tool specs from the catalog and invokes the dispatcher.
- connectors_microsoft365_adapter — the actual Microsoft Graph adapter executed by the connector bridge.
- routers_connectors — exposes `/connectors/execute` and validates `X-Bridge-Token`.
