...
The module is part of the broader [shared_integrations](../skills/shared_integrations.md) family and is consumed by the [agent_system](../core/shared_core.md), [workflow_system](../core/shared_core.md), and various [routers](../core/shared_api_routers.md) that surface Confluence capabilities to the UI.
...
1. **User-scoped credentials** — Every call resolves the user's stored Atlassian token via [platform_credentials](../core/shared_core.md). Missing credentials raise `PermissionError` with a clear remediation message.
...
4. **Resilience** — Every request passes through a named `confluence` [circuit breaker](../core/shared_core.md) and an exponential-backoff retry wrapper.
...
|------------|--------|------|
| `logger` | [core.logger](../core/shared_core.md) | Structured logging for errors and successful operations. |
| `get_atlassian_creds` | [core.platform_credentials](../core/shared_core.md) | Resolve user-specific Atlassian email + API token. |
| `get_product_for_repo` | [core.platform_credentials](../core/shared_core.md) | Map a repo name to a product's Confluence space. |
| `get_breaker` | [core.circuit_breaker](../core/shared_core.md) | Obtain the named `confluence` circuit breaker. |
| `retry_llm` | [core.retry](../core/shared_core.md) | Exponential-backoff retry for transient failures. |
| `llm_proxy_headers` | [core.proxy_tool_use](../core/shared_core.md) | Build authenticated headers for the LLM proxy. |
| `sanitize` | [core.prompt_sanitizer](../core/shared_core.md) | Strip dangerous/control characters from titles and bodies. |
...
Calls [get_atlassian_creds](../core/shared_core.md) to retrieve the user's personal Atlassian credentials. Raises `PermissionError` if none are stored, prompting the user to add a token under **Profile → Atlassian Token**.
...
> Note: This is intentionally a simplified converter for agent-generated content. Complex Confluence layouts should use dedicated document-generation tools such as [doc_generator](../skills/shared_integrations.md).
...
`confluence_tools` is one of many integration tool modules in the [shared_integrations](../skills/shared_integrations.md) layer. It is registered alongside [jira_tools](../skills/shared_integrations.md), [github_tools](../skills/shared_integrations.md), [gitlab_tools](../skills/shared_integrations.md), and others so that agents and workflows can access enterprise knowledge stores.
...
The module relies on the platform's cross-cutting concerns — authentication, credential storage, circuit breaking, retry, logging, and proxy routing — which are documented in [shared_core](../core/shared_core.md).

---

## References

- [shared_integrations](../skills/shared_integrations.md) — Overview of all integration tool modules.
- [shared_core](../core/shared_core.md) — Core infrastructure: logging, credentials, circuit breakers, retry, proxy headers.
- [mcp_servers](../mcp/mcp_servers.md) — MCP server implementations, including ConfluenceMCPServer.
- [shared_api_routers](../core/shared_api_routers.md) — HTTP routers that may call Confluence tools.
