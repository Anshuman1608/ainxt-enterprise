...
## Enabling an OAuth2 Connector (e.g. Gmail)

The architecture sections above describe how the module works once a
connector is configured. This section is the practical checklist for
actually turning one on.

An OAuth2 connector (Gmail, Google Drive, Google Calendar, Microsoft 365,
Slack, GitHub-OAuth) needs **four** things before "Connect" works, in this
order — skipping any one produces a different, confusing failure:

1. **Provider-side app registration.** For Google connectors: a Google
   Cloud project, the specific API enabled (Gmail API for Gmail — Drive and
   Calendar need their own APIs enabled too, even under the same project),
   an OAuth consent screen (add your own account as a test user if the app
   isn't verified — otherwise Google blocks sign-in outright), and an
   OAuth 2.0 Client ID (type: Web application). This gives you a Client ID
   and Client Secret. Microsoft 365 is the equivalent Azure AD App
   Registration.

2. **The exact redirect URI registered with the provider.** Built as
   `{CONNECTOR_OAUTH_REDIRECT_BASE}/ainxt/v1/api/connectors/oauth/callback/{connector_name}`
   (see `routers/connectors_router.py`'s `_redirect_uri()`). It must match
   character for character — trailing slash, scheme, port, all of it — or
   the provider rejects the flow with `redirect_uri_mismatch`.
   `CONNECTOR_OAUTH_REDIRECT_BASE` must be wherever your frontend is
   actually served from (e.g. `ai-ui`'s port in a split gateway+SPA Docker
   deployment), not the API-only backend's own port — the post-auth landing
   step needs to reach an actual page, not just the API endpoint.

3. **Three env vars, actually reaching the process that serves the API** —
   `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`, `CONNECTOR_OAUTH_REDIRECT_BASE`
   (see the root `README.md`'s environment table and `.env.example`). If
   `gateway` runs in a container, these must be threaded through
   `docker-compose.yml`'s `environment:` block for that service — a bare
   `.env` entry does not automatically reach a container, since Compose's
   `environment:` is an explicit allow-list, not a passthrough.
   `CONNECTOR_OAUTH_REDIRECT_BASE` has no default — left unset, every
   OAuth2 connector's "Connect" click crashes outright.

4. **The connector enabled in the frontend allowlist.**
   `ai-ui/src/components/Connectors.jsx`'s `ENABLED_CONNECTORS` Set gates
   which connectors are ever rendered, independent of the backend's
   `is_active` flag or anything server-configured — a connector can be
   fully configured and still never appear in the UI if its name isn't in
   that Set. This is baked into the `ai-ui` static build (Vite), so a code
   change here needs a real image rebuild, not just a container restart.

**Symptoms mapped to which of the four is missing**, since each produces a
different-looking failure that doesn't obviously point at its cause:

| Symptom | Likely missing step |
|---|---|
| Connector never appears in the UI at all (even the card), regardless of config | Step 4 — check `ENABLED_CONNECTORS` |
| Clicking Connect immediately shows a 400/500 error, no provider screen ever opens | Step 3 — client ID/secret not reaching the process |
| Provider screen opens, but shows "This app isn't verified" and blocks entirely | Step 1 — your account isn't added as a test user |
| Provider screen opens, consent completes, but you land on `redirect_uri_mismatch` | Step 2 — registered URI doesn't match `_redirect_uri()`'s actual output |
| Consent completes, but you land on a raw `{"detail":"Not Found"}` (not a provider error) | Step 2 — `CONNECTOR_OAUTH_REDIRECT_BASE` points at the API-only backend instead of the frontend |
| Everything above passes, but the connector's tools return `401`/`insufficient_scope` | The connector's own `connectors/seed.py` entry needs a scope your existing token doesn't have — OAuth scopes are not retroactive; disconnect and reconnect |
...
| Module | Relationship |
|--------|-------------|
| **[shared_integrations.md](../skills/shared_integrations.md)** | Contains the connector infrastructure (`ConnectorEngine`, `ConnectorRegistry`, `OAuth2Handler`, `ConnectorMetrics`) and all adapter implementations (`microsoft365`, `gitlab`, `jira`, `slack`, `gmail`, etc.) |
...
| **[office.md](../documents/office.md)** | The Office component with `ConnectorActionCard` for write actions and `OfficeServer` for the SSE flow |
| **[profile.md](../auth/profile.md)** | The Profile component with the API Token Vault — source of PATs for GitLab/Jira connectors |
| **[config.md](../core/config.md)** | `authFetch` / `apiFetch` HTTP utilities used by all frontend connector calls |
| **[mcp_system.md](../mcp/mcp_system.md)** | The MCP registry where connector tools are registered as `{connector}__{tool}` for LLM tool-use |
| **[shared_core.md](../core/shared_core.md)** | `ComplianceEngine` used for PCI/PII scanning of connector responses; `CredentialVault` for token encryption |
| **database.md** | PostgreSQL schema for `connector_definitions`, `user_oauth_tokens`, `user_connector_permissions` |
