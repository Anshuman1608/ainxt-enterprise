...
The tools are intentionally narrow: they give agents quantitative signals and a controlled write path, while the nuanced fit narrative and final decisions remain the agent's responsibility. All three functions are wrapped by the [`ATSToolsMCPServer`](../mcp/mcp_servers.md) and registered into the platform's [MCP system](../mcp/mcp_system.md) so that agents, workflows, and chat interfaces can call them through the standard Model Context Protocol.
...
- **MCP server base** — [`BaseMCPServer`](../mcp/mcp_servers.md) provides JSON-RPC dispatch, compliance gating, and audit logging.
- **MCP registry** — the server is registered through the platform's [MCP registry](../mcp/mcp_system.md).

---

## How It Fits into the System

`ats_tools` is one of many integration toolsets under [`shared_integrations`](shared_integrations.md). It follows the same pattern as [`calendar_tools`](shared_integrations.md), [`jira_tools`](shared_integrations.md), and [`task_tracker_tools`](shared_integrations.md):
...
3. The server is registered in the central [MCP registry](../mcp/mcp_system.md), making the tools available to agents, workflows, and chat runtimes.
4. Agents in [`agent_factory_pipeline`](../agents/agent_factory_pipeline.md) or [`agent_system`](../core/shared_core.md) can compose these tools into larger recruiting workflows.

Because `propose_stage_update` writes to an outbox rather than the ATS, the module respects the platform's human-in-the-loop governance model. For more on how compliance and audit gates are applied to MCP tool calls, see [`mcp_servers`](../mcp/mcp_servers.md) and the platform guardrails in [`shared_core`](../core/shared_core.md).
...
- [`mcp_servers`](../mcp/mcp_servers.md) — MCP server implementations, including `ATSToolsMCPServer`.
- [`mcp_system`](../mcp/mcp_system.md) — MCP registry, bridge, and client infrastructure.
- [`agent_factory_pipeline`](../agents/agent_factory_pipeline.md) — agent assembly that may bind these tools.
- [`shared_core`](../core/shared_core.md) — compliance, audit, and guardrail primitives used by the MCP server base.
