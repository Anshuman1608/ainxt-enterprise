...
|---|---|---|
| **Connector Adapters** | Provider-specific SaaS API adapters (Microsoft 365, Google Drive, Gmail, GitLab, Jira, Confluence, Slack, Zoom, DocuSign, DPI). | [shared_integrations_connector_adapters.md](shared_integrations_connector_adapters.md) |
| **Connector Infrastructure** | `ConnectorEngine`, `ConnectorRegistry`, `OAuth2Handler`, `ConnectorMetrics`, consent handling, and MCP bridge for connector execution. | [shared_integrations_connector_infrastructure.md](shared_integrations_connector_infrastructure.md) |
...
| **Calendar Tools** | Read-only `.ics` calendar introspection, free-slot computation, and draft event creation. | [calendar_tools.md](../connectors/calendar_tools.md) |
...
| **Docker Execution Tool** | Sandboxed execution of arbitrary Python code inside ephemeral Docker containers. | [docker_execution_tool.md](../sandbox/docker_execution_tool.md) |
...
| **Email Tools** | Read-only `.eml` triage and draft-reply creation to an outbox. | [email_tools.md](../connectors/email_tools.md) |
...
| **n8n Tools** | Autonomous n8n workflow generation and invocation of existing webhook workflows. | [n8n_tools.md](../connectors/n8n_tools.md) |
| **Run Diff Tools** | Multi-repo git-diff inspection for in-progress SDLC runs. | [run_diff_tools.md](../sdlc/run_diff_tools.md) |
...
| **Task Tracker Tools** | Lightweight task CRUD with JSON-backed persistence and MCP exposure. | [task_tracker_tools.md](../api/task_tracker_tools.md) |
| **Translator Tools** | Glossary-aware translation toolkit with agent-driven and MT-engine modes. | translator_tools.md |
...
- [shared_core](../core/shared_core.md) — Core infrastructure (logging, circuit breakers, retry, credentials, compliance) consumed by adapters and tools.
- [shared_api_routers](../core/shared_api_routers.md) — HTTP routers that surface connector actions, document generation, and SDLC operations to clients.
- abstudio_backend — Backend APIs for catalog, agents, workflows, and templates that may bind integration tools.