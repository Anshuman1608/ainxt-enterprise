...
The backend `upload_chat_files` endpoint in [chat_router](../chat/chat_router.md) performs a multi-stage pipeline for each file:
...
|------------|--------|-------|
| `previewCache.js` | [ai_ui_frontend_utils](../ui/ai_ui_frontend_utils.md) | `cacheStore`, `cachedGet`, `cachePurgeExpired` for browser Cache API management |
| `config.js` | [config](../core/config.md) | `authFetch` for authenticated requests, `API_BASE` for endpoint URLs |
| `DialogProvider.jsx` | [ui_dialog](../ui/ui_dialog.md) | `useToast` for user notifications (errors, info, cancellations) |
| `DocumentPreviewModal.jsx` | [document_preview](../documents/document_preview.md) | Full-screen file preview with format-specific renderers |
| `messageContent.js` | [ai_ui_frontend_utils](../ui/ai_ui_frontend_utils.md) | `stripMemoryTag`, `stripSystemPrefix` for message content processing |
...
| Dependency | Module | Usage |
|------------|--------|-------|
| `chat_router.py` | [chat_router](../chat/chat_router.md) | `upload_chat_files`, `get_attachment`, `get_attachment_raw` endpoints |
| `core/storage.py` | [core_infrastructure](../core/core_infrastructure.md) | `ObjectStorage` for file persistence (local FS or MinIO) |
| `core/document_parser.py` | [document_processing](../documents/document_processing.md) | `parse_file_structured` for text extraction |
| `agents/compliance_engine.py` | [agent_system](../agents/agent_system.md) | `ComplianceEngine.validate_input` for PII/PCI detection |
| `db/models.py` | database | `ChatAttachment` ORM model for persistence |
...
- [chat_router](../chat/chat_router.md) — Backend API router for chat file operations
- [document_preview](../documents/document_preview.md) — `DocumentPreviewModal` and format-specific renderers
- [ai_ui_frontend_utils](../ui/ai_ui_frontend_utils.md) — Shared utilities including `previewCache.js`
- [config](../core/config.md) — `authFetch` and `API_BASE` configuration
