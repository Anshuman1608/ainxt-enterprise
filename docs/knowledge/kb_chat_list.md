...
| --- | --- |
| **Filter KB chats** | Uses `isKbChat` from [`kbChat.js`](kb_chat.md) to show only KB-scoped chats. |
...
| [`kbChat.js`](kb_chat.md) | Utility that classifies a chat object as a KB chat. |
| [`kbFormat.js`](knowledge_base.md) | Date/scope formatting helpers used in the delete confirmation. |
...
Both endpoints are called through `authFetch` from [`config.js`](../core/config.md), which attaches credentials and the auth token.
...
- [`kbChat.js`](kb_chat.md) — `isKbChat` classification utility.
- [`config.js`](../core/config.md) — `API_BASE` and `authFetch`.
