...
The module is consumed by the ComplianceEngine which orchestrates detection, redaction, and blocking decisions based on a configurable per-type policy (`redact` / `block` / `off`).
