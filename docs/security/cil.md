...
- **Downstream**: the gateway/router, [model_routing](../llm/model_routing.md), [context_engine](../llm/context_engine.md), tool dispatch, and document-generation pipelines.

CIL depends on:

- [model_routing](../llm/model_routing.md) — `models/model_router.py` supplies the model cascade used by `cil/intent.py`.
- kv_store — `core/kv` provides the cache backend for intent classification results.
- [core_infrastructure](../core/core_infrastructure.md) — `core/config.py` and `core/logger.py` for configuration and logging.
...
- [model_routing.md](../llm/model_routing.md) — model router and `models/doc_intent.py`.
- kv_store.md — caching backend.
- [core_infrastructure.md](../core/core_infrastructure.md) — logging and configuration.
