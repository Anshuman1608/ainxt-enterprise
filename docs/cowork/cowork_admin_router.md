...
|------------|--------|------|
| `get_current_user` | [authentication](../auth/authentication.md) | JWT/API-key extraction and user context enrichment |
| `require_admin` | [authentication](../auth/authentication.md) | Admin-role enforcement for marketplace endpoints |
| `can_approve` | [authentication](../auth/authentication.md) | Non-raising check for `ad_level ≤ 3` or admin |
| `CoworkMemory` | memory_system | Postgres-backed per-user preference and note store |
| `cowork_roles` service | [services](../workers/services.md) | Role specialist CRUD, context rendering, marketplace |
| `_compliance_check` | [mcp_system](../mcp/mcp_system.md) | PII/secret/PAN screening for memory notes |
| `publish_inbox_item` | store_layer | Inbox notifications for governance events |
| `SessionLocal` / `User` | database | Approver lookup for role submission notifications |
...
- [authentication](../auth/authentication.md) — JWT validation, RBAC, and SSO
- memory_system — `CoworkMemory` and the broader memory service
- [services](../workers/services.md) — `cowork_roles` service layer with role CRUD and context rendering
- store_layer — `inbox_store` for governance notifications
- [mcp_system](../mcp/mcp_system.md) — MCP server base with compliance checking
