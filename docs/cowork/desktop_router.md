...
- **KV Store**: MCP registrations are stored in the workflow KV database with an 8-hour TTL, ensuring stale registrations expire automatically. See `core/kv`.
...
- `core/kv` — Redis KV store used for MCP registration state.
