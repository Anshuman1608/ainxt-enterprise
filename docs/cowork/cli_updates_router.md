...
Both routers share the same local release directory (`CLI_RELEASES_DIR`) and the same authentication/authorization primitives from [`auth_router`](../auth/auth_router.md) and [`auth/dependencies`](../core/shared_core.md).
...
| `core.logger.logger` | Structured logging for update and fleet events. | [`shared_core.md#core_infrastructure`](../core/shared_core.md) |
| `db.database.SessionLocal` | SQLAlchemy session management. | [`shared_core.md#database`](../core/shared_core.md) |
| `db.models.CliVersionRecord` | Persistence model for CLI installation telemetry. | [`shared_core.md#database`](../core/shared_core.md) |
...
- Fleet analytics data is stored in the same PostgreSQL database used by the rest of the platform, modeled by `CliVersionRecord` in [`shared_core.md#database`](../core/shared_core.md).
...
- **Monitoring**: The router emits structured log lines prefixed with `[cli-updates]` and `[cli-fleet]`; these can be scraped by the platform's observability stack (see [`shared_core.md#observability`](../core/shared_core.md)).
