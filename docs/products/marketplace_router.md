...
- Validation and sanitization happen first via `validate_tool_register_request` in [core/security_validation](../core/shared_core.md).
...
- **[core.config](../core/shared_core.md)** — `RDB_REGISTRY` constant selecting the KV database index.
- **[core.kv](../core/shared_core.md)** — `get_kv` and `KVError` for marketplace metadata storage.
- **[core.logger](../core/shared_core.md)** — Structured logging.
- **[core.security_validation](../core/shared_core.md)** — `validate_tool_register_request` for input sanitization.
- **db.database / db.models** — `SessionLocal` and the `MCPServer` SQLAlchemy model.
...
- **AI-UI plugin catalog**: `GET /plugins/curated` feeds the CLI/web plugin discover view. The file is produced by [workers/external_sync_worker](../workers/workers.md).
