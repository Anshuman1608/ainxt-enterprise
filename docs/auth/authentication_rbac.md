...
| `core.logger` | Structured logging for denied access and errors | core_logger |
| `db.database.SessionLocal` | SQLAlchemy session factory for HOD user-scope queries | database |
| `db.models.User` | User ORM model (department, id) used for HOD scoping | database |
| `store.sdlc_governance_approvers.approver_domains_for` | Returns active governance domains a user may approve | store_layer |
...
### Platform-wide HOD-approval mode (`HOD_APPROVAL_ENABLED`)

`is_hod(current_user)` and `get_hod_departments(current_user)` never read `HOD_APPROVAL_ENABLED` themselves — they simply reflect whatever `is_hod`/`hod_departments` were baked into the session payload. The suppression happens exactly once, upstream, in `auth/dependencies.py::enrich_user_context()` (the "Tier 1" choke point of the platform's two-tier HOD-mode design): when `HOD_APPROVAL_ENABLED` is `False` (the default — flat/admin-only mode, for deployments with no department/HOD hierarchy), `enrich_user_context()` forces `is_hod=False` and `hod_departments=[]` for every user, in both the Redis-cache-hit and DB-lookup code paths, before this module ever sees the payload.

The practical effect on this module, with **zero code changes here**: `is_hod()` always returns `False`, `get_hod_departments()` always returns `[]`, and `get_visible_user_filter()` therefore always takes the `HOD -->|no| Empty` branch above for non-admins — every RBAC consumer of these three helpers automatically behaves as if no HOD hierarchy exists, platform-wide. Existing HOD-hierarchy deployments must set `HOD_APPROVAL_ENABLED=true` to keep the diagram above's HOD branch reachable.
...
|---|---|---|
| **Shared API Routers** | `_require_director`, `_require_admin_or_operator`, `_require_admin`, `require_broadcast_user` | [shared_api_routers](../core/shared_api_routers.md) |
| **ABStudio Backend API** | `_wrapped_gateway_auth`, `require_admin` in `app/api/deps.py` | [api_deps](../api/api_deps.md) |
| **Gateway** | Route-level authorization for agent, workflow, chat endpoints | [gateway](../core/gateway.md) |
| **SDLC Governance Router** | Governance domain approval, suppression management | [sdlc_router](../sdlc/sdlc_router.md) |
| **Budget Router** | Admin/operator gates for budget management | [budget_router](../llm/budget_router.md) |
