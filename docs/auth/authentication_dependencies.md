...
**Platform-wide HOD-approval mode (`HOD_APPROVAL_ENABLED`) — Tier 1 of the two-tier design**

`enrich_user_context` is the single choke point ("Tier 1") for suppressing the platform's department/HOD hierarchy when it isn't in use. `core/config.py::HOD_APPROVAL_ENABLED` defaults to `False` — flat/admin-only mode, for deployments with no department/HOD org chart. In **both** the Redis-cache-hit branch and the DB-lookup branch above, the `is_hod`/`hod_departments` values are AND-gated/blanked with the flag before being merged into the payload:

```python
is_hod_flag     = HOD_APPROVAL_ENABLED and ((hod_check is not None) or bool(hod_departments))
hod_departments = hod_departments if HOD_APPROVAL_ENABLED else []
```

When `HOD_APPROVAL_ENABLED` is `False`, every session gets `is_hod=False`, `hod_departments=[]` — regardless of what `department_hod_mapping`/`users.hod_email` actually contain. Since every downstream consumer of `is_hod(current_user)`/`get_hod_departments(current_user)` (see [authentication_rbac](authentication_rbac.md)'s `get_visible_user_filter`, the `BudgetManager.jsx` frontend's `IS_HOD` state, the KB-deletion-history "HOD sees org-wide PUBLIC" ACL) reads these fields rather than querying the mapping table directly, this single change flips all of them to their existing non-HOD behaviour with zero changes at the consumer. A handful of places that resolve a HOD directly for *routing* purposes (not via `is_hod()`) — e.g. `store/budget_store.py::resolve_approvers_for_request`, `routers/governance_router.py::_resolve_hod_user_ids` — have their own one-line branch on the same flag ("Tier 2"), documented in their respective modules. Existing HOD-hierarchy deployments must set `HOD_APPROVAL_ENABLED=true` to keep today's behaviour, byte-for-byte, unchanged.
...
| [core_infrastructure](../core/core_infrastructure.md) | Provides `redis_client` (config) and `logger` used by this module. |
| database | Provides `SessionLocal` and the `User` / `department_hod_mapping` models. |
