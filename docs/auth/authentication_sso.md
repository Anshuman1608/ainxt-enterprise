...
| `db/database`, `db/models` | `SessionLocal`, `User` model (sso_subject, sso_provider fields) | database |
| `store/credential_vault` | `encrypt_value()` — encrypts Entra OAuth tokens at rest | store_layer |
