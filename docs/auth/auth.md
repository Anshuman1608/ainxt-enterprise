...
| [`Login.jsx`](login.md) | Authenticates the user and calls `setSession()` on success. |
| [`Profile.jsx`](profile.md) | Reads `session` to display user details. |
| [`Chat.jsx`](../chat/chat.md), [`KbChat.jsx`](../knowledge/kb_chat.md) | May read `session` to scope requests or display user-specific data. |
| `config.js` | `authFetch` and `apiFetch` may attach tokens derived from the session object. |
...
| Module | Relationship |
| ------ | ------------ |
| [`auth_router`](../core/shared_api_routers.md) | Provides login, register, refresh, logout, SSO, and session management endpoints. |
| [`shared_core_authentication`](../core/shared_core.md) | Contains RBAC, LDAP, and SSO logic used by the backend auth layer. |
...
- [`login.md`](login.md) — Login screen that populates the auth session.
- [`profile.md`](profile.md) — User profile that reads the session.
- `ai_ui_frontend_config.md` — API fetch helpers that may use session tokens.
- [`shared_api_routers.md#auth_router`](../core/shared_api_routers.md) — Backend authentication endpoints.
- [`shared_core.md#authentication`](../core/shared_core.md) — Backend RBAC, LDAP, and SSO implementation.
