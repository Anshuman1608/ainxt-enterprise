...
- **API token vault** — store, list, update, and delete user-owned tokens (`local_llm`, `atlassian`, `gitlab`, `github`). Token values are encrypted at rest with Fernet and are only returned to the client in masked form, except for an explicit, audit-logged "reveal" endpoint.
...
| `validate_profile_update_request` | [core/security_validation](../core/shared_core.md) | Validates `name` and `gitlab_username` for injection / XSS / special characters. |
| `validate_token_upsert_request` | [core/security_validation](../core/shared_core.md) | Validates token value and label before persistence. |
| `logger` | [core/logger](../core/shared_core.md) | Used for audit logging of token reveals and upserts. |
| `db.database.engine` | db/database | SQLAlchemy engine for `users` and `user_tokens` tables. |
...
- **Token types**: `local_llm`, `atlassian`, `gitlab`, `github`. Unknown types are rejected with HTTP 400.

---

## 8. Internal helper: `get_decrypted_token`

`get_decrypted_token(user_id: str, token_type: str) -> Optional[str]` is exported for use by other backend modules (notably the model-routing layer for the in-house model tier). It performs the same database lookup and Fernet decryption as `reveal_token` but does not expose the value over HTTP and does not audit-log.
...
- **Chat / Memory**: custom instructions may be consumed by [chat_router](../chat/chat_router.md) and memory_router to shape persona-driven responses.
- **Model routing**: `get_decrypted_token` is used by the model-routing layer (see [model_router](../llm/model_routing.md)) when an in-house-model-tier credential is required.
- **Desktop / Git workflows**: `reveal_token` for `gitlab` enables the desktop client to perform `git clone` with the user's stored PAT.
- **Security**: input validation is delegated to [core/security_validation](../core/shared_core.md); audit logging uses [core/logger](../core/shared_core.md).
