...
|------------|--------|---------|
| `authFetch` | [config](../core/config.md) | Authenticated HTTP requests with correlation IDs and credentials. |
...
| Dependency | Module | Purpose |
|------------|--------|---------|
| `profile_router` | [shared_api_routers](../core/shared_api_routers.md) | CRUD endpoints for profile, tokens, and custom instructions. |
| `api_keys_router` | [shared_api_routers](../core/shared_api_routers.md) | Generation and revocation of IDE API keys. |
| `get_current_user` / `_require_jwt_auth` | [auth](auth.md) | JWT authentication and authorization. |
| `users`, `user_tokens`, `user_api_keys` | database | Persistent storage for profile data and credentials. |
...
- **Connectors / GitLab & Jira**: Stored GitLab and Atlassian tokens are used by connector adapters and tools to act on behalf of the user. See [connectors](../connectors/connectors.md) and [shared_integrations](../skills/shared_integrations.md).
- **Desktop App**: The desktop client can request a plaintext token reveal (audit-logged) to perform `git clone` with the user's stored GitLab PAT. See [desktop_app](../cowork/desktop_app.md).
- **IDE Plugins**: IDE API keys expose an OpenAI-compatible endpoint so external editors can route completions through AiNxt. See [endpoint_proxy_router](../core/shared_api_routers.md) and [gateway](../core/gateway.md).
...
- IDE API keys labeled `endpoint:*` are managed by the Endpoint Management module and cannot be revoked from the Profile page. See [endpoint_manager](../products/endpoint_manager.md).
