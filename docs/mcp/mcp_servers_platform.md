...
The module is intentionally thin: it translates MCP `tools/call` requests into authenticated HTTP calls against the platform's REST API (the [gateway](../core/gateway.md)), then returns the results as MCP-compliant text content. It reuses the shared `BaseMCPServer` infrastructure for protocol handling, compliance gating, and audit logging.
...
| [gateway](../core/gateway.md) | Calls HTTP API | All tool handlers invoke endpoints served by the platform gateway. |
| [core_logger](../core/shared_core.md) | Uses | Logs server lifecycle and errors via `core.logger`. |
...
It can also be registered with the platform's MCP registry and exposed through the [mcp_server_router](../core/shared_api_routers.md) over SSE or streamable HTTP.
...
- [gateway](../core/gateway.md) — Platform REST API that the platform MCP server calls.
...
- [shared_api_routers.md#mcp_server_router](../core/shared_api_routers.md) — HTTP/SSE registration and routing for MCP servers.
