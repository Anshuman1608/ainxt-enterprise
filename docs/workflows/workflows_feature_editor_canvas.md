...
| Module | Relationship |
|--------|-------------|
| workflowStore (`store/workflowStore.js`) | Central state store — nodes, edges, selection, execution state, undo/redo |
...
- Workflow Store — Zustand store with undo/redo, execution state, graph validation
