...
|------------|------|----------------|---------------|
| Workflow Engine | `workflows/engine.py` | DAG execution, step dispatch, HITL pause/resume, PCI checks, persistence | workflow_system_engine |
| Document Pipeline | `workflow/document.py` | Staged document generation planner with shared `DocumentState` and anti-drift figures | workflow_system_document_pipeline |
| Workflow Selector | `workflow/selector.py` | Pure intent-to-pipeline selection (`qa`, `document`, `sdlc`, `summarize`, etc.) | workflow_system_selector |
| Summarize Pipeline | `workflow/summarize.py` | Segment → map → reduce → format planner for summarization | workflow_system_summarize |
...
- **Model routing**: LLM steps are dispatched through `models.model_router` (see [model_routing](../llm/model_routing.md)).
- **Sandboxing**: Code and shell steps run through `sandbox.self_healing_engine` and `sandbox.docker_executor` (see [sandbox](../sandbox/sandbox.md)).
...
- **Memory & state**: Redis (`memory.redis_memory`, `core.kv`) provides fast transient state; Postgres (`db.models.WorkflowRunRecord`) provides durable run records (see database, memory_system).
- **Notifications**: Approval gates and workflow completion/failure publish inbox items via `store.inbox_store` (see store_layer).
- **Intent / CIL**: The selector is designed to consume a duck-typed `ConversationState` produced by the CIL layer; see [cil](../security/cil.md) and [pipeline](../core/pipeline.md) for intent detection and dispatch shaping.
