...
See core_db_pool.md for details on the shared pool shim.

### 3. Optimistic Concurrency and Data Integrity

`update_workflow` supports optimistic concurrency via `expected_updated_at`. If the stored `updated_at` timestamp does not match, a `StaleWorkflowError` is raised carrying the current row, allowing callers (e.g., [api_workflows.md](../api/api_workflows.md)) to surface an HTTP 409 conflict.
...
After updating a workflow or agent, the module calls `_governance("reconcile", ...)` to keep governance status in sync. See [core_governance.md](../sdlc/core_governance.md) and [api_governance.md](../api/api_governance.md) for the approval lifecycle.
...
- [api_workflows.md](../api/api_workflows.md): `list_workflows`, `create_workflow_route`, `update_workflow_route`, etc., delegate to `update_workflow`, `delete_workflow`, and duplicate helpers.
- [api_agents.md](../api/api_agents.md): `create_agent_route`, `update_agent_route`, `delete_agent_route` use `update_agent` and `delete_agent`.
- [api_catalog.md](../api/api_catalog.md): `list_skills_catalog`, `list_tools_catalog`, `delete_catalog_skill`, `delete_tool_route` use `list_skills`, `list_tools`, `delete_skill`, `delete_tool`.
- [api_templates.md](../api/api_templates.md) / [api_template_admin.md](../api/api_template_admin.md): template listing, usage, and admin seeding flows rely on seed-template helpers and `_upsert_seed_entry`.
- [api_execution.md](../api/api_execution.md): fetches workflow/agent rows and records executions via `get_execution`.
- [api_triggers.md](../api/api_triggers.md): `list_all_enabled_triggers` feeds the scheduler at startup.

### With Runtime and Scheduler

- [engine_native_engine.md](../agents/engine_native_engine.md): loads workflow `graph_data` and agent/tool definitions stored by this repo.
- [services_trigger_scheduler.md](../workers/services_trigger_scheduler.md): uses `list_all_enabled_triggers` to re-register scheduled jobs after a restart.

### With Core Modules

- core_db_pool.md: shares the single `_SharedPoolShim` instance; no separate pool is created.
- [core_config.md](../core/core_config.md): reads `postgres_enabled()` and other configuration flags.
- [core_governance.md](../sdlc/core_governance.md): reconciles governance status after workflow/agent mutations.

### With Models

- [app_models.md](../core/app_models.md): defines the Pydantic request/response shapes (`Workflow`, `AgentNode`, `TriggerOut`, etc.) that API routes pass into repository functions.
...
- core_db_pool.md — shared PostgreSQL pool shim
- [core_config.md](../core/core_config.md) — configuration and feature flags
- [core_governance.md](../sdlc/core_governance.md) — governance reconciliation
- [api_workflows.md](../api/api_workflows.md) — workflow HTTP endpoints
- [api_agents.md](../api/api_agents.md) — agent HTTP endpoints
- [api_catalog.md](../api/api_catalog.md) — skills/tools catalog endpoints
- [api_templates.md](../api/api_templates.md) — user template endpoints
- [api_template_admin.md](../api/api_template_admin.md) — admin template/seed endpoints
- [api_execution.md](../api/api_execution.md) — workflow execution endpoints
- [api_triggers.md](../api/api_triggers.md) — trigger management endpoints
- [engine_native_engine.md](../agents/engine_native_engine.md) — workflow runtime engine
- [services_trigger_scheduler.md](../workers/services_trigger_scheduler.md) — trigger scheduler
- [app_models.md](../core/app_models.md) — Pydantic data models
