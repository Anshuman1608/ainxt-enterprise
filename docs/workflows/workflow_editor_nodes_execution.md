...
- Surface **name conflicts** by turning the node red and showing a warning badge. The conflict check compares the node's name against:
  ...
  - Saved agents from `agentsStore`.
  - Saved workflows from `dashboardStore`.
...
Defined in `workflowStore.js` `getDefaultNodeData('agent')`:
...
- `store` — `workflowStore`, `agentsStore`, `dashboardStore` state management.
