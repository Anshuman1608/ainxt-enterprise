...
- `workflowStore` – Zustand store holding the current workflow graph, execution state, run telemetry, and chat thread state.
- `dashboardStore` – loads, creates, duplicates, and deletes workflows/templates.
- [`useFactoryChatStream`](../core/shared_features.md) – reusable Server-Sent Events (SSE) hook that powers the factory chat experience.
- [`FactoryChatShell`](../core/shared_features.md) – generic modal chat UI used by both agent and workflow factories.
- `triggersStore` / [`TriggerModal`](../ui/triggers_feature.md) / [`TriggerSection`](../ui/triggers_feature.md) – scheduling and webhook triggers attached to workflows.
...
See [Workflow Factory Chat](workflows_feature_factory_chat.md) for details.
...
- [Canvas](workflows_feature_editor_canvas.md) – the React Flow canvas, drag-and-drop, minimap, and layout helpers.
- [Chat Panel](workflows_feature_editor_chat_panel.md) – run input, streaming output, message rendering, file downloads, and run control.
- Config Panel – node configuration, generated instruction acceptance, and deletion.
- Debug Log View – execution timeline, status pills, generated files, and JSON inspection.
- Nodes – visual node components (agent, condition, loop, start, end, evaluation gate, subflow).
- Edges – custom edge rendering and deletion.
- Conditions – condition builder, cases, and loop-while editors.
- Sidebar – node palette and tool/skill catalog.
- Run Settings Strip – run-time parameters and overrides.
- Subflow Picker – selecting another workflow as a sub-flow node.
- Loop Items Picker – configuring loop iteration sources.
...
- `workflowStore` owns the in-editor state:
...
- `dashboardStore` owns the list views:
...
|------------|--------|------|
| `workflowStore` | store | Current workflow graph and execution telemetry |
| `dashboardStore` | store | Workflow/template library |
| `useFactoryChatStream`, `FactoryChatShell` | [shared_features](../core/shared_features.md) | Reusable factory chat engine and UI shell |
| `FactoryFileChips`, `DownloadNotice`, `sniffGeneratedFiles` | [shared_features](../core/shared_features.md) | Generated file download UX |
...
| `TriggerModal`, `TriggerSection` | [triggers_feature](../ui/triggers_feature.md) | Scheduling/webhook triggers |
| `TemplateCardMenu`, `TemplateCreateModal` | [templates_feature](../api/templates_feature.md) | Template admin UI |
| `useTriggerPortalContainer` | [triggers_feature](../ui/triggers_feature.md) | Shared portal container for toasts/modals |
| `formatDate`, `stripTemplateTag` | [utils](../ui/utils.md) | Formatting helpers |
| `API_BASE`, `buildAuthHeaders` | [config](../core/config.md) | Backend API configuration |
