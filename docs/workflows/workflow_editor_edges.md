...
4. **Maintaining editor state consistency** by delegating all mutations to the central `workflowStore`.
...
`AiEdge` reads and writes the following slices of `workflowStore`:
...
- **It relies on the central store.** All mutations pass through `workflowStore`, which also provides utilities such as `createWorkflowEdgeId`, `hasIllegalCycle`, and `pruneToConnectedSubgraph`.
...
- `store` — Central Zustand store providing `insertNodeOnEdge`, `removeEdge`, `hoveredEdgeId`, and `activeNodeIds`.
