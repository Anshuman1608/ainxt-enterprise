...
The frontend node types correspond to the backend workflow model defined in [`app_models`](../core/app_models.md). Default data shapes for new nodes are produced by `getDefaultNodeData` in `workflowStore`, which keeps the canvas schema aligned with the engine in [`engine_native_engine`](../agents/engine_native_engine.md).
