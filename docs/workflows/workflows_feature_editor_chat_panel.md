...
the `workflowStore` (Zustand + zundo temporal middleware).  The relevant
...
|---|---|---|
| `workflowStore` | `store` | Chat state, execution state, run context, node graph |
| `dashboardStore` | `store` | `updateWorkflow` for Save & approve persistence |
| `triggersStore` | `store` | Recent trigger executions, mark seen |
...
- Workflow Store (`store`) — Zustand store with chat, execution, and run-context slices
