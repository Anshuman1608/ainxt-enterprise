...
[workflow editor Sidebar](workflow_editor.md).

Each node is a **presentational React component** rendered by React Flow inside the
[Canvas](workflow_editor.md). They subscribe to the
workflow store for live execution state (active node highlighting, loop
progress) and expose typed **source handles** that the backend
[NativeEngine](../agents/engine_native_engine.md) reads during graph traversal to decide which
edge to follow.
...
2. **Data shape contract** — each node's `data` object must match what the
   [ConfigPanel](workflow_editor.md) writes and what the backend
   [NativeEngine](../agents/engine_native_engine.md) reads.
...
([`app/models.py::ConditionCase`](../core/app_models.md)):
...
[`buildCombinedExpressionPreview`](../core/constants.md) from `constants/operators.js` to
...
workflow store to apply the `executing` CSS class. It also reads
`data.status` / `data.executionStatus` / `data.state` for `success` / `error`
visual states.

#### Related Editors

Condition nodes are configured in the [ConfigPanel](workflow_editor.md)
...
[`app.loop.runner`](../agents/loop_runner.md) — the same helper used by the
...
[loop_runner](../agents/loop_runner.md) for the evaluator's rubric and scoring contract.
...
workflow store. The backend emits `loop_iteration_start` and
...
Loop nodes are configured in the [ConfigPanel](workflow_editor.md),
which renders mode-specific editors:

- **for_each** → [LoopItemsPicker](workflow_editor.md) for
...
|-----------|------|----------------|
| `workflowStore` | State management | store |
| `constants/operators.js` | Expression preview helpers | [constants](../core/constants.md) |
...
| `NativeEngine` | Backend graph traversal | [engine_native_engine](../agents/engine_native_engine.md) |
| `app/models.py` | Backend data models | [app_models](../core/app_models.md) |
| `loop_evaluator.py` | LLM judge + controller | [engine_loop_evaluator](../agents/engine_loop_evaluator.md) |
| `loop/runner.py` | `evaluate_llm_judge` helper | [loop_runner](../agents/loop_runner.md) |
...
The workflow store's `getDefaultNodeData(type)` function provides
...
See store for the complete `getDefaultNodeData` implementation.
...
| Workflow store (Zustand) | store |
| Backend execution engine | [engine_native_engine](../agents/engine_native_engine.md) |
| Backend data models | [app_models](../core/app_models.md) |
| Loop evaluator (LLM judge) | [engine_loop_evaluator](../agents/engine_loop_evaluator.md) |
| Loop runner system | [loop_runner](../agents/loop_runner.md) |
| Loop models | [loop_models](../agents/loop_models.md) |
| Constants (operators) | [constants](../core/constants.md) |
