...
- **Downstream**:
  - [`shared_core.md`](../core/shared_core.md) — the `EvalEngine` in `core/evals.py` produces the metrics and results that evaluation suites consume.
  - [`shared_api_routers.md`](../core/shared_api_routers.md) — the `evals_router` exposes evaluation results through HTTP endpoints such as `list_eval_results`, `eval_summary`, and `eval_trend`.
  - `ai_ui_frontend.md` — the `EvalsDashboard` component visualizes evaluation summaries, trends, and result rows for users.
