...
The router interacts with the following models from db/models.py:
...
|---|---|---|
| `auth/rbac.py` | [Authentication](../auth/authentication.md) | `require_admin_flag` dependency for all routes. |
| `core/config.py` | [Core Infrastructure](../core/core_infrastructure.md) | Feature flags and weekly mail schedule config. |
| `db/models.py` | Database | All Coach-related ORM models. |
| `agents/coach_evaluator.py` | [Coach System](coach_system.md) | Rule registry, score computation, inbox publishing. |
| `agents/coach_recommender.py` | [Coach System](coach_system.md) | Personalized recommendation generation. |
...
| `services/smtp_service.py` | [Services](../workers/services.md) | HTML email delivery for manual coaching notes. |
| `store/inbox_store.py` | Store Layer | Inbox item publishing for coaching note delivery. |
