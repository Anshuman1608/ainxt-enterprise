...
|-----------|-----------|------|
| `workflow_repo` | [core_workflow_repo](../workflows/core_workflow_repo.md) | Trigger CRUD, execution persistence, workflow/agent lookups |
| `NativeEngine` | [engine_native_engine](../agents/engine_native_engine.md) | Workflow/agent execution via SSE streaming |
| `governance` | [core_governance](../sdlc/core_governance.md) | Audit events, budget checks, usage tracking |
| `compliance_engine` | [agent_system](../agents/agent_system.md) | PII/PCI compliance validation on trigger inputs |
| `prompt_injection` | [core_infrastructure](../core/core_infrastructure.md) | Injection detection on trigger payloads |
| `loop.runner` | [loop_runner](../agents/loop_runner.md) | `TriageSkill` for Loop Engineering triage cycles |
| `core.config` | [core_config](../core/core_config.md) | `triage_interval_cron`, `loop_triage_enabled`, LLM endpoint helpers |
| `db_pool` | core_db_pool | `SHARED_POOL` for lazy pool binding in RQ workers |
| `app.models` | [app_models](../core/app_models.md) | `TriggerCreate`, `TriggerUpdate`, `TriggerSchedule`, `TriggerOut` |
...
See [api_triggers](../api/api_triggers.md) for the full webhook flow.
...
|----------|---------|-----------|
| `init_scheduler()` | Start APScheduler (triage only) + bootstrap triage jobs | `_lifespan` in [app_main](../core/app_main.md) |
| `shutdown_scheduler()` | Stop APScheduler, clear job indices | `_lifespan` |
| `register_trigger(trigger)` | Compute initial `next_run_at` for a new/updated trigger | `create_trigger_route`, `update_trigger_route` in [api_triggers](../api/api_triggers.md) |
...
### 11.1 FastAPI Lifespan ([app_main](../core/app_main.md))
...
### 11.2 API Layer ([api_triggers](../api/api_triggers.md))
...
### 11.4 Loop Engineering ([loop_runner](../agents/loop_runner.md))

Loop CRUD endpoints call `register_triage_job` / `deregister_triage_job` to keep APScheduler in sync with the loops table. `_fire_triage` delegates to `TriageSkill.run()` from [loop_runner](../agents/loop_runner.md).

### 11.5 Execution Engine ([engine_native_engine](../agents/engine_native_engine.md))
