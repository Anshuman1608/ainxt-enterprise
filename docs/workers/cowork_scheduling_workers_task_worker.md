...
| [Agent System](../agents/agent_system.md) → OrchestratorAgent | `agent.run(mode="office")` | Execute the task through the office-assistant planner |
| [Agent System](../agents/agent_system.md) → ComplianceEngine | `validate_input`, `validate_output`, `redact_text` | NPCI compliance redaction |
| [Model Routing](../llm/model_routing.md) → ModelRouter | `generate(model_hint="complex")` | Re-ask envelope repair |
| [Connector Infrastructure](../skills/shared_integrations_connector_infrastructure.md) → MCP Bridge | `call_tool` | Send email via the same pipeline as interactive CoWork |
| [Connector Infrastructure](../skills/shared_integrations_connector_infrastructure.md) → ConnectorEngine | `_check_user_permission` | Platform-wide permission check |
| [Connector Infrastructure](../skills/shared_integrations_connector_infrastructure.md) → ConnectorRegistry | `get_user_status` | Resolve user's M365 email address |
| Store Layer → Inbox Store | `publish_inbox_item` | Make headless run results visible to the user |
| Store Layer → Skill Loop Store | `record_run_signature` | Self-improving skill loop capture |
| [Core Infrastructure](../core/core_infrastructure.md) → Logger | `logger` | Structured logging |
| [Core Infrastructure](../core/core_infrastructure.md) → Config | `ENABLE_SKILL_LOOP`, `SKILL_LOOP_SOURCES` | Feature flags for skill loop |
| Database | `SessionLocal` | PostgreSQL session factory |
