...
The workers layer sits between API producers (gateway, routers) and platform storage. Jobs are enqueued to Redis queues, consumed by RQ worker processes, and results are persisted to Postgres, Redis, or object storage.
...
flowchart TB
    ...
    subgraph QueueLayer["Queue Layer (Redis)"]
        Q_CHAT[chat_queue]
        Q_AGENT[agent_queue]
        Q_SDLC[sdlc_queue]
        Q_DOC[doc_queue]
...
- [sdlc_pipeline_workers](../sdlc/sdlc_pipeline_workers.md) — SDLC pipeline execution and HITL resume
...
- [broadcast_coach_workers](../connectors/broadcast_coach_workers.md) — Broadcasts, Coach ingestion, graph edges
