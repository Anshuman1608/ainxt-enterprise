...
Launches `workers/kafka_consumer.py` as a subprocess. If the consumer exits, it waits 10 seconds and restarts (supervisor loop). The Kafka consumer handles event streams for SDLC, embeddings, budget, audit log, threads, agents, chat history, and metrics — see kafka_event_consumer.
...
| kv_store | Provides `core/kv/queue.py` — the worker/connection factory that abstracts Redis vs. RustyCluster |
