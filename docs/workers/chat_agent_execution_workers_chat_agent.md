...
The `chat_agent_execution_workers_chat_agent` module is the **core asynchronous execution layer** for user-facing chat and agent interactions in the AiNxt platform. It contains two RQ (Redis Queue) job functions that are enqueued by the [gateway](../core/gateway.md) and consumed by background worker processes:
...
|---|---|---|
| `AgentRunner` | [agent_system](../agents/agent_system.md) (`agents/agent_builder.py`) | Core agent execution engine |
| `logger` | [core_infrastructure](../core/core_infrastructure.md) (`core/logger.py`) | Structured logging |
...
|---|---|---|
| `compliance_engine` | [agent_system](../agents/agent_system.md) | Input/prompt/answer compliance gating |
| `model_router` | [model_routing](../llm/model_routing.md) | LLM routing, streaming, fallback |
| `RedisMemory` | memory_system | Conversation history persistence |
| `hybrid_search` / `hybrid_retriever` | [model_routing](../llm/model_routing.md) | RAG retrieval + BGE reranking |
| `budget_store` | store_layer | Budget checking and usage increment |
| `semantic_cache` | store_layer | L2/L3 semantic caching |
| `context_manager` | [core_infrastructure](../core/core_infrastructure.md) | Engineer cross-session context |
| `distributed_semaphore` | [core_infrastructure](../core/core_infrastructure.md) | LLM concurrency throttle |
| `kafka_producer` | [core_infrastructure](../core/core_infrastructure.md) | Async persistence to Postgres |
| `job_queue` | [core_infrastructure](../core/core_infrastructure.md) | Enqueue doc generation jobs |
| `doc_worker` / `doc_worker_agent` | [document_knowledge_workers](document_knowledge_workers.md) | Document generation |
| `doc_generator` (tools) | [shared_integrations](../skills/shared_integrations.md) | Filename/theme utilities |
| `classifier` | [model_routing](../llm/model_routing.md) | Query complexity + domain classification |
| `query_rewriter` | [model_routing](../llm/model_routing.md) | Code-domain query rewriting |
| `section_query_router` | [core_infrastructure](../core/core_infrastructure.md) | KB section-aware reranking |
| `chat_utils` | [core_infrastructure](../core/core_infrastructure.md) | PII masking, cache keys, repo detection |
...
| Topic | Documentation |
|---|---|
| Agent execution engine | [agent_system](../agents/agent_system.md) — `AgentRunner`, `AgentBuilder` |
| Model routing & fallback | [model_routing](../llm/model_routing.md) — `ModelRouter`, gateway dispatch |
| Conversation memory | memory_system — `RedisMemory`, `PostgresMemory` |
| Compliance & governance | [agent_system](../agents/agent_system.md) — `ComplianceEngine` |
| Job queue infrastructure | [core_infrastructure](../core/core_infrastructure.md) — `enqueue_job`, queue management |
| Worker process management | [worker_orchestration](#worker-orchestration) — `start_workers.py` |
| Document generation | [document_knowledge_workers](document_knowledge_workers.md) — `doc_worker`, `doc_worker_agent` |
| Budget management | store_layer — `budget_store` |
| Semantic caching | store_layer — `semantic_cache` |
| Kafka event consumption | [kafka_event_consumer](kafka_event_consumer.md) — `kafka_consumer.py` |
| Gateway API layer | [gateway](../core/gateway.md) — `gateway.py` |
