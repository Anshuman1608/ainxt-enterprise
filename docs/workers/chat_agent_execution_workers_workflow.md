...
- **[`core_infrastructure`](../core/core_infrastructure.md)** — Job queue, logging, and KV store infrastructure.
- **`database`** — Database models including `WorkflowRunRecord` and `workflow_history`.
- **`memory_system`** — `PostgresMemory` and other memory backends.
