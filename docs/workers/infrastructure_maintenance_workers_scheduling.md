...
`core_db_pool`. The same lazy binding is repeated inside
...
| `core_db_pool` | **Infrastructure** — `SHARED_POOL` used for lazy pool binding in worker processes |
| `database` | **Data model** — `ScheduledWorkflow` ORM model for Path A |
...
| `SHARED_POOL` | `core_db_pool` | Lazy-bound when `workflow_repo._pool` is None in a worker process |
