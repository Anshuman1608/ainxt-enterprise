...
> **Note:** The diagram above is a simplified UI-perspective view. The authoritative state machine lives in the backend ([`agents/sdlc_state_machine.py`](../sdlc/shared_core_sdlc_pipeline.md)) and the stage manifest in [`store/sdlc_stage_manifest.py`](../sdlc/shared_core_sdlc_pipeline.md). The UI never drives state transitions directly — it only sends action requests that the backend validates and executes.
...
| `store/sdlc_store.py` | store_layer — run persistence and state management |
| `store/sdlc_stage_manifest.py` | store_layer — canonical pipeline manifest |
