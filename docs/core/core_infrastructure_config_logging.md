...
| KV Store | Per-DB backend selection (`REDIS` vs `RUSTYCLUSTER`) originates from `kv_backend_for()` and `KV_BACKEND_MAP`. |
| Database | `postgres_dsn()`, `postgres_read_dsn()`, `pgvector_dsn()`, and `pgvector_read_dsn()` supply connection strings to SQLAlchemy session factories. |
