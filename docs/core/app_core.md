...
> See [triggers_feature.md](../ui/triggers_feature.md) for `TriggerNotifications`.
...
|---|---|---|
| `useWorkflowStore` | store.md | Canvas graph (`nodes`, `edges`), execution state (`isExecuting`, `workflowId`), knowledge config, name/id setters |
| `useDashboardStore` | store.md | Workflow list, `updateWorkflow`, `loadWorkflows`, `useTemplate` (clone) |
| `useAgentsStore` | store.md | Agent list, `loadAgents` |
| `useTemplateAdminStore` | store.md | `updateTemplate` (optional template editor; safe to remove) |

### Utility dependencies

| Utility | File | Purpose |
|---|---|---|
| `editorPersistence` | [utils.md](../ui/utils.md) | `localStorage`-backed UI view-state: open-editor pointer, active thread, composer draft, selected node. Namespaced by user id. |
| `validateName` | [utils.md](../ui/utils.md) | `validateEntityName` (format + uniqueness) and `suggestFreeName` (auto-resolve duplicates). Mirrors backend rules in `workflow_repo.py`. |
...
> See [utils.md](../ui/utils.md) for the `editorPersistence` API (`ensureUserNamespace`, `loadOpenEditor`, `saveOpenEditor`, `loadSelectedNode`, etc.).
...
> See [utils.md](../ui/utils.md) for `validateEntityName` and `suggestFreeName`. These mirror the backend rules in `ABStudio/backend/app/core/workflow_repo.py::_validate_name_format`.
...
| [skills_feature.md](../skills/skills_feature.md) | `SkillsDashboard` is rendered in the skills section (no callbacks needed — self-contained). |
| [tools_feature.md](../skills/tools_feature.md) | `ToolsDashboard` is rendered in the tools section (self-contained). |
...
| [triggers_feature.md](../ui/triggers_feature.md) | `TriggerNotifications` bell lives in `AppTopBar` (dashboard + agent editor surfaces; intentionally absent from the workflow editor where `ChatPanel` surfaces triggered runs inline). |
| [governance_feature.md](../sdlc/governance_feature.md) | `StatusBadge` and `SubmitApprovalButton` render in the editor header. |
| store.md | All four Zustand stores consumed by `App`. |
| [utils.md](../ui/utils.md) | `editorPersistence` (localStorage view-state) and `validateName` (name validation). |
...
| abstudio_backend.md | Backend API endpoints consumed via stores: `/workflows`, `/workflows/{id}`, `/templates/{id}`, `/templates/{id}/use`, `/agents`, `/auth/me`. |
