...
The middleware stack is mounted inside the main FastAPI/Starlette application in `gateway.py` (see [gateway.md](gateway.md)). It runs before any router is matched, which means every path—including the OpenAI-compatible compatibility endpoints, IDE endpoints, and ABStudio API routes—receives the same treatment.
...
See [middleware_client_source.md](middleware_client_source.md) for the detection order, canonical values, and special handling for the desktop app and browser-automation extension.
...
See middleware_rate_limit.md for limit configuration, exempt paths, identity resolution, and header semantics.
...
See middleware_budget.md for enforced paths, model-hint logic, identity resolution caching, and the budget-exceeded response format.

## Shared Dependencies

The middleware layer relies on several platform modules that are documented separately:

- **[core_infrastructure.md](core_infrastructure.md)** — `core.logger` for request-scoped logging and `core.rate_limiter` for sliding-window enforcement.
- **[auth.md](../auth/auth.md)** — `auth.jwt_handler.decode_token` and `auth.api_key_auth` for identity resolution from Bearer tokens and platform API keys.
- **store_layer.md** — `store.budget_store` and `store.inbox_store` for budget checks and budget-alert notifications.
