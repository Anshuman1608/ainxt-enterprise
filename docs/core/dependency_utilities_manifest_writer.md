...
`dependency_utilities` grouping. It relies on the store_layer
...
|---|---|---|
| `store.sdlc_store.list_run_repos` | store_layer | Retrieves all `sdlc_run_repos` rows for a given run, ordered by `build_order` then `repo` |
...
| store_layer | Provides `sdlc_store.list_run_repos` for run-repo persistence |
