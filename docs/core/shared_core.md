...
`shared_core` sits between the platform's entry points (gateway, routers, workers, frontends) and the external infrastructure (Postgres, Redis, LLM providers, identity providers, SaaS APIs).
...
flowchart TB
    ...
    subgraph shared_core["shared_core"]
        ...
        DB["database + kv_store<br/>Postgres, pgVector, Redis"]
...
| `database` | SQLAlchemy engines, ORM models, RLS, migrations | database |
| `authentication` | JWT/API-key auth, LDAP, SSO, RBAC/ABAC | [authentication](../auth/authentication.md) |
| `agent_system` | Agent framework, orchestration, reasoning, compliance, safety | [agent_system](../agents/agent_system.md) |
| `shared_core_sdlc_pipeline` | AI-driven software delivery lifecycle engine | [shared_core_sdlc_pipeline](../sdlc/shared_core_sdlc_pipeline.md) |
| `mcp_system` | MCP registry, tool/skill routing, external MCP clients | [mcp_system](../mcp/mcp_system.md) |
| `memory_system` | Durable scoped memory and sensitivity-aware storage | memory_system |
| `middleware` | Request ID, client source, rate limiting, budget middleware | [middleware](middleware.md) |
| `model_routing` | Model selection, hybrid search, knowledge graph, KB versioning | [model_routing](../llm/model_routing.md) |
| `shared_core_knowledge_base` | Document ingestion, chunking, entity registry | [shared_core_knowledge_base](../knowledge/shared_core_knowledge_base.md) |
| `connectors_integrations` | OAuth, connector adapters, DPI consent | [connectors_integrations](../connectors/connectors_integrations.md) |
| `guardrails_tools` | Runtime input safety / NeMo Guardrails | [guardrails_tools](../security/guardrails_tools.md) |
| `workflow_system` | DAG-based workflow execution and planners | [workflow_system](../workflows/workflow_system.md) |
| `store_layer` | Domain stores for budget, inbox, SDLC, threads, SSO | store_layer |
| `services` | Budget digests, coach ingestion, Teams integration, notifications | [services](../workers/services.md) |
| `llm_spend` | LLM spend fetching and reporting | [llm_spend](../analytics/llm_spend.md) |
| `coach_system` | Coach rule evaluation and feedback | [coach_system](../coach/coach_system.md) |
| `evals_evolution` | Evaluation harness and safe self-improvement | [evals_evolution](../observability/evals_evolution.md) |
| `pipeline` | Degradation ladder, dispatch decisions, stream events | [pipeline](pipeline.md) |
| `profiles` | Domain profiles and policy-as-data | [profiles](../llm/profiles.md) |
| `router_policy` | Pure model-selection policy | [router_policy](../llm/router_policy.md) |
| `observability` | Turn tracing and metrics | [observability](../observability/observability.md) |
| `sandbox` | Secure code and document execution | [sandbox](../sandbox/sandbox.md) |
| `cil` | Conversation Intelligence Layer | [cil](../security/cil.md) |
| `context_engine` | Context-window planning | [context_engine](../llm/context_engine.md) |
| `document_processing` | Docling / legacy document parsing | [document_processing](../documents/document_processing.md) |
| `kv_store` | Backend-agnostic Redis layer | kv_store |
| `ckms` | Centralized key management | [ckms](../auth/ckms.md) |
| `indexers` | Confluence / Jira indexing | [indexers](../knowledge/indexers.md) |
| `ainxt_scripts` | CI indexing and validation scripts | [ainxt_scripts](ainxt_scripts.md) |
| `presenton_patches` | Presentation-generation compatibility layer | [presenton_patches](../presentation/presenton_patches.md) |
| `shared_core_tools` | Agent-callable tool libraries | [shared_core_tools](../skills/shared_core_tools.md) |
| `scripts_utilities` | Operational and seeding scripts | [scripts_utilities](scripts_utilities.md) |
| `dev_workspace` | Development scratchpad utilities | [dev_workspace](../cowork/dev_workspace.md) |
| `gunicorn_config` | Production Gunicorn configuration | [gunicorn_config](gunicorn_config.md) |
| `doc_generation` | Document-generation helpers | [doc_generation](../documents/doc_generation.md) |