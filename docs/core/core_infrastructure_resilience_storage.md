...
`EVALSHA` with automatic `EVAL` fallback — see kv_store for the
...
| `core.kv.get_kv()` | `CircuitBreaker`, `DistributedSemaphore` | Backend-agnostic KV access (Redis) | kv_store |
| `core.kv.KVError` | `CircuitBreaker` | Detect KV unavailability | kv_store |
| `core.kv.KVClient.register_script()` | `DistributedSemaphore` | Atomic Lua script execution | kv_store |
...
> [core_infrastructure_config_logging](core_infrastructure_config_logging.md)
> and kv_store for the full DB allocation table and backend
> selection logic.
...
| KV layer (Redis abstraction) | kv_store |
