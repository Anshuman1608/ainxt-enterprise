...
> See db_models.md for the full ORM schema of `MessageFeedback`, `RepoPermission`, and `memory_entries`.
...
| Dependency | Purpose |
|------------|---------|
| [auth_router.md](../auth/auth_router.md) / `auth.dependencies` | JWT-based user authentication (`get_current_user`). |
| [chat_router.md](chat_router.md) | Provides the message context that feedback is attached to. |
| memory_router.md | Consumes preference memory entries written from feedback. |
...
> For the full chat lifecycle, see [chat_router.md](chat_router.md). For how feedback drives memory and personalization, see memory_router.md.
