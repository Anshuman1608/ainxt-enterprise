...
> **Who this guide is for:** The person setting up AiNxt Enterprise — typically someone on an IT or DevOps team. If you're an end user who just wants to use the platform, ask your IT team to set it up and give you the URL. You don't need this guide.
>
> If you're looking for quick commands, env var reference, or the restart guide, see [`docs/QUICK_REFERENCE.md`](QUICK_REFERENCE.md).
...
- [ ] **~10 GB memory** available to Docker
- [ ] **~16 GB free disk** space
- [ ] A decision on which AI provider to use (Ollama = free and local; Anthropic/OpenAI/Gemini = cloud, requires a paid API key)
- [ ] On Windows: WSL2 + Ubuntu + Docker Desktop with WSL integration enabled (see the [Windows setup guide](../README.md#windows-setup-step-by-step) in the README)

That's it. No Python, Node, PostgreSQL, or Kafka to install separately — the installer handles everything.
...
It starts **seven** services:

| Service | Port | Why |
|---|---|---|
| `postgres` | 5432 | Main database **and** the pgvector store |
| `redis` | 6379 | Cache, RQ job queues, 9 KV logical DBs, login-lockout counters |
| `kafka` | 9092 | Required event pipeline — carries chat, audit and usage events |
| `kafka-consumer` | — | Required. Writes those events into Postgres |
...
Prometheus/Grafana and the embedding service are **not** started — see
[Optional features](#optional-features). Kafka is deliberately not in that
list: it is required, and both it and its consumer start by default. See
[The event pipeline](#the-event-pipeline).
...
> **When to use this:** If you're a developer who wants hot-reload, a debugger, or to run the API as a normal process you can inspect. For most setups, the Docker path above is simpler.
...
> **Plain English:** AiNxt doesn't come with an AI model built in — you bring your own. You can use a free local model (Ollama, runs on your machine, no internet required) or a cloud provider (Anthropic, OpenAI, Google — requires a paid API key). The installer asks you which one you want and sets it up for you. You can add more providers or switch the default at any time from the **LLM Providers** screen in the admin panel.
...
budget instead (default $50, configurable via `BUDGET_BASE_COST_USD`, shown
bottom-left in the UI). To add a key later:
...
> **Plain English:** AiNxt uses a message queue (called Kafka) to make sure nothing gets lost — think of it like a reliable inbox between services. The installer sets it up for you automatically. This section explains what it does and what breaks if it's not running, so you can diagnose problems if they arise.

Kafka is **required**, and the one-command install sets it up for you. It is
documented here because the reason is not obvious and because a broken pipeline
looks exactly like a healthy install.

The gateway does not write most rows synchronously. Chat turns, audit entries,
`model_usages`, and thread, SDLC, budget, agent and Coach events are
*published* to a topic; `workers/kafka_consumer.py` consumes them and performs
the `INSERT`. So two things must be running:

| Piece | Container | Without it |
|---|---|---|
| Broker | `ainxt-kafka` | Events cannot be published; they queue in Redis instead |
| Consumer | `ainxt-kafka-consumer` | Events are published and never become rows |

If either is down, `/health` still returns green and the UI still renders. The
symptom is missing data: a conversation you reopen is empty, Analytics shows no
spend, the audit log has gaps.

With `KAFKA_ENABLED=false`, `core/kafka_producer.py` does not discard events —
it pushes them onto Redis lists named `kafka:fallback:{topic}` carrying a
**7-day TTL**. Those lists are drained only when `kafka_consumer.py` next
starts. Start a consumer inside that window and the backlog is written; leave
it longer and the events expire unread. The one exception with a direct
synchronous write is the compliance-blocked audit entry.

```bash
docker compose logs -f kafka-consumer   # confirm it is consuming
./doctor.sh                             # broker, consumer and Redis backlog, as required checks
```

Running natively? The broker still runs in Docker; `./install.sh --local`
starts the consumer as a host process, logging to `log/kafka-consumer.out` and
stopped by `./stop-local.sh`. To run it by hand:
...
> **Plain English:** The following features are switched off on a fresh install. You don't need any of them to get started — turn them on when you're ready.
...
### Knowledge Base — parsing, embedding, and reranking (semantic search)

> **Plain English:** This lets AiNxt search your documents by *meaning*, not just keywords. For example, asking "what is our refund policy?" will find the right paragraph even if it doesn't use those exact words. It's optional — the platform works without it, but the Knowledge Base page will show a setup banner until you run this.

```bash
./kb-setup.sh
```
`./install.sh` offers to run this for you interactively at the end of a
fresh install (say yes and it calls this same script) — but it's also a
standalone, re-runnable script: say no now and run it later at any time, no
fresh install needed. It builds `embed-svc` locally from the repo
(`services/embed_svc/`) — there is no published image to pull, and none is
required. Leave `EMBED_SVC_URL` empty in `.env` to run without the
Knowledge Base — this is why `/health` reports `degraded` on a default
install, `embed_svc` is absent by design (the Knowledge Base page in the
app shows a banner pointing back at `./kb-setup.sh` in that case). See
[`KB_SETUP.md`](KB_SETUP.md) for the full model/library breakdown and how to
choose a different embedding/reranker model.
...
> **Start here:** Run `./doctor.sh` first. It checks every part of the install and tells you exactly what to fix. Most problems are solved by reading its output. The specific scenarios below are for cases where `doctor.sh` passes but something still isn't working.
