...
The component is part of the workflow editor sub-module tree and is rendered inside the [ConfigPanel](ConfigPanel.md) when a Loop node is selected and its mode is set to `for_each`.
...
Three pure functions support the picker. They are also imported directly by the [ConfigPanel](ConfigPanel.md) for upstream-node derivation.
...
The [ConfigPanel](ConfigPanel.md) renders `LoopItemsPicker` only when the selected node is a Loop node and its mode is `for_each`:
...
The `itemsExpression` value flows through the workflow store (`workflowStore.js`):
...
Defined in `ABStudio/backend/app/api/chat.py::get_node_last_output`, this endpoint delegates to `NativeEngine.get_node_last_output(thread_id, node_id)`, which loads the persisted record from the [CheckpointStore](../agents/checkpoint.md) via `load_node_output(thread_id, node_id)`.
...
| Module | Relationship |
|--------|-------------|
| [ConfigPanel](ConfigPanel.md) | Parent component that renders `LoopItemsPicker` inside the Loop configuration form. |
...
| workflowStore | Zustand store holding nodes, edges, and the `getWorkflowForExecution` serializer that forwards `itemsExpression` to the backend. |
...
| [engine_native_engine](../agents/engine_native_engine.md) | The `NativeEngine` class that executes workflows, runs `_run_loop`, and provides `get_node_last_output()`. |
| [checkpoint](../agents/checkpoint.md) | Checkpoint store layer that persists per-node outputs (`load_node_output`) queried by the picker. |
