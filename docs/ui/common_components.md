...
All components are implemented as functional React components using hooks. They communicate with the backend through the shared API layer in `ABStudio/frontend/src/config/api` (see [`config`](abstudio_frontend.md) for details) and rely on parent components to persist state into the workflow/agent model.
...
- **Does not own persistence**: State is always lifted to parent components that interact with [`workflowStore`](abstudio_frontend.md) or the agent API.
- **Does not own layout chrome**: Shell-level layout, navigation, and dashboards live in [`app_core`](abstudio_frontend.md) and feature modules.
