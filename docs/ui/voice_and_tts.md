...
[`gateway.py`](../core/gateway.md). They keep API keys server-side, enforce
...
(the "no-lazy-load" rule), either in the dedicated [`llm_proxy`](../llm/llm_proxy.md)
...
1. If `LLM_PROXY_URL` is configured, the request is forwarded to
   [`llm_proxy /llm/tts`](../llm/llm_proxy.md). This is the production path
...
|---------|-----------|---------|---------|
| `llm_proxy` | [`llm_proxy.md`](../llm/llm_proxy.md) | `voice_tts` | Production TTS proxy with outbound internet access. |
...
|----------|-----------|-------|
| `Chat.jsx` | `ai_ui_frontend_chat.md` | `handleMicToggle` captures browser speech; `sendMessageForVoice` submits transcribed text. |
| `KbChat.jsx` | `ai_ui_frontend_kb_chat.md` | Same voice input flow in knowledge-base chat. |
| `VoiceMode.jsx` | `ai_ui_frontend_voice_mode.md` | Dedicated full-duplex voice UI (if present). |

### Shared gateway dependencies

* [`gateway.md`](../core/gateway.md) — the host module providing routing, auth, and
  common exception handling.
* [`auth.md`](../auth/auth.md) / [`shared_core.md`](../core/shared_core.md) — identity and RBAC
  via `_require_auth`.
...
* The `ai_ui_frontend` chat components use browser-native
...
* The [`gateway`](../core/gateway.md) module hosts these endpoints alongside chat,
  agent, workflow, and audit routes, reusing the same auth and logging
  infrastructure.
* The [`llm_proxy`](../llm/llm_proxy.md) service handles the actual OpenAI TTS call in
