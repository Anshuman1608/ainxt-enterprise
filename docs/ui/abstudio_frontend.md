...
|---|---|---|
| **app_core** | Root `App` component, top-level navigation, dashboard/editor view switch, autosave orchestration, reload-restore, template preview/clone, and error boundary. | [app_core.md](../core/app_core.md) |
...
| **skills_feature** | Skills catalog dashboard and `SkillFactoryChat` for AI-assisted skill authoring. | [skills_feature.md](../skills/skills_feature.md) |
| **tools_feature** | Read-only tools catalog with search, grouping, and detail modal. | [tools_feature.md](../skills/tools_feature.md) |
| **templates_feature** | Feature-flagged template admin UI: create, edit, reset, save-to-seed, and delete templates. | [templates_feature.md](../api/templates_feature.md) |
| **triggers_feature** | Trigger scheduler UI, execution history, and global notification bell. | [triggers_feature.md](triggers_feature.md) |
...
| **shared_features** | Cross-cutting factory chat shell, file chips, OCR text preview, and download notices. | [shared_features.md](../core/shared_features.md) |
| **store** | Zustand stores: `workflowStore` (canvas + execution state) and `triggersStore` (trigger CRUD + notifications). | store.md |
| **utils** | Client-side persistence (`editorPersistence`), transient ID generation (`makeId`), and thread/chat formatting helpers (`threadHelpers`). | [utils.md](utils.md) |
| **hooks** | Reusable React hooks, currently `useHoverTooltip` for accessible hover/focus tooltips. | [hooks.md](hooks.md) |
| **constants** | Canonical field/operator catalogs and expression-preview helpers for the visual condition builder. | [constants.md](../core/constants.md) |
...
- [core_workflow_repo.md](../workflows/core_workflow_repo.md) — backend persistence layer
- [engine_native_engine.md](../agents/engine_native_engine.md) — workflow execution engine