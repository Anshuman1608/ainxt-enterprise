...
1. **Parent SDLC UI** — typically the [SDLCPipeline](../sdlc/sdlc_pipeline.md) view — passes the current `run` object into `NavigatorActivity`.
2. **Backend SDLC events API** — the [sdlc_router](../sdlc/sdlc_router.md) exposes `/sdlc/runs/{runId}/events` and is consumed via `apiFetch` from [config](../ui/ai_ui_frontend_app_core.md).
3. **Agent loop / worker** — the [sdlc_agent_loop](../sdlc/shared_core_sdlc_pipeline.md) and [sdlc_worker](../workers/workers.md) produce the `agent-loop` actor events that this component displays.
...
The component looks for `read_file` or `explore-read` substrings, then extracts the first quoted path-like token matching `'"['"]`.
...
| `API_BASE`, `apiFetch` | `../config` | Backend base URL and authenticated fetch helper. See [config](../ui/ai_ui_frontend_app_core.md). |

### 7.2 Related Backend Modules

- [sdlc_router](../sdlc/sdlc_router.md) — serves the `/sdlc/runs/{runId}/events` endpoint.
- [sdlc_worker](../workers/workers.md) — persists run events produced by the SDLC pipeline.
- [sdlc_agent_loop](../sdlc/shared_core_sdlc_pipeline.md) — the agent loop that emits `agent-loop` actor events.

### 7.3 Related Frontend Modules

- [SDLCPipeline](../sdlc/sdlc_pipeline.md) — the primary parent view that hosts `NavigatorActivity` during SDLC runs.
- [ai_ui_frontend_app_core](../ui/ai_ui_frontend_app_core.md) — contains `App.jsx` and global configuration.
