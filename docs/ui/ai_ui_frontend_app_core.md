...
All API calls from the app shell use `authFetch` from the [config](../core/config.md) module, which:
...
> **See also:** [config](../core/config.md) module for `authFetch` and `apiFetch` implementation details.

### 5.3 Access Control

User visibility of sidebar items is controlled by `ad_level` (Active Directory level) and `role`. The `Sidebar` component uses a `canSee(maxLevel)` function: items with `maxLevel >= user.ad_level` (or admin role) are shown. Some items also have server-side allowlist probes (e.g., `/broadcast/access`).
...
|---|---|---|
| `authFetch`, `apiFetch` | [config](../core/config.md) | Authenticated HTTP requests with retry and correlation IDs |
| `coworkOfficeClearKey` | [ai_ui_frontend_hooks](ai_ui_frontend_hooks.md) | Desktop bridge — clears OS-persisted CLI keys on logout |
| `Login` | [login](../auth/login.md) | Authentication screen with CAPTCHA and server-side session verification |
...
| `Projects` | [projects](../products/projects.md) | Project workspace (receives hoisted project state) |
...
| `/products` | `products` | `ProductManager` | [product_manager](../products/product_manager.md) |
| `/codebase` | `codebase` | `CodebaseManager` | [codebase_manager](../codebase/codebase_manager.md) |
| `/projects` | `projects` | `Projects` | [projects](../products/projects.md) |
...
| `/monitoring` | `monitoring` | `Monitoring` | [monitoring](../observability/monitoring.md) |
| `/analytics` | `analytics` | `AgentAnalytics` | [agent_analytics](../agents/agent_analytics.md) |
| `/evals` | `evals` | `EvalsDashboard` | [evals_dashboard](../observability/evals_dashboard.md) |
| `/model-governance` | `model-governance` | `ModelGovernance` | [model_governance](../llm/model_governance.md) |
| `/budget-manager` | `budget` | `BudgetManager` | [budget_manager](../llm/budget_manager.md) |
| `/level-overrides` | `level-overrides` | `LevelOverrides` | [level_overrides](../llm/level_overrides.md) |
| `/broadcast` | `broadcast` | `EmailBroadcast` | [email_broadcast](../connectors/email_broadcast.md) |
| `/endpoint-manager` | `endpoint-manager` | `EndpointManager` | [endpoint_manager](../products/endpoint_manager.md) |
| `/dept-metrics` | `dept-metrics` | `DeptMetrics` | [dept_metrics](../observability/dept_metrics.md) |
| `/memory` | `memory` | `Memory` | memory |
| `/connectors` | `connectors` | `Connectors` | [connectors](../connectors/connectors.md) |
| `/cowork-setup` | `cowork-setup` | `CoworkSettings` | [cowork_settings](../cowork/cowork_settings.md) |
| `/office` | `office` | `Office` (keep-alive) | — |
| `/code` | `cowork` | `Code` | [code](../codebase/code.md) |
| `/documents` | `docs` | `DocsPanel` | [documents](../documents/documents.md) |
| `/profile` | `profile` | `Profile` | [profile](../auth/profile.md) |
| `/coach` | `coach` | `Coach` | [coach](../coach/coach.md) |
| `/skill-proposals` | `skill-proposals` | `SkillProposals` | [skill_proposals](../skills/skill_proposals.md) |
...
> **See also:** [gateway](../core/gateway.md) module for the backend API gateway that serves these endpoints.
...
> **See also:** [desktop_app](../cowork/desktop_app.md) module for the Electron desktop application and IPC bridge.
