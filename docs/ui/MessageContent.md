...
Copy feedback is local state (`copied`) and resets after two seconds. The copy helper used here is shared with `ToolCallDetails` (see Utilities).
...
For details on the surrounding chat panel, see [ChatPanel](ChatPanel.md). For file download and attachment rendering, see [FileHandling](FileHandling.md). For the workflow editor canvas and node configuration, see [workflow_editor](../workflows/workflow_editor.md).
...
1. A user builds or runs a workflow in the [Workflow Editor](../workflows/workflow_editor.md).
2. Execution events stream from the backend through [`api_execution`](../api/api_execution.md) and into the frontend via [`ChatPanel`](ChatPanel.md).
...
- [workflow_editor](../workflows/workflow_editor.md) — the workflow editor feature that contains `ChatPanel`.
- workflowStore — Zustand store used by `ChatPanel` for run state and thread history.
- [api_execution](../api/api_execution.md) — backend API that streams workflow execution events consumed by the chat panel.
