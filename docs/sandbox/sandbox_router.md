...
| Layer | Mechanism |
|-------|-----------|
| **Authentication** | Only authenticated users can invoke `/sandbox/exec`. See auth/dependencies. |
| **Input validation** | Pydantic schema limits language choices and code size. |
| **Compliance pre-check** | `compliance_engine.check()` blocks PCI/PII/secrets before execution. See agents/compliance_engine. |
| **Rate limiting** | 30 executions/hour/user via Redis sliding window. |
| **Container isolation** | `DockerExecutor` uses `--network none`, 512 MB memory, 50% CPU quota, `no-new-privileges`, and auto-removes the container. See sandbox/docker_executor. |
...
- **sandbox/docker_executor** — the actual containerized/subprocess execution engine.
- **agents/compliance_engine** — the PCI/PII/secret scanning service used by the router and the executor.
- **auth/dependencies** — the shared authentication dependency used across all protected routers.
- **core/kv** — the Redis KV abstraction used for the sliding-window rate limiter.
- **[routers/admin_router](../api/admin_router.md)** — can reload compliance configuration that affects sandbox blocking behavior.
...
- auth/dependencies
- agents/compliance_engine
- sandbox/docker_executor
- core/kv
- [routers/admin_router](../api/admin_router.md)
