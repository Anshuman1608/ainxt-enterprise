...
|------------|--------|---------|
| `core/model_registry.py` | [core_infrastructure](../core/core_infrastructure.md) | Approved model IDs, display labels, feature flags, blocked models |
| `core/circuit_breaker.py` | [core_infrastructure](../core/core_infrastructure.md) | Per-provider circuit breakers |
| `core/logger.py` | [core_infrastructure](../core/core_infrastructure.md) | Structured logging with request ID context |
| `core/proxy_tool_use.py` | [core_infrastructure](../core/core_infrastructure.md) | LLM proxy headers |
| `models/classifier.py` | (sibling) | Complexity classification with regex + LLM fallback |
| `core/config.py` | [core_infrastructure](../core/core_infrastructure.md) | Redis cache config (`RDB_CACHE`) |
| `core/kv` | kv_store | Redis client for intent cache |
| `core/prompts.py` | [core_infrastructure](../core/core_infrastructure.md) | `INTENT_CLASSIFIER_PROMPT` template |
...
| Module | Description |
|--------|-------------|
| model_routing_retrieval | Hybrid search, metadata retrieval, local model inference |
| model_routing_knowledge_graph | Graph resolution and KB expansion |
| model_routing_versioning | KB version resolution |
| model_routing_document_intent | Document intent detection |
| [profiles](profiles.md) | Domain profiles, routing policy, context policy, response shaping |
| [router_policy](router_policy.md) | Model spec, route request, routing policy |
