...
This module is part of the broader ai_ui_frontend application and is closely related to the [auth](../auth/auth.md), [user_management](../auth/user_management.md), and [budget_manager](budget_manager.md) modules, all of which rely on AD level for access control and approval workflows.
...
|------------|--------|-------|
| `authFetch` | [config](../core/config.md) | Authenticated HTTP requests to the backend. |
| `useAuth` / `AuthContext` | [auth](../auth/auth.md) | Read current user's `ad_level` and `role` for access guard and level constraints. |
...
| `_require_director` | [auth_router](../auth/auth_router.md) | Backend guard ensuring only director-level users or admins can manage overrides. |
| `UserLevelOverride`, `User`, `OrgTree` | database | Persistence of overrides and restoration of original levels. |
...
For more details on how levels are enforced across the platform, see the [auth](../auth/auth.md) and [auth_router](../auth/auth_router.md) documentation. For the broader user-management surface, see [user_management](../auth/user_management.md).
