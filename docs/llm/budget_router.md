...
The **Budget Router** (`routers/budget_router.py`) is the central API layer for all budget management, allocation, utilization tracking, and cost chargeback operations in the AiNxt platform. It exposes a comprehensive set of FastAPI endpoints under the `/budget` tag that serve three distinct user personas — **end users** (self-service budget views), **HODs / reporting managers** (team oversight and budget-increase approvals), and **admins** (global allocation, HOD cap management, monthly resets).

The module implements a controlled allocation model where every user's cost budget is the sum of a **base allocation** ($50 default, configurable via the `BUDGET_BASE_COST_USD` env var) and an **extra allocation** (granted via approved HOD budget-increase requests). Both reset to defaults on the monthly reset cycle — nothing carries over. The only way a user's budget changes is the HOD approval flow.

### HOD-approval-hierarchy mode toggle (`HOD_APPROVAL_ENABLED`)

Everything in this document describes the router's behaviour when `core/config.py::HOD_APPROVAL_ENABLED=true` — a department/HOD hierarchy exists (`department_hod_mapping` populated) and is used to route budget-increase requests to the requester's Head of Department. This remains the byte-for-byte-unchanged behaviour for any deployment that opts in.

`HOD_APPROVAL_ENABLED` **defaults to `false`** — flat/admin-only mode, the out-of-the-box behaviour for self-hosted and flat-org deployments with no department/HOD hierarchy. In this mode, `store/budget_store.py::resolve_approvers_for_request()` routes every increase request to the pool of active admins instead of a department HOD (`_list_active_admin_emails()`), so:

- `POST /budget/request-increase` — 422 fallback message changes to *"No admin is available to approve this request. Contact your administrator."* when nobody is unroutable (i.e., no active admins exist).
- `POST /budget/requests/{id}/approve` / `.../reject` — the "primary actor" check becomes `is_admin(current_user)` instead of `is_hod(current_user)`, and `check_hod_cap=False` is passed to `approve_budget_request()` — flat-mode approvals have **no cap check at all**, purely admin judgment.
- `POST /budget/hod/self-increase` — also accepts an admin actor (`is_admin(current_user)`) in addition to an HOD actor. The admin branch applies the increase directly to `budget_configs` with **no `hod_allocation_ledger` row and no cap check** — it's audited via a logger-only `_audit_admin_action()` call instead of `hod_budget_governor.reserve_and_record()`.
- `GET`/`PUT /budget/admin/org-cap` (admin-only) — new endpoints, unrelated to `hod_allocation_caps`, that manage a single org-wide monthly spend cap (`ainxt.org_wide_budget_cap`, see [`services/org_budget_governor`](../workers/services.md)) used to gate SDLC-run and managed-endpoint cloud spend in flat mode. Budget-increase requests are **not** gated by this cap.
- The HOD-only endpoints below (`/budget/hod/cap-status`, `/budget/hod/direct-reports`, `/budget/hod/delegates`, `/budget/admin/hods*`, `/budget/admin/hod-audit`) are untouched code-wise but become unreachable in practice, since nobody has `is_hod=True` when the flag is off (Tier-1 suppression in `auth/dependencies.py::enrich_user_context()`).

See the plan/ADR for the full two-tier design. Existing HOD-hierarchy deployments must set `HOD_APPROVAL_ENABLED=true` on upgrade to keep this document's behaviour unchanged.
...
graph LR
    subgraph "User Total Budget"
        BASE["base_cost_usd<br/>$50 default"]
...
- **`BUDGET_ADMIN`** — applied to team/admin endpoints (`GET /budget/team`, `GET /budget/admin/hods`, `PUT /budget/admin/hods/{email}/cap`, `GET /budget/admin/hod-audit`, `GET /budget/my-increases`).
...
This is the core business workflow of the module — the only mechanism by which a user's budget grows.
...
2. **Cache invalidation**: `_invalidate_roster_cache()` deletes all `budget:users:roster:v1:*` keys after any mutating action (budget increase, usage reset).
...
| `_ROSTER_DEFAULT_BASE_COST_USD` | $50 (env: `BUDGET_BASE_COST_USD`) | Default base for unseeded roster rows; mirrors `store.budget_store.BASE_COST_USD` |
...
The `BudgetMiddleware` (see [middleware](../core/middleware.md) documentation) intercepts every LLM-generating request and calls `check_budget(user_id)` from `budget_store` before allowing the request through. If the user's cumulative spend has reached their `max_cost_usd_total`, the middleware returns a `429 BUDGET_EXCEEDED` response and publishes an inbox notification.
...
graph TB
    ...
    ADMIN --> A_EP["GET /budget/users<br/>GET /budget/summary<br/>GET /budget/admin/hods<br/>PUT /budget/admin/hods/{email}/cap<br/>GET /budget/requests"]
...
| Module | Usage |
|--------|-------|
| `store/budget_store` | Redis + Postgres budget/usage CRUD, increase request lifecycle |
| [`services/hod_budget_governor`](../workers/services.md) | HOD monthly cap enforcement and status |
| [`services/budget_audit_service`](../workers/services.md) | Monthly snapshot, reset, and pre-reset warnings |
| [`services/hierarchy_service`](../workers/services.md) | Org-tree subtree resolution for reporting managers |
| `store/inbox_store` | In-app notification publishing |
| [`services/smtp_service`](../workers/services.md) | HTML email delivery for HOD notifications |
| [`auth/rbac`](../auth/authentication.md) | Role checks (`is_admin`, `is_hod`), scope filters |
| [`auth/dependencies`](../auth/authentication.md) | `get_current_user` dependency |
| [`core/rate_limiter`](../core/core_infrastructure.md) | `enforce_rate_limit_with_behaviour`, `BUDGET_REQUEST`, `BUDGET_ADMIN` |
| [`core/security_validation`](../core/core_infrastructure.md) | `validate_budget_request` input sanitization |
| [`core/logger`](../core/core_infrastructure.md) | Structured logging |
| [`core/config`](../core/core_infrastructure.md) | `postgres_dsn()` |
| `db/database` | `SessionLocal` for SQLAlchemy sessions |
| `db/models` | `User`, `BudgetConfig`, `ModelRateTable` ORM models |
| [`middleware/budget_middleware`](../core/middleware.md) | Per-request budget enforcement (consumes `check_budget`) |
| [`workers/start_workers`](../workers/worker_orchestration.md) | Cron threads for monthly reset and warning emails |
...
| **SMTP Relay** | Email delivery for HOD approval notifications, pre-reset warnings |
