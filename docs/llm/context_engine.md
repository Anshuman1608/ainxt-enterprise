...
- Manage durable cross-session memory (see memory_system).
...
See memory_system for details on how `chat_summarizer` maintains per-chat rolling summaries and decides what deserves long-term memory.
...
- memory_system — rolling chat summaries, `distill_turn`, and long-term memory decisions.
