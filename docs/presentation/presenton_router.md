...
PPT Wizard and the chat-driven PPT intent flow.
...
`presenton_worker` running on the document queue.
...
| **Status polling** | `presentation_status` reads `ppt:result:{job_id}` from the stream KV (Redis, selected via `RDB_STREAM`) and returns `processing` / `done` / `error` with download metadata and a cost breakdown. |
...
The outline prompt mandates a minimum number of chart slides and KPI stat
slides, requires real figures matched to the requested subject, and constrains
bullets to ≤14 words. Domain, units and currency follow the user's own prompt —
an earlier version appended a fixed Indian-financial context instruction, which
steered every deck towards payments framing regardless of the subject. The response schema includes an optional
...
shared_core → model_routing. The response is cleaned of
...
Reads `ppt:result:{job_id}` from the stream KV (`RDB_STREAM` → Redis DB6).
...
| Dependency | Usage |
|---|---|
| [`auth.dependencies`](../core/shared_core.md) → `get_current_user` | JWT/session authentication on every endpoint |
| [`agents.compliance_engine`](../core/shared_core.md) → `compliance_engine` | Input validation / blocking before LLM and Presenton calls |
| [`core.config`](../core/shared_core.md) → `PRESENTON_URL`, `PRESENTON_USER`, `PRESENTON_PASSWORD`, `PPT_LLM_MODEL`, `RDB_STREAM` | Service URLs, credentials, KV database selector |
| [`core.job_queue`](../core/shared_core.md) → `enqueue_job`, `Q_DOC` | Async job dispatch to the document worker lane |
| [`core.kv`](../core/shared_core.md) → `get_kv` | Stream KV access for result polling and download path resolution |
| [`core.logger`](../core/shared_core.md) → `logger` | Structured logging |
| [`core.prompt_sanitizer`](../core/shared_core.md) → `sanitize` | Prompt sanitisation before LLM calls and job enqueue |
| [`models.model_router`](../core/shared_core.md) → `model_router` | LLM generation with Claude → GPT → Ollama fallback chain |
| [`workers.presenton_worker`](../workers/workers.md) → `generate_ppt_job`, `_fail` | Async worker that calls Presenton and publishes results |
| [`tools.doc_generator`](../skills/shared_integrations.md) → `smart_filename` | Filename generation from prompt (used by the worker) |
...
| **Stream KV** (Redis DB6) | `ppt:result:{job_id}` | Job result storage with TTL |
...
| `RDB_STREAM` | — | KV database selector (Redis DB6) |
