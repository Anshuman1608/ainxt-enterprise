...
def main() -> int:
    ...
    try:
        ...
        # Auto-discover this provider's real model catalog FIRST — a fresh
        # install should be immediately usable in chat without the admin
        # having to open the "LLM Providers" screen and click "Sync" by hand
        # first. Cloud families (anthropic/openai/gemini) only do this once,
        # when the provider has no models yet at all — never overwrites/
        # duplicates an admin's already-curated list on a re-run of
        # install.sh. Ollama's discovery only ever ADDS models it finds
        # already pulled that aren't registered yet (never removes/
        # overwrites anything), so it's safe to attempt every run.
        existing_model_ids = {
        ...
        # --seed-model is checked AGAINST the discovery results above, not
        # inserted blindly — it used to add e.g. bare "llama3.2" as an
        # available model whether or not that exact tag was ever actually
        # pulled (Ollama tags are versioned, e.g. "llama3.2:3b"; install.sh
        # passes the bare suggestion name from
        # config/ollama_model_suggestions.json, which frequently isn't the
        # real tag). That left a phantom, never-callable model sitting in
        # the picker next to the real discovered ones — confirmed live: a
        # host Ollama with exactly 2 real models pulled showed 3 entries.
        # Only add it if discovery (or a prior run) actually confirms it, or
        # if discovery wasn't run at all (family has no live catalog to
        # check against, e.g. a plain openai_compatible endpoint) — in that
        # case there's nothing to validate against, so keep the old
        # trust-the-caller behavior.
        if args.seed_model:
            if args.seed_model in existing_model_ids:
                pass  # already present (pre-existing, or just discovered above)
            elif args.family == "ollama":
