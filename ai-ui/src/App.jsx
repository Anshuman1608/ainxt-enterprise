...
// AdminRoute — guards the "Observe" pages (Monitoring, Analytics, Eval
// Observatory) so a non-admin who navigates directly to the URL (bypassing
// the sidebar, which already hides these links) is redirected to /chat
// instead of reaching the page. This is a UX-level gate only — the real
// enforcement is server-side (see routers/evals_router.py, memory_router.py
// analytics endpoints, and jobs_router.py's queue-stats endpoint, all of
// which require role="admin").
...
export default function App() {
  ...
  // Set whenever /auth/me reports is_temp_password:true — a valid session
  // exists (the temp password from the forgot-password flow IS a real
  // password), but the user must set their own before entering the app.
  // Checked on every mount, not just right after login, so refreshing the
  // page mid-flow cannot be used to skip the reset.
  ...
  //
  // `useRef(expr)` only STORES `expr` once, but React evaluates the
  // argument expression on EVERY render regardless — so the old
  // `useRef(apiFetch(...))` fired a brand-new fetch on every single
  // re-render of App, not just on mount. App holds chat/streaming state
  // (chats/projectMessages below), so a streamed local-model response
  // updating on every token re-rendered App hundreds of times over one
  // reply — confirmed live: one chat message generated 372 calls to
  // GET /auth/ui-config, which (combined with everything else on the page)
  // was enough on its own to trip the global per-user rate limit. The
  // lazy-init-inside-render check below (`current === undefined`) is the
  // standard way to run an expensive initializer through useRef exactly
  // once, since only the FIRST render's assignment actually takes effect.
  const _piiEnabledPromise = useRef();
  if (_piiEnabledPromise.current === undefined) {
    _piiEnabledPromise.current = apiFetch(`${API}/auth/ui-config`)
  ...
  useEffect(() => {
    ...
    authFetch(`${API}/auth/me`)
      .then(r => r.ok ? r.json() : null)
      .then(async data => {
        if (data && data.is_temp_password) {
          // Valid session, but a temp password is still active — hold on the
          // login screen's forced reset view instead of granting entry.
  ...
  function handleLogout() {
    ...
    setUser(null);
    // BUG FIX: forcePasswordReset is only ever set by the mount-time /auth/me
    // check (once, on page load) and only ever cleared by a completed reset.
    // Without clearing it here too, logging out of a temp-password account
    // WITHOUT finishing the reset left it stuck true — so <Login/> remounts,
    // its effect sees the stale flag, and forces the "set new password"
    // screen on the very next login attempt, even a completely normal one
  ...
  if (!user) {
    return (
      <Login
        ...
        onPasswordResetComplete={() => setForcePasswordReset(false)}
      />
    );
  }
    ...
    <div className="flex h-screen w-screen overflow-hidden bg-white">
      ...
      <div className="flex flex-col flex-1 min-w-0 h-full overflow-hidden">
        <Routes>
          ...
          <Route path="/llm-provider-config" element={
