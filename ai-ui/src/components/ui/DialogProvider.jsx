...
export function ToastProvider({ children }) {
  ...
  // Memoized on [push] (itself stable — see its own useCallback deps above),
  // so `toast` keeps ONE identity for the life of the app. Without this, a
  // fresh object literal every render meant any consumer with `toast` in a
  // useEffect/useCallback dependency array (e.g. LLMProviderConfig's
  // loadProviders) re-triggered on every render — including renders caused
  // by ToastProvider's OWN state update when a toast fires. A single failed
  // fetch became: toast.error() -> re-render -> new `toast` identity -> the
  // effect depending on it fires again -> fetch fails again -> repeat, as
  // fast as React would render — confirmed live as the source of dozens of
  // stacked "Failed to load providers: HTTP 429" toasts from one 429.
  const toast = useMemo(() => ({
