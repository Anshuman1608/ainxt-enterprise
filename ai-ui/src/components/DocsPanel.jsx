...
const MODULES = [
  ...
  },
  {
    ...
    icon: Database,
    label: "Codebase",
    what: "Connect and index repositories so Chat and Agents can retrieve from them.",
  ...
  },
  {
    ...
    icon: BookMarked,
    label: "CodeWiki",
    what: "Generated, browsable documentation for any git repository you point it at — a module tree with a page per significant component. Independent of Codebase indexing; the repo doesn't need to be indexed there first.",
    ...
    when: "Right after cloning a repository, and before your first deep question about it.",
    useCases: [
      { dept: "Engineering", icon: Cpu, examples: [
  ...
  },
  {
    ...
    icon: Server,
    label: "LLM Providers",
    what: "Register the LLM providers the platform can call — Anthropic, OpenAI, Gemini, an OpenAI-compatible endpoint, or a self-hosted Ollama instance — with their API key, base URL and available models.",
    why: "Model routing has to resolve to a real, reachable provider. This is where that provider is added, tested and enabled before governance or budget rules ever apply to it.",
    who: "Administrators.",
    when: "During setup, when adding a new provider, or when rotating a credential.",
    useCases: [
      { dept: "Engineering", icon: Cpu, examples: [
        "Add a self-hosted Ollama endpoint and enable a specific local model.",
        "Test a provider's connection after rotating its API key.",
      ]},
      { dept: "Risk", icon: AlertTriangle, examples: [
        "Disable a provider outright rather than relying on downstream policy to block it.",
      ]},
    ],
  },
  {
