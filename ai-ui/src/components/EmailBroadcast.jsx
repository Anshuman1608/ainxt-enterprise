...
export default function EmailBroadcast({ user }) {
  ...
  // re-render. The lazy-init check below (`current === undefined`) runs the
  // fetch exactly once, since only the first render's assignment takes effect.
  const piiEnabledPromise = useRef();
  if (piiEnabledPromise.current === undefined) {
    piiEnabledPromise.current = apiFetch(`${API_BASE}/auth/ui-config`)
...
function BroadcastDetail({ broadcastId, onBack }) {
  ...
  // for the full explanation (and why this is lazy-initialized rather than
  // passed straight to useRef). Fetched once per BroadcastDetail mount.
  const piiEnabledPromise = useRef();
  if (piiEnabledPromise.current === undefined) {
    piiEnabledPromise.current = apiFetch(`${API_BASE}/auth/ui-config`)
      .then((r) => (r.ok ? r.json() : null))
