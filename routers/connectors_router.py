...
# ── PAT-based connector connect (GitLab, GitHub, Jira) ───────────────────────
# Maps connector name → profile token_type stored in user_tokens table.
# Kept in sync with connectors/engine.py:_PAT_TOKEN_TYPE_MAP (the Buddy
# auto-connect path) — both must know the same connectors, or a PAT connects
# from one surface and silently not the other.
...
def _jira_site_base(user_id: str) -> str:
    """Resolve the Jira site base URL for this user: a per-user 'jira_url' entry
    in Profile → API Token Vault wins; otherwise the gateway-wide JIRA_URL env
    (single-site deployments). Returns "" when neither is set. `normalize_jira_site`
    collapses whatever was pasted (bare host / site root / API URL / full browser
    URL) to scheme://host."""
    from routers.profile_router import get_decrypted_token
    from tools.jira_tools import normalize_jira_site
    return normalize_jira_site(
        get_decrypted_token(user_id, "jira_url") or os.getenv("JIRA_URL", "")
    )
...
async def pat_connect(
    ...
    if not raw:
        ...
        }.get(token_type, "Atlassian API Token")
        return JSONResponse(
            status_code=428,
            content={
    ...
    if connector_name == "gitlab":
        ...
        # GitHub PATs are bare tokens (ghp_… / github_pat_… / gho_…). The Profile
        # UI may still store a "user:token" pair for symmetry with GitLab — strip
        # any such prefix. Mirrors connectors/engine.py:_try_auto_connect_pat's
        # github branch (Bearer auth against api.github.com).
        ...
        jira_url = _jira_site_base(user_id)
        metadata = {
            "auth_type": "pat",
            "pat_header": "Authorization",
            "pat_scheme": "Basic",
...
def _ui_redirect(connector_name: str, success: bool, error: str = "") -> str:
    ...
    #
    # Includes the /portal prefix because ai-ui's production build sets
    # base: '/portal/' (vite.config.js) and nginx.conf only serves the SPA
    # under /portal/* — a bare /connectors matches no nginx location block
    # and 404s whenever the non-popup fallback (window.location.replace, or
    # the <meta refresh> in _oauth_complete()) fires, e.g. when the OAuth
    # provider page opens as a plain tab rather than a script-driven popup
    # with a live window.opener.
    if success:
        return f"/portal/connectors?connected={urllib.parse.quote(connector_name)}"
    err_enc = urllib.parse.quote(error[:100]) if error else "unknown"
    return f"/portal/connectors?error={err_enc}&connector={urllib.parse.quote(connector_name)}"
...
<html>
  ...
  <body>
    ...
    <script>
      (function () {{
        // window.close() doesn't require window.opener to exist — only that
        // this window was opened via script, which is always true here. The
        // connect flow opens the OAuth provider page with `noopener`, so
        // window.opener is always null and the postMessage branch can never
        // run; always attempt to close instead of gating on it, with a
        // redirect fallback if the browser refuses. The original
        // window/tab's own status-polling loop is the actual source of
        // truth for the connection, independent of this page.
        var payload = {payload_json};
        try {{
          if (window.opener && !window.opener.closed) {{
            window.opener.postMessage(payload, window.location.origin);
          }}
        }} catch (e) {{ /* cross-origin or no opener — fine, ignore */ }}
        setTimeout(function () {{
          try {{ window.close(); }} catch (e) {{ /* some engines refuse — fall through to redirect below */ }}
          setTimeout(function () {{ window.location.replace({target_json}); }}, 400);
        }}, 600);
      }})();
    </script>
  </body>
