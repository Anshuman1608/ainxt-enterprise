...
async def _stream_claude(
    ...
    if proxy_url:
        # ARCH-F-CORE-003: fast-fail if the Claude proxy link is already known
        # to be down, rather than paying a full 30s connect timeout on every
        ...
        lines = _claude_lines_via_proxy(proxy_url, payload, _proxy_headers, _breaker, timings)
    else:
        # Direct mode — no LLM_PROXY_URL configured. Call Anthropic directly
        # via the already-initialized root ClaudeGateway singleton instead of
        # forwarding to a proxy that doesn't exist. See _claude_lines_direct().
        lines = _claude_lines_direct(payload, timings)

    async for sse in _claude_lines_to_anthropic_sse(
            lines, req, model_hint, user_id, timings, source_channel, request_id,
    ):
        yield sse


async def _claude_lines_via_proxy(
        proxy_url: str,
        payload: dict,
    ...
    """Fetch Claude tool-use NDJSON lines from the LLM proxy (services/llm_proxy).
    ...
    Transport-level failures are surfaced as a single `{"error": ...}` line so
    the shared _claude_lines_to_anthropic_sse() consumer handles them exactly
    like a mid-stream provider error, instead of duplicating SSE-emission
    logic here.
    """
    ...
    for _attempt in range(2):
        try:
            ...
            async with client.stream("POST", f"{proxy_url}/llm/claude-tools-stream", json=payload, headers=headers) as resp:
            ...
            yield json.dumps({"error": str(e), "error_type": "api_error"})
            return
        except Exception as e:
...
async def _claude_lines_direct(
        payload: dict,
        timings: Optional[dict[str, float]] = None,
) -> AsyncGenerator[str, None]:
    """Direct-mode Claude tool-use stream — used when LLM_PROXY_URL is unset.

    Calls Anthropic directly via the already-initialized root ClaudeGateway
    singleton (gateway_claude.claude_gateway — the same credentials/singleton
    models/model_router.py already falls back to when no proxy is configured).
    Its SDK client is synchronous, so the call runs in a background thread and
    is bridged to this async generator via an asyncio.Queue — the same bridge
    pattern services/llm_proxy/main.py already uses for its OpenAI/Gemini
    tools-stream endpoints. Emits the same tbs/tad/txt/stop/error NDJSON-line
    shape the LLM-proxy path produces (mirroring services/llm_proxy/main.py's
    claude_tools_stream), so no provider-specific logic is needed downstream.
    """
    ...
    def _put(line: str) -> None:
        ...
        try:
            ...
            logger.warning(f"[messages-compat/claude-direct] queue put failed: {e}")

    def _worker() -> None:
        in_tok = out_tok = cache_read_tok = cache_creation_tok = 0
        try:
            if claude_gateway is None:
                raise RuntimeError("Claude gateway not available")
            stream_kwargs: dict = {
                "model":      payload["model"],
                "max_tokens": payload["max_tokens"],
                "system":     [{"type": "text", "text": payload["system"]}],
                "messages":   payload["messages"],
            }
            if payload.get("tools"):
                stream_kwargs["tools"] = payload["tools"]
            with claude_gateway.client.messages.stream(**stream_kwargs) as stream:
                for ev in stream:
                    etype = getattr(ev, "type", "")

                    if etype == "message_start":
                        ms_u = getattr(getattr(ev, "message", None), "usage", None)
                        if ms_u:
                            in_tok             = getattr(ms_u, "input_tokens", 0) or 0
                            cache_read_tok     = getattr(ms_u, "cache_read_input_tokens", 0) or 0
                            cache_creation_tok = getattr(ms_u, "cache_creation_input_tokens", 0) or 0

                    elif etype == "content_block_start":
                        cb = getattr(ev, "content_block", None)
                        if cb and getattr(cb, "type", "") == "tool_use":
                            idx = getattr(ev, "index", 0)
                            _put(json.dumps({"tbs": {"index": idx, "id": cb.id, "name": cb.name}}))
                        ...
                        if delta:
                            if getattr(delta, "type", "") == "text_delta":
                                _put(json.dumps({"txt": {"text": delta.text}}))
                            elif getattr(delta, "type", "") == "input_json_delta":
                                _put(json.dumps({"tad": {"index": idx, "partial_json": delta.partial_json}}))
            ...
            logger.error(f"[messages-compat/claude-direct] error: {exc}", exc_info=True)
            ...
            try:
                ...
                pass
            _put(json.dumps({"error": str(exc), "status_code": _status_code, "error_type": _err_type}))
        finally:
            _put(None)  # sentinel
...
async def _claude_lines_to_anthropic_sse(
    ...
    """Parse Claude tools-stream NDJSON lines (from either the proxy or direct
    mode) into Anthropic SSE events, mirroring the _oai_chunks_to_anthropic_sse
    convention below for the OpenAI/Gemini path."""
    block_idx     = 0
    in_text_block = False
    in_tool_block = False
    in_tok = out_tok = 0
...
async def _stream_oai_format(
    ...
    if not proxy_url:
        # Direct mode — no LLM_PROXY_URL configured. Call the provider
        # directly via the already-initialized root gateway singleton instead
        # of forwarding to a proxy that doesn't exist. See _oai_lines_direct()
        # / _gemini_lines_direct().
        lines = (
            _oai_lines_direct(model_hint, oai_msgs, oai_tools, req.max_tokens)
            if provider == "openai"
            else _gemini_lines_direct(model_hint, oai_msgs, oai_tools, req.max_tokens)
        )
        async for sse in _oai_chunks_to_anthropic_sse(
                lines, req, model_hint, user_id, timings, source_channel=source_channel,
        ):
            yield sse
        return
...
# ── Direct-mode (no LLM_PROXY_URL): OpenAI / Gemini tool-streaming ───────────
# Used only when LLM_PROXY_URL is unset. Both providers' SDK clients on the
# already-initialized root gateway singletons (gateway_openai.openai_gateway,
# gateway_gemini.gemini_gateway) are synchronous, so each call runs in a
# background thread and is bridged to an async generator via an asyncio.Queue
# — the same bridge pattern services/llm_proxy/main.py already uses for its
# own /llm/openai-tools-stream and /llm/gemini-tools-stream endpoints. Both
# emit the same OpenAI-format NDJSON chunk lines the proxy path already
# produces, so they plug straight into the existing, unmodified
# _oai_chunks_to_anthropic_sse() consumer above.

async def _oai_lines_direct(
        model: str,
        oai_msgs: list[dict],
        oai_tools: Optional[list],
        max_tokens: int,
) -> AsyncGenerator[str, None]:
    """Direct-mode OpenAI tool-use stream. Near-verbatim port of
    services/llm_proxy/main.py's _run_oai_tools_stream, swapped onto the root
    OpenAIGateway singleton."""
    from gateway_openai import openai_gateway
    ...
    def _put(item: str) -> None:
        ...
        try:
            ...
            logger.warning(f"[messages-compat/openai-direct] queue put failed: {e}")
    ...
    def _worker() -> None:
        try:
            ...
            if oai_tools:
                ...
                # Suppress reasoning_effort when tools are present — the OpenAI
                # API rejects the parameter alongside tool calls/results.
                kwargs["reasoning_effort"] = "none"
            if max_tokens:
                kwargs["max_completion_tokens"] = max_tokens

            response = openai_gateway.client.chat.completions.create(**kwargs)
            ...
            logger.error(f"[messages-compat/openai-direct] error: {exc}", exc_info=True)
            err_chunk = {
                "choices": [{
                    "index": 0,
...
# overflow so memory stays bounded regardless of conversation volume. Mirrors
# services/llm_proxy/main.py's cache of the same name — duplicated rather than
# imported so this process never has to load services/llm_proxy's isolated
# `core` package (its module-level sys.path patch is meant for a standalone
# uvicorn entrypoint, not for import into this process).
_THOUGHT_SIG_CACHE: "OrderedDict[str, bytes]" = OrderedDict()
_THOUGHT_SIG_CACHE_LOCK = threading.Lock()
...
def _oai_msgs_to_gemini(messages: list):
    ...
    The Anthropic SDK that fronts this router strips unknown fields from
    content_block, so the signature cannot round-trip through the client.
    Resolution: when we don't have a signature for an assistant tool_call,
    demote BOTH the assistant function_call AND the paired user
    ...
    Verbatim port of services/llm_proxy/main.py's helper of the same name.
    """
    ...
    for msg in messages:
        ...
        for tc in (msg.get("tool_calls") or []):
            ...
            sig_b64 = fn.get(GEMINI_THOUGHT_SIG_KEY)
            if sig_b64:
                try:
            ...
            for tc in (msg.get("tool_calls") or []):
                ...
                if tc_id in demoted_ids:
                    ...
                    parts.append(_gt.Part(text=(
                        ...
                        f"with arguments {json.dumps(args)} and received the "
...
async def _gemini_lines_direct(
        model: str,
        oai_msgs: list[dict],
        oai_tools: Optional[list],
        max_tokens: int,
) -> AsyncGenerator[str, None]:
    """Direct-mode Gemini tool-use stream. Near-verbatim port of
    services/llm_proxy/main.py's _run_gemini_tools_stream, swapped onto the
    root GeminiGateway singleton. Converts OpenAI-format messages/tools to
    Gemini's native format, then re-emits OpenAI-format ChatCompletionChunk
    NDJSON lines so it plugs into the same _oai_chunks_to_anthropic_sse()
    consumer already used for the OpenAI path."""
    ...
    def _put(item: str) -> None:
        ...
        try:
            ...
            logger.warning(f"[messages-compat/gemini-direct] queue put failed: {e}")
    ...
    def _worker() -> None:
        ...
        def _oai_chunk(delta: dict, finish=None) -> str:
            ...
            return json.dumps({
                ...
                "created": created_ts, "model": model, "choices": [choice],
            }) + "\n"
        ...
        try:
            ...
            contents    = _oai_msgs_to_gemini(oai_msgs)
            gemini_tool = _oai_tools_to_gemini(oai_tools) if oai_tools else None
            ...
            if gemini_tool:
                ...
                # Disable thinking when tools are active — see the identical
                # comment in services/llm_proxy/main.py's _run_gemini_tools_stream
                # for why (thought_signature round-trip through this SDK boundary).
                try:
            ...
            fn_index  = 0
            last_kind = None  # "text" | "fncall"
            for chunk in gemini_gateway.client.models.generate_content_stream(
                    model=model,
                    contents=contents,
                ...
                try:
                    if hasattr(chunk, "usage_metadata") and chunk.usage_metadata:
                        ...
                        prompt_tok = getattr(um, "prompt_token_count", 0) or prompt_tok
                        output_tok = getattr(um, "candidates_token_count", 0) or output_tok
                except Exception:
                    pass
                ...
                for part in parts:
                    ...
                    if txt:
                        ...
                        start_fn: dict = {"name": fc.name, "arguments": ""}
                        sig = getattr(part, "thought_signature", None)
                        if sig:
                            _thought_sig_put(tool_id, sig)
                            start_fn[GEMINI_THOUGHT_SIG_KEY] = _b64.b64encode(sig).decode("ascii")
                        _put(_oai_chunk({"tool_calls": [{
                            "index": fn_index, "id": tool_id, "type": "function",
                            "function": start_fn,
                        }]}))
                        _put(_oai_chunk({"tool_calls": [{
                            "index": fn_index,
            ...
            finish_reason = "tool_calls" if last_kind == "fncall" else "stop"
            if last_kind is None:
                _put(_oai_chunk({
                    "content": "[Gemini returned no visible content. "
                               "Try raising max_tokens or rephrasing the request.]",
                }))
            if prompt_tok or output_tok:
                _put(json.dumps({
                    "id": completion_id, "object": "chat.completion.chunk",
                    "created": created_ts, "model": model, "choices": [],
                    "usage": {
            ...
            logger.error(f"[messages-compat/gemini-direct] error: {exc}", exc_info=True)
...
async def list_models_compat(request: Request):
    ...
    # Attach dynamically discovered in-house models not already present.
