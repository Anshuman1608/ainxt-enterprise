...
def _cloud_stream_response(
    ...
    Two dispatch paths, sharing the same billing/gating wrapper below:
      - `tools` present  -> services.cloud_tool_stream.stream_cloud_tools when
        LLM_PROXY_URL is configured (the shared client also used by
        gateway.py's IDE route), else services.managed_endpoint_direct_llm
        .direct_stream_cloud_tools — a managed-endpoint-only direct-to-provider
        fallback that does NOT affect the IDE route's own behavior.
    ...
    try:
        ...
        if tools:
            from services.endpoint_model_catalog import provider_of
            from services.managed_endpoint_direct_llm import llm_proxy_configured

            provider = provider_of(model)
            # Managed endpoints only: when LLM_PROXY_URL isn't configured,
            # call the provider directly instead of hard-failing — see
            # services/managed_endpoint_direct_llm.py's module docstring for
            # why this is a separate module rather than a change to the
            # shared services.cloud_tool_stream (also used by gateway.py's
            # IDE/Kilo-Code tool-calling routes, which are deliberately left
            # with their existing LLM_PROXY_URL-required behavior).
            if llm_proxy_configured():
                from services.cloud_tool_stream import stream_cloud_tools as _tool_stream_fn
            else:
                from services.managed_endpoint_direct_llm import direct_stream_cloud_tools as _tool_stream_fn
            for tok in _tool_stream_fn(
