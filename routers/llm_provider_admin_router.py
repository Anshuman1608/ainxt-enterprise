...
# Ollama has no single well-known catalog to filter by name prefix the way
# OpenAI's can (anyone can `ollama pull` anything, under any tag) — but every
# embedding model this platform itself configures/suggests
# (nomic-embed-text, mxbai-embed-large, snowflake-arctic-embed, ...) has
# "embed" in its name, matching the wider Ollama library convention too.
# Without this, discovery/sync listed the Knowledge Base's OWN embedding
# model in the chat picker right next to real chat models — confirmed live:
# a host Ollama with one chat model and one embedding model pulled showed
# both as selectable chat models.
_OLLAMA_NON_CHAT_SUBSTRINGS = ("embed",)
