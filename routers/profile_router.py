...
# "jira_url" is not a secret — it's the user's Jira site URL
# (https://org.atlassian.net), stored here so the Jira connector can be
# per-user/per-site without a gateway-wide JIRA_URL env. It rides the same
# encrypted vault path as the tokens (over-encryption, harmless) and is read
# by connectors_router._jira_site_base() / engine._try_auto_connect_pat().
TOKEN_TYPES = {"local_llm", "atlassian", "gitlab", "github", "jira_url"}
...
class TokenUpsert(BaseModel):
    ...
    token_type: Literal["local_llm", "atlassian", "gitlab", "github", "jira_url"]
    value:      str
