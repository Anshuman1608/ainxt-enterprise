...
# ── Google sign-in ───────────────────────────────────────────────────────────
# ENABLE_GOOGLE_LOGIN=false (default) — no "Continue with Google" button, and
#                           /auth/google/* returns 404. An existing deployment
#                           that upgrades without touching .env is unaffected.
# ENABLE_GOOGLE_LOGIN=true  — shows the button on both the sign-in and the
#                           sign-up view. The OAuth client and Google endpoints
#                           are read from the `gmail` connector definition, so
#                           GOOGLE_CLIENT_ID / GOOGLE_CLIENT_SECRET must be set
#                           and the login callback URL must be registered as an
#                           additional redirect URI on that same OAuth client.
#                           See auth/google_auth.py.
#
# Read live via os.getenv() inside auth/google_auth.py rather than captured
# here, so the flag can be flipped without a code change; this constant exists
# for callers that want the boolean at import time.
ENABLE_GOOGLE_LOGIN: bool = os.getenv("ENABLE_GOOGLE_LOGIN", "false").lower() == "true"

# ── Enterprise org-chart / HOD approval hierarchy kill-switch ────────────────
# HOD_APPROVAL_ENABLED=true  — enterprise mode, unchanged behavior.
#                             Domain/department pickers (Knowledge Base scope
#                             picker, Product creation) list real org_tree
#                             departments, synced from an AD/HR CSV. Approval
#                             routing can reach a department's Head of
#                             Department via department_hod_mapping.
# HOD_APPROVAL_ENABLED=false — individual/OSS mode: no org chart, no HOD to
#                             route to. Both department pickers collapse to
#                             the single fixed value "USER", and newly
#                             self-registered users get department="USER"
#                             automatically so they can see their own
#                             products/KB docs. Manual .env toggle, set after
#                             install (not an ./install.sh prompt).
#
# Defaults to FALSE — flat/admin-only mode ("user raises, admin approves") for
# every subsystem that historically required a department HOD mapping: budget
# increases, SDLC run budget gating, managed-endpoint cloud spend, product
# catalog submission, workflow/skill/agent governance promotion, and the
# monthly HOD digest. A fresh install has an empty org_tree, so defaulting to
# true would leave every department picker blank.
#
# Organisations that maintain a real org chart (department_hod_mapping,
# users.hod_email) opt in with HOD_APPROVAL_ENABLED=true.
#
# This constant used to be assigned twice in this file — "true" here and
# "false" in the feature-router block below — with the later assignment
# silently winning. Consolidated to this single read; false preserves the
# behaviour that was already in effect.
HOD_APPROVAL_ENABLED: bool = os.getenv("HOD_APPROVAL_ENABLED", "false").strip().lower() == "true"
...
# ── Forgot password — temp password delivery when SMTP is not configured ────
# SHOW_TEMP_PASSWORD_IN_UI=true  (default) — when SMTP is not configured (the
#                          OSS default — see the "primary path, not a
#                          fallback" comment on forgot_password() in
#                          routers/auth_router.py), the temp password is
#                          returned in the API response and the login page
#                          shows it directly. Without this, a single-admin
#                          OSS install with no SMTP has no way to complete a
#                          password reset except reading gateway container
#                          logs — a poor default for the common case.
# SHOW_TEMP_PASSWORD_IN_UI=false — the SMTP-less fallback prints the temp
#                          password to the server console ONLY (never in the
#                          API response); the login page tells the user to
#                          ask an administrator to check the logs.
#
# SECURITY: POST /auth/forgot-password is UNAUTHENTICATED — it accepts only an
# email address. With this flag on (the default), anyone who can reach that
# endpoint and knows (or guesses) a registered email obtains a working
# password for that account, with no proof of ownership. Set this to false on
# any deployment reachable by untrusted users — a public-facing or multi-
# tenant install — where you are not the only one who can reach the login
# page. This never applies when SMTP is configured: email delivery, when
# available, is always used instead and is never accompanied by this.
SHOW_TEMP_PASSWORD_IN_UI: bool = os.getenv("SHOW_TEMP_PASSWORD_IN_UI", "true").lower() == "true"
...
# Embedding provider — which backend embed_svc's /embed calls actually use.
# Always routed through the embed svc at EMBED_SVC_URL; this only picks
# which of embed_svc's own backends (see services/embed_svc/embedder.py)
# handles the request:
#   ollama (default) — local, via OLLAMA_URL, no API key needed.
#   openai            — text-embedding-3-small via OPENAI_API_KEY.
#   nomic             — any OpenAI-compatible /v1/embeddings endpoint
#                       (self-hosted or otherwise), via NOMIC_EMBED_URL.
#
# Read by workers/index_worker.py (index-time embedding) AND
# models/hybrid_search.py::pgvector_search (query-time embedding for
# retrieval) — the SAME constant, deliberately, so the two can never drift
# apart. Changing this after repos are already indexed requires re-indexing
# them: a different provider/model is not vector-comparable with what's
...
# HOD_APPROVAL_ENABLED — the master toggle for the department/Head-of-Department
# approval hierarchy — is defined once, near the top of this file. It used to be
# re-read here with a different default, which silently shadowed it.
...
def validate_prod_config() -> None:
    ...
    if not os.getenv("FERNET_KEY") and not os.getenv("VAULT_ENCRYPTION_KEY"):
        hard_errors.append(
...
def startup_config_check() -> None:
    ...
    if _kafka_enabled:
        ...
        lines.append("  ❌  Kafka           : DISABLED (KAFKA_ENABLED=false) — chat history, audit log")
        lines.append("                        and usage rows will NOT reach Postgres. Events queue to")
        lines.append("                        Redis kafka:fallback:* with a 7-day TTL and are written")
        lines.append("                        only when workers/kafka_consumer.py next starts.")
...
#
# DEFAULT_AD_LEVEL=6 — most junior. Directory sync (if enabled) sets
#                     the real level; the registration default is a safe
#                     fallback that prevents accidental privilege escalation.
DEFAULT_AD_LEVEL: int = int(os.getenv("DEFAULT_AD_LEVEL", "6"))
...
# gigabytes  the warm 300s budget kills
