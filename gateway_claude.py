...
class ClaudeGateway:
    ...
    def __init__(self):
        ...
        if not api_key:
            # Fallback: an admin-configured Anthropic provider with no .env key
            # at all (see core.llm_provider_registry). No-op when the env var
            # is already set — the common case is unaffected.
            try:
                from core.llm_provider_registry import resolve_credential_for_family
                api_key = resolve_credential_for_family("anthropic")
            except Exception:
                pass
    ...
    def generate(
        ...
        try:
            ...
            #
            # Callers upstream (e.g. gateway.py's local-model KV-cache-hoisting
            # optimization, _LOCAL_KV_CACHE_HOIST) may prepend a
            # {"role": "system", ...} message at position 0 — a format built for
            # OpenAI-style/local chat APIs. Anthropic's Messages API rejects a
            # role="system" entry as messages[0] (400 invalid_request_error: "use
            # the top-level 'system' parameter for the initial system prompt");
            # it only accepts role="system" at non-initial positions (the
            # mid-conversation system message feature). Hoist any LEADING
            # system-role message(s) into the top-level `system` param instead of
            # forwarding them as-is — this is the only place messages_payload is
            # built for generate(), so it fixes every dispatch path (streaming,
            # blocking, and any provider fallback) through this gateway.
            system_prompt = None
            if isinstance(prompt, list):
                _msgs = list(prompt)
                _leading_system = []
                while _msgs and _msgs[0].get("role") == "system":
                    _leading_system.append(_sanitize(_msgs[0].get("content") or ""))
                    _msgs = _msgs[1:]
                if _leading_system:
                    system_prompt = "\n\n".join(s for s in _leading_system if s)
                messages_payload = [
            ...
            def _call():
                try:
                    ...
                    # models_without_temperature()'s list: if Anthropic's 400
                    # mentions `temperature` at all, retry once without it
                    # rather than trusting an exact wording match (Anthropic's
                    # own phrasing for "this param isn't accepted anymore" has
                    # not been consistent across model generations).
                    if "temperature" in _base_kwargs and "temperature" in str(e).lower():
                        logger.warning(f"{request_id} → model={model} rejected temperature — retrying without it")
                        _base_kwargs.pop("temperature", None)
                        return self.client.messages.create(**_base_kwargs)
                    raise
