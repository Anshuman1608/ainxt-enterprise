...
DEFAULT_DSN = {
    ...
    # search_path — EVERY table this module touches (conversations, agent_runs,
    # workflow_history, model_usages, memory_entries) lives in the POSTGRES_SCHEMA
    # schema ("ainxt"), created by db/migrate.py. All SQL in this module is
    # UNQUALIFIED ("INSERT INTO model_usages ..."), so without this option the
    # connection resolves against the server default search_path ('"$user",
    # public') and NOT ONE of those tables is visible:
    #   to_regclass('model_usages') -> None   (but 'ainxt.model_usages' exists)
    # Every raw-SQL call then failed with 'relation "..." does not exist' while
    # .available stayed True (it only reflects that the socket opened), so the
    # failures surfaced only as per-call error logs — e.g. create_model_usage()
    # losing the /chat/video-generate usage/cost audit row on every video.
    # db/database.py pins the same search_path on all four SQLAlchemy engines
    # (and core/config.py on its DSN dict); this module was the lone omission.
    # public stays on the path so extension functions (pgvector, pg_trgm,
    # gen_random_uuid) keep resolving unqualified.
