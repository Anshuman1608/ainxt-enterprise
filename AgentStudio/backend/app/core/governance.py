...
def _resolve_budget_failure_fallback_model() -> str:
    ...
    budget store is blind) — ``get_default_model_id(prefer_free=True)`` only
    *prefers* a free model and can return a paid one if none is configured —
    so a non-local value is rejected and ``""`` is returned — which callers
    turn into a hard deny rather than untracked cloud spend.
    """
    from app.core.llm_handler import resolve_fallback_model
