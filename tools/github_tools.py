...
def github_list_my_repos(limit: int = 20) -> str:
    """
    List repositories the authenticated user can access, most-recently-updated
    first. Takes no repo argument — it is the natural GitHub equivalent of
    gitlab_list_my_mrs / jira_get_current_user and doubles as the connection
    probe (GET /user/repos needs only the PAT).

    Args:
        limit: Maximum number of repos to return (1–100, default 20).

    Returns:
        Formatted list of "owner/repo" lines, or an error message.
    """
    per_page = max(1, min(int(limit or 20), 100))
    result = _get(f"/user/repos?sort=updated&per_page={per_page}")
    if isinstance(result, dict) and "error" in result:
        return f"[Error listing repositories: {result['error']}]"
    if not isinstance(result, list):
        return f"[Unexpected response: {result}]"
    if not result:
        return "No repositories accessible with this token."
    lines = ["Your GitHub repositories (most recently updated first):"]
    for repo in result:
        vis = "private" if repo.get("private") else "public"
        lines.append(f"  {repo.get('full_name', '?')} [{vis}] — {repo.get('description') or 'no description'}")
    return "\n".join(lines)
