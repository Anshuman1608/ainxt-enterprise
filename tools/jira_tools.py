...
def set_credentials(email: str, token: str, base_url: str = "") -> None:
    ...
    base_url (optional) is the per-user Jira site root (e.g.
    https://your-org.atlassian.net) captured at connect time from Profile →
    API Token Vault. When empty the module falls back to the JIRA_URL env var,
    so existing single-site callers (the SDLC pipeline) are unaffected.
    """
    _thread_local.email = (email or "").strip()
    _thread_local.token = (token or "").strip()
    _thread_local.base_url = (base_url or "").strip().rstrip("/")
...
def _auth_header(email: str, token: str) -> str:
    """Atlassian Cloud REST auth header: ``Basic base64(email:api_token)``.

    Only the direct-call path (LLM_PROXY_URL unset — local/OSS deployments) uses
    this; the proxy path hands email/token to the LLM proxy as JSON fields. This
    helper was referenced by _do_direct()/_get_file_content() but never defined,
    so the first request on a proxy-less deployment raised NameError before the
    request was sent.
    """
...
def normalize_jira_site(raw: str) -> str:
    """Reduce whatever a user pasted for their Jira site to ``scheme://host``.

    Accepts a bare host ("org.atlassian.net"), the site root, an API URL
    ("…/rest/api/3"), or a full browser URL copied from the address bar
    ("https://org.atlassian.net/jira/for-you?tab=…") — all collapse to
    "https://org.atlassian.net". Returns "" for empty input.

    Lives here (a dependency-light util module) so both routers/connectors_router
    and connectors/engine can import it without a router→engine or
    engine→router import edge.
    """
    raw = (raw or "").strip()
    if not raw:
        return ""
    ...
    # Couldn't parse a host — fall back to trimming known API suffixes.
    site = raw.rstrip("/")
    for _suf in ("/rest/api/3", "/rest/api/2", "/rest/api"):
        if site.endswith(_suf):
            site = site[: -len(_suf)].rstrip("/")
            break
    return site


def _jira_base() -> str:
    # Per-user site URL injected by JiraAdapter (Profile → API Token Vault) wins;
    # JIRA_URL env is the single-site fallback for the SDLC pipeline / service acct.
    tl_base = getattr(_thread_local, "base_url", "")
    if tl_base:
        return tl_base.rstrip("/")
